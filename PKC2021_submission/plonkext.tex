...
This is turn risen a question---\emph{Are zkSNARKs' security models useful in
 real life?} Answering that becomes especially important when one realises that all
...
know the witness. Hence for secure real-life deployment of zkSNARKs
requires answering to the number of questions---\emph{How can the parties be sure that the
...
CRS for all circuits of a given size are called \emph{universal}. One of the first universal
...
soundness}.
Because of the Bellare et al.~\cite{AC:BelFucSca16} impossibility result, one cannot wish for a
...
The first universal zkSNARK with updatable and short CRS was
$\sonic$ proposed by Maller et al.~in \cite{CCS:MBKM19}. Eventually, Gabizon et
...
zkSNARKs used in practice. This is since, arguably, in the real life one simply cannot
assume that the adversary who tries to break security of a system does not
have access to any proofs provided by other parties using the same
zero-knowledge scheme. On the contrary, in the most popular applications of
zkSNARKs, like privacy-preserving blockchains, proofs made by all
blockchain-participants are usually public. Thus, it is only reasonable to require 
...
response property \emph{are simulation-extractable out-of-the box} when the
Fiat--Shamir transformation is applied to them. Although a similar problem has been
...
To show that our result is useful and practical we prove that two of the most efficient updatable and universal
...
First, following \cite{INDOCRYPT:FKMV12}, we need to show that
...
transcripts, each containing three messages and sharing the first message. For 
...
$\numberofconstrains$ is the number of constraints in the proven circuit---for
$\plonkprot$ and $(\noofc + 1)$--where $\multconstr$ and
...
that share a common root. Fortunately, one
...
by the prover are deterministic (intuitively,~the prover does not use its randomness
...
also had to show that the modified KZG polynomial commitment schemes \cite{AC:KatZavGol10}
proposed in \cite{EPRINT:GabWilCio19} and \cite{CCS:MBKM19} have a \emph{unique opening
...
able to produce a transcript indistinguishable from a transcript produced by
an
...
provided by their authors utilise trapdoors. For our reduction to work,
we need simulators that provide indistinguishable proofs relying only on
rearranging the ordering of messages and picking suitable verifier's challenges. 
That is, any PPT
...
where witness extraction requires more transcripts than merely two.
...
%\michals{12.11}{I am not sure that section is needed}
...
Section 3 compounds of new notions and theorems that instantiate our
framework. Then, in Section 4, we show our main result, that is a proof of
simulation-soundness for a class of zero-knowledge proofs of knowledge. In Section 5 we
show that $\plonk$ fulfils requirements of our framework and in fact is
simulation extractable. Due to the lack of space, the analogous result for
$\sonic$ has been moved to \cref{sec:sonic}.
...
``l'' in \emph{ldlog} relates to use of Laurent polynomials in the
assumption.}. Due to the lack of space, the assumptions have been moved to
...
the committer $\committer$ can prove  the receiver $\receiver$ that some
polynomial $\p{f}$ which $\committer$ committed to evaluates to $s$ at some point
$z$ chosen by $\receiver$.
$\plonk$ and $\sonic$ use variants of the KZG polynomial commitment scheme. We
denote the first by $\PCOMp$, presented in \cref{fig:pcomp}, and the latter by
$\PCOMs$, presented in \cref{fig:pcoms}. (Due to the lack of space,
\cref{fig:pcoms} was moved to \cref{sec:additional_preliminaries}.)
%
%
...
$\PCOM$:
\begin{description}
	\item[Evaluation binding] a $\ppt$ adversary $\adv$ which outputs a
		commitment $\vec{c}$ and evaluation points $\vec{z}$ has at most
		negligible chances open the commitment to two different evaluations
		$\vec{s}, \vec{s'}$. That is, let 
	$k \in \NN$ be the number of committed polynomials,
	$l \in \NN$ number of evaluation points,
	$\vec{c} \in \GRP^k$ be the commitments,  
	$\vec{z} \in \FF_p^l$ be the arguments the polynomials are evaluated at, $\vec{s},\vec{s}' \in \FF_p^k$ the evaluations, and 
...
\begin{description}
	...
	$l \in \NN$ number of evaluation points,
	$\vec{c} \in \GRP^k$ be the commitments,  
	$\vec{z} \in \FF_p^l$ be the arguments the polynomials are evaluated at, $\vec{s} \in \FF_p^k$ the evaluations, and 
...
\begin{description}
	\item[Commitment of knowledge] For every $\ppt$ adversary $\adv$ there
...
\end{description}
		Intuitively when a commitment scheme is a commitment of knowledge then if
		an adversary produces a (valid) commitment $c$, which it can open,
		then it also knows the underlying polynomial $\p{f}$ which commits to that
		value. \cite{CCS:MBKM19} shows, using AGM, that $\PCOMs$ is a commitment of
		knowledge. The same reasoning could be used to show that property for
		$\PCOMp$. We
		skip a proof for that fact due to the lack of space. 
...
\end{description}
	A CRS $\crs$ may come with a secret string called \emph{trapdoor} $\td$ that allows the simulator to
	simulate. 
	In this paper we distinguish simulators that requires a trapdoor to be able
	to provide an indistinguishable proof and those that do not. We call the
	former \emph{CRS-simulators} and denote by $\simulator_\td$ for trapdoor
	$\td$.
...
Faust et al.~\cite{INDOCRYPT:FKMV12} show that every such $\sigmaprot_\fs$ is
simulation-extractable. This result is presented in \cref{sec:forking_lemma}
along with the instrumental forking lemma, cf.~\cite{CCS:BelNev06}.
...
\begin{lemma}[General forking lemma II]
	...
	Let $\zdv$ be a $\ppt$ algorithm that on input $y, h_1, \ldots, h_q$ returns $(i, s)$ where $i \in \range{0}{q}$ and $s$ is called a side output. 
...
%\michals{12.11}{CONTINUE HERE}
%\michals{13.11}{that should be placed below, in the end of the section? In the
%proof?}
}

We note that in the original forking lemma the forking algorithm $\forking$
cf.~\cref{fig:forking_lemma} gets only as input $y$ and elements $h^1_1, \ldots, h^1_q$ are randomly picked from $H$ internally by $\forking$. However, assuming that
$h^1_1, \ldots, h^1_q$ are random oracle responses, thus are random, makes our
change only notational.
...
\verifier, \simulator)$. Let $\proofsystem_\fs$ be $\proofsystem$ after
Fiat--Shamir transform,
Denote by $a_0, \ldots, a_{\mu}$ protocol messages output by the prover, 
We say that $\proofsystem$ has \emph{unique responses after $i$} if for any
$\ppt$ adversary $\adv$: 
\[
	\prob{
		\begin{aligned}
...
\emph{rewinding-based knowledge soundness}. More precisely, we state that an
...
\begin{definition}[rewinding-based knowledge soundness]
...
outputs $\inp_{\advse}$. 
We show a reduction $\rdvur$ that utilises $\advse$, who outputs a valid
proof for $\inp_\advse$, to break the $\ur{k}$ property of $\ps$. 
...
when it fails to build a $(1, \ldots,  1, n_j, 1, \ldots, 1)$-tree of accepting transcripts
$\tree$ by rewinding $\advse$. Denote that event by $\event{\errfrk}$.
Note that for every acceptable proof $\zkproof_{\advse}$, we may
assume that whenever $\advse$ outputs in Round $k + 1$, a message
$\zkproof_{\advse}[k + 1]$, then $(\inp_\advse,
...
\emph{fresh}.}, i.e.~there is no simulated proof $\zkproof_\simulator$ on
$\inp_\simulator$ such that $(\inp_\advse, \zkproof_\advse \range{1}{k + 1})
= (\inp_\simulator, \zkproof_\simulator \range{1}{k + 1})$. Otherwise, the game would be already interrupted by the
...
To explain importance of the unique response property we describe our
extractor here. The extractor takes as input relation $\REL$, CRS $\crs$,
$\advse$'s code, its randomness $r$, the output instance $\inp_\advse$ and proof
$\zkproof_\advse$, as well as the list $Q$ of simulated proofs (and their
instances) and the list of random oracle queries and responses $Q_\ro$. 
Then, $\ext$ starts a forking algorithm $\genforking^{n_j + 1}_\zdv(y,h_1, \ldots, h_q)$ for $y =
(\advse, r, \inp_\advse, \zkproof_\advse, Q)$ where we set $h_1, \ldots, h_q$ to be the consecutive
queries from list $Q_\ro$. 
We run $\advse$ internally in $\zdv$. 
...
%
Note, that since the view of the
adversary run inside $\zdv$ is the same as its view with access to real
random oracle and simulator, it produces exactly the same output. After the
first run, $\zdv$ outputs the index of a random oracle query that
was used by $\advse$ to compute the challenge
$\ro(\zkproof_\advse\range{0}{j - 1}.\ch)$ it had to answer in the $j$-th
round and adversary's transcript, in $\genforking$ description denoted by
$s_1$. Then new random oracle responses are picked and the adversary is
rewound to the point just prior the response to RO query
$(\zkproof_\advse\range{0}{j - 1})$. The adversary gets a random oracle
response from a new set of responses $h^2_i, \ldots, h^2_q$. If the
adversary requests a simulated proof after seeing $h^2_i$ 
...
$\ro(\zkproof_\advse\range{0}{j - 1})$, and a new transcript form the
adversary $s_2$. $\zdv$ is run with different random oracle responses till
$n_j + 1$ valid transcripts are collected.
...
\[
	...
	\waccProb\right)\,.
\]
\ngame{3} This is identical to $\game{2}$ except that now the environment uses the
...
transcript that does not break $\ur{k}$-ness of $\ps$, then $\waccProb \geq
\accProb - \epsur$, where $\accProb$ is the probability of $\advse$ outputing an acceptable proof as defined in \cref{def:simext}. It thus holds
\[
	\label{eq:frk}
...
show that $\plonkprot$ has the $\ur{2}$ property, cf.~\cref{lem:plonkprot_ur}.
...
$\ur{2}$ and show, in a similar fashion to \cite{INDOCRYPT:FKMV12}, that it is simulation-extractable. That is, we build reductions that given a simulation extractability adversary $\advse$ either breaks the protocol's unique response property or breaks the $\dlog$ assumption, if extracting a valid witness from a tree of transcripts is impossible. See \cref{thm:plonkprotfs_se}.
...
$\epsbind$-binding and has unique opening property with security
$\epsop$, then probability that a $\ppt$ adversary $\adv$ breaks
$\plonkprot$'s $\ur{2}$ property is at most $\epsk + 2\cdot\epsbind + \epsop$.% assuming the polynomial commitment scheme $\PCOMp$
%is of knowledge.
\end{lemma}
\begin{proof}[sketch]
Let $\adv(\REL,\crs = (\gone{1, \chi, \ldots, \chi^{\numberofconstrains
+ 2}}, \gtwo{\chi}))$ be an adversary trying to break the $\ur{2}$-ness of
$\plonkprot$.
It is sufficient to observe that the first 2 rounds of the protocol
determines, along with the verifiers challenges, the rest of it. 
In Round 3 the adversary outputs a commitment to
polynomial $\pt(X)$ which assures that all constraints of
the system are fulfilled. Since the commitment scheme is deterministic,
there is only one value $c$ that is a commitment to $\pt(X)$. 
Assume that $\adv$ outputs $c' \neq c$ and is later able to open $c'$ to
$y = \pt(\chz)$, where $\chz$ is a random point determined later.
...
$\gone{\p{f}(\chi)} = c'$.
Thus if $c' \neq c$ and the commitment scheme is evaluation binding then
$\adv$ when picking $c'$ picks it as a commitment to $\p{f}$ which evaluates
at $\chz$ to $\pt(\chz)$. The probability of that is negligible as there can
only be degree of $\pt$ many overlapping points and $\chz$
remains random for $\adv$ when it computes $c'$. Hence, the probability that
$\adv$ is able to produce two different outputs of Round 3 is upper-bounded
by $\epsk + \epsbind$.
...
\conclude
Hence the probability that $\adv$ breaks $\ur{2}$-property of $\plonkprot$ is upper-bounded by $\epsbind + \epsop$.
\qed
\end{proof}
}
...
Let $\adv$ be a $\ppt$ algebraic adversary. The probability $\epsss$ that $\adv$ breaks 
 $(1, 1, 1, \numberofconstrains + 3, 1)$-rewinding-based knowledge soundness of $\plonkprot$ is upper-bounded as
 \[
...
Assume that  $\plonkprot$ is $\ur{2}$ with security $\epsur(\secpar)$, and computational special-sound with security $\epsss(\secpar)$.
Let $\ro\colon \bin^* \to \bin^\secpar$ be a random oracle. 
...
% Probability $\frkProb$ is a lower bound for a successful witness extraction from two transcripts.  Let
% $\trans_1 = (\inp, a, b, z)$ and $\trans_2 = (\inp, a, b', z')$ be the
% transcripts.  Both $\trans_1$ and $\trans_2$ have to be \emph{acceptable},
% i.e.~$i > 0$ and the probability that $\zdv$ accepts a transcript is
% denoted by $\accProb$. Index $i$ can be interpreted as an index of $h_i$
% which was sent as a challenge for $(\inp, a)$, i.e~the index of a random
% oracle query that has been used to produce prover's message in the point the
% proof branches (regarding the random oracle queries). This index has to be guessed by
...
\begin{lemma}
	...
	$\adv$ breaks rewinding-based knowledge soundness of $\sonicprot$ is upper-bounded as
	\[
...
%
	Let $\adv$ be an adversary that produces a $(1, 1, \noofc + 1, 1, 1)$-tree of acceptable
	...
	polynomial $\p{r}(X, y)$ has degree at most $(\noofc)$
	and since $\adv$ answered correctly on $(\noofc + 1)$
	different challenges $z$ (for the sake of concreteness let us call them
	$z_1, \ldots, z_{\noofc + 1}$) then $(\multconstr +
...
$\pR,\pT,\pa,\pb$ from the proof (the other proof elements are fixed by these elements
...
$k_{q}$ is
the instance value). When the proven instance is correct, $\pt(0,Y)$ is a zero
...
\[
		\extProb  \geq \frac{1}{q^{\noofc}} (\accProb - \epsur)^{\multconstr +
		\linconstr + 1} - \eps.
	\]
	 for some negligible $\eps$.,
	 % $\multconstr$ and $\linconstr$ being,
...
\end{description}
	...
	\begin{theorem}[Simulation soundness]
		...
		$\eps_\dlog(\secpar)$-hard, $\plonkprot$ is sound and $\ur{2}$ with security $\epss(\secpar)$ and $\epsur(\secpar)$ respectively. 
