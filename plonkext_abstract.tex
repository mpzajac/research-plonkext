...
\begin{document}
	...
	Hence, there is no computational overhead compared to other works that took an existing zkSNARK and apply some existing transformation which inevitable lead to efficiency loss---either because the the final proof is longer (\michals{31.08}{Cite gabizon on SE of Groth}\cite{}), the prover has to prove a more complicated statement (\michals{31.08}{cite OR constructions}), or, in the context of showing correctness of computation, the arithmetisation of choice is less efficient (\michals{31.08}{Groth Maller}).
