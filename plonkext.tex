...
For the sake of completeness we also provide reduction\footnote{Called sometimes \emph{a special extractor}.} $\rdv_\se$ that from a given tree of acceptable transcripts extracts a witness (effectively making $\advse$ lose). 
...
\begin{itemize}
  ...
  soundness}
\ncase{When $\nevent{E}_0$ happens}
Since in this case all of the transcripts from $\tree$ are acceptable by the polynomial verification equation, that is for each transcript $\vereq(X) = 0$, then the special extractor $\rdv_\se$ reveals the witness $\wit$, such that $\REL(\inp, \wit)$ holds, by interpolating polynomials $\p{a}(X), \p{b}(X), \p{c}(X)$. 
This happens except some negligible probability $\epsbatch$, which corresponds to the verification error that could occur due to use of batching in verification.
Hence
\[
