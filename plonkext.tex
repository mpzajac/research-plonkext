...
\begin{theorem}
	$\plonkprot$ is simulation-extractable agains algebraic adversaries.
\end{theorem}
\begin{proof}[Proof idea]
	...
	At first $\reduction$ obtains $\crs$ from $\plonkprot.\kgen$ and passes it to $\advse$. 
	Assume that 
	\begin{itemize}
		\item the adversary makes $q_\simulator$ queries to the simulator obtaining $q_\simulator$ simulated proofs;
	...
	\end{itemize}
	The reduction guesses query $i_\simulator$ and $r$ with probability $1/(3 q_\simulator)$ and assume it guesses correctly.
	...
	\begin{itemize}
		...
		\item If $r = 1$, $\rdv$ pauses its interaction with the verifier and continues generating the proof as $\simulator$ would generate.
	...
	\end{itemize}
	When $\advse$ decides to continue generating its malicious proof the reduction resumes its communication with $\plonkprot.\verifier$. That is, for each $\advse$ message it responds with the corresponding challenge from the verifier. In the end, the adversary returns a proof $\zkproof$ along with a vector of coefficients for each group element in $\zkproof$.
	
	Assume the proof is acceptable. 
