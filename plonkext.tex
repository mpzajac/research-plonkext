...
\begin{proof}
  ...
  by the verifier if the proof system be sound. \markulf{28.08}{Given the commitment
...
$\cdv_\plonkprot$ or $\cdv_\dlog$ \markulf{28.08}{For dlog the CRS needs to be
  computed from the monomials?}, where the former represents game where the reduction tries to break soundness of $\plonkprot$ and the latter game of breaking the dlog assumption. We run the reductions in parallel and use $\rdv$ to denote either $\rdvks$ or $\rdvdlog$.
...
proof.\markulf{27.08}{Why is this obvious? The adversary could randomize a
  ...
  unique response property here.} The reduction tries to guess which queries,
...
\markulf{27.08}{Maybe include all oracle queries, not just adversarial ones
  here? At least one query needs to be adversarial however?}, are going to be used in the final proof. Denote the queries by $h_{i_1}, \ldots, h_{i_6}$. 
