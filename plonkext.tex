...
\p{z}(\chz\omega)\]
Given the evaluations $\simulator$ computes polynomial $\p{r}(X)$ as an honest
prover would, i.e.  
\[
		\p{r}(X) = 
		\begin{aligned}
...
Next, the simulator computes $\ev{t}$ as 
\[
	\ev{t}  = \frac{1}{\p{Z_H}(\chz)}
		\Big(
			\p{r} (\chz) + \pubinppoly(\chz) - (\p{a}(\chz) + \beta \p{S_{\sigma
			1}}(\chz) + \gamma) (\p{b}(\chz) + \beta \p{S_{\sigma 2}}(\chz) + \gamma) \\
			(\p{c}(\chz) +
			\gamma)\p{z}(\chz \omega) \alpha - \lag_1 (\chz) \alpha^2
		\Big) \,.
\]
Finally, $\simulator$ picks randomly a polynomial $\p{t}$ such that $\p{t}
(\chz) = \ev{t}$ and concludes this round as an honest prover
would by dividing $\p{t}$ into $\p{t_{hi}}, \p{t_{mid}}, \p{t_{lo}}$ and
outputting $\gone{\p{t_{hi}}(\chi), \p{t_{mid}}(\chi), \p{t_{lo}}(\chi)}$. 
After this round the simulator outputs $\chz$ as a challenge.

In the next round, Round 4, the necessary evaluations at $\chz$ are already
...
%
		...
		\michals{21.X}{Update the ss proof for Plonk with $\epss$} 
		Note that the 
...
\subsection{Zero knowledge}
\michals{26.X}{ We need to show that it is possible to simulate the proof using
a simulator that does not require any trapdoor (simulation by reprogramming
the oracle, or "zero knowledge in the classical sense", i.e. everybody can
provide an acceptable simulated transcript}
% \cref{sec:sonic}
\begin{lemma}
...
that $\ptxsim$ is indeed $0$ when evaluated at $y$ assures that the transcript
