...
\begin{definition}
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$, let $\adv$ and
  $\tdv$ be $\ppt$ algorithms. $\tdv$ has oracle access to $\adv$ and provides
  it with (oracle) access to random oracle $\ro$ and $\ps$ simulator
  $\simulator$ -- more precisely $\tdv$ has an internal procedure $\bdv$ that
  provided $\srs$ and random oracle queries' responses $h_1, \ldots, h_1$ gives
  $\adv$ access to the random oracle and simulates proof for it. In the end,
  $\bdv$ returns the proof $\adv$ returns and 

  \begin{figure}
    \centering
 		\fbox{ \procedure{$\tdv (\srs \sample \kgen(\REL))$}
      {h_1^{1}, \ldots,
      h_{q}^1 \sample H
      (i, \inp, \zkproof_1) \gets \bdv(\srs, h_1^{1}, \ldots, h_{q}^{1})\\
        % i_1 \gets i\\
        \pcif \verifier(\srs, \inp, \zkproof_i) = 0\ \pcreturn (0, \bot)\\
        \pcfor j \in \range{2}{m}\\
        \pcind h_{1}^{j}, \ldots, h_{i - 1}^{j} \gets h_{1}^{j - 1}, \ldots,
        h_{i - 1}^{j - 1}\\
        \pcind h_{i}^{j}, \ldots, h_{q}^{j} \sample H\\
        \pcind (i_j, \zkproof_j) \gets \bdv(\srs, h_1^{j}, \ldots, h_{i -
          1}^{j}, h_{i}^{j},
        \ldots, h_{q}^{j})\\
        \pcind \pcif \verifier(\srs, \inp, \zkproof_i) = 0\ \pcreturn (0, \bot)\\
        %\pcind \pcif i_j = 0 \lor i_j \neq i\ \pcreturn (0, \bot)\\
        % \pcif \exists (j, j') \in \range{1}{m}^2, j \neq j' : (h_{i}^{j} =
        % h_{i}^{j'})\
        % 
        % pcreturn (0, \bot)\\
        \pcelse \pcreturn (1, \tree)}
    }
  \end{figure}

  We say that $\ps$ is $(1, \ldots, n_k, \ldots, 1)$-forking special sound if
  for any adversary probability that
  \[
    \Pr\left[
      \begin{aligned}
