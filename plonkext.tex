...
$\sonic$ proposed by Maller et al.~in \cite{CCS:MBKM19}. This scheme was later
improved by Gabizon in \cite{EPRINT:Gabizon19c}. Eventually Gabizon et
...
algebraic group model (AGM) and random oracle (RO) model to show their
security and use their own representation of circuits instead of the
``standard'' QAP. However, they are also one of the  most interesting schemes
for practitioners for their efficiency, security models and universality.
...
first round as an honest prover would. Because of the randomisers used
polynomial $\pr(X, Y)$ computed by the simulator is indistinguishable from a
polynomial provided by an honest user for a distinguisher that has less than
three evaluations of $\pr(X, Y)$. 
...
random, $\ptxsim(Y)$ is a polynomial of degree $\linconstr +
\multconstr$ with $0$ coefficients for $Y^{i}$, for $i \in
\range{0}{\multconstr}$. Also, $\ptxsim(Y) / Y^{\multconstr + 1}$ is
a degree-$(\linconstr - 1)$ polynomial of random coefficients. Probability
that a random degree-$(\linconstr - 1)$ polynomial over $\FF_p[Y]$ has a root is at
least $\infrac{1}{(\linconstr - 1) !}$, see \cref{lem:root_prob} for a
proof of that bound. Since we assume that $\linconstr =
\poly$, we can say that the polynomial $\ptxsim$ as picked by the simulator
has roots with non-negligible probability. Furthermore, these roots can be
found and are random $\FF_p$ elements. Hence, the challenge $y$ picked by the
simulator comes from the same distribution as if it was picked by an honest
verifier.
