...
random, $t(0,Y) / Y^{\multconstr + 1}$ is
...
= \poly$, we can say that the polynomial $t(0,Y)$ as computed by the simulator
has roots with non-negligible probability. Furthermore, these roots can be found
and the simulator picks fresh $\vec{a}, \vec{b}$ until $t(0,Y)$ has a root. As
...
fact that $t(0,y) = 0$ guarantees that the transcript provided by the simulator is
