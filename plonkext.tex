...
\begin{theorem}[Simulation extractability of the Fiat--Shamir transform III]
	...
	Let $\proofsystem = (\prover, \verifier, \simulator)$ be an interactive strongly $\ur{i}$-protocol for a relation $\REL$ \michals{10.06}{Here, new version of \emph{strongly} $\ur{i}$-protocol -- even unbounded adversary cannot provide two valid proofs that differ after $i$}. 
...
\begin{proof}
	...
	\begin{itemize}
		\item $\simulator_{\fs, \ro}$, that takes care about random oracle queries. Assume that there is an upper bound $Q = \poly$ for the number of queries that may be asked.
		For a query $x$ simulator $\simulator_{\fs, \ro}$ checks whether a list of queries $L$ contains a pair $(x, y)$, for some $y$, and, if that is the case, outputs $y$. 
		...
		\item $\simulator_{\fs, \zkproof}$ that provides simulated proofs. The simulator may also program random oracle queries. That is, it may add pair $(x, y)$ to $L$ unless there is a pair that have $x$ on the first position. In that case it outputs $\bad$.
	\end{itemize}
	...
	The instance and proof are output after $\adv_\se$ learnt simulated proofs from $\simulator_{\fs, \zkproof}$.
	
	For $j > i$ divide the adversary's proof $\zkproof$ into two parts: 
	first, $\vec{a}^\heartsuit = (a_0, \ldots, a_j)$ and second $\vec{a}^{\spadesuit} = (a_{j + 1}, \ldots, a_\mu)$.
	...
	Note that $(\inp, \vec{a}^\heartsuit)$ is a query to the random oracle that is performed by the adversary. 
	Otherwise, if the query was made by the simulator $\simulator_{\fs, \zkproof}$ then, since $\proofsystem$ is an $\ur{i}$-protocol and $j \geq i$, a proof starting with $(\inp, \vec{a}^\heartsuit)$ is in the list $Q$ and cannot be used by the simulation-extractability adversary $\adv_\se$.
	
	For each $j > i$, after the oracle was queried with $(\inp, \vec{a}^\heartsuit)$, the adversary $\adv_\se$ is rewound and a new randomness for $\simulator_{\fs, \ro}$ is picked. This step is repeated $n_j$ times.
	More precisely, assume that $(\inp, \vec{a}^\heartsuit)$ was $J_j$-th query to $\simulator_{\fs, \ro}$, then the sequence of answers $y_1, \ldots, y_Q$ changes to $y_1, \ldots, y_{J_j}, y'_{J_j + 1}, \ldots, y'_{Q}$, for some fresh and independent $y'_{J_j + 1}, \ldots, y'_{Q}$ picked accordingly to $r$. 
	Since the first $J_j$ queries do not change, the adversary, whose randomness tape remains the same, submits $(\inp, \vec{a}^\heartsuit)$ as its $J_j$-th query.
