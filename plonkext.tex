...
\end{description}
A SRS $\crs$ may come with a secret string called \emph{trapdoor} $\td$ that
...
that do not. We call the former \emph{SRS-simulators} and denote by
...
SRS $\crs$, $\advse$'s code, its randomness $r$, the output instance
...
the basis compound of the input group elements, i.e.~$\plonkprot$'s SRS. See
...
\end{lemma}
\begin{proof}
  Let $\crs$ be $\plonkprot$'s SRS and denote by $\crs_1$ all SRS's
...
\begin{proof}
  ...
  provides the adversary with a SRS $\crs$, then the adversary outputs an
...
$\chz$ and computes the SRS $\crs$ from $\chi$. Elements $b_i$ are interpreted as
