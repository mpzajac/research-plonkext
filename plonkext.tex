...
\begin{proof}
  ...
  one to break the discrete logarithm problem, probability that the real
...
\begin{proof}
  ...
  As previously, the adversary cannot provide two different evaluations for the
  committed polynomials, since that would require breaking the evaluation
  binding property, what happens (by the union bound) with probability at most
  $7 \cdot (\epsbind + \infrac{2}{\abs{\FF_p}})$. The latter terms comes since
  the adversary does not provide an opening for each of the commitment
  separately, but does that in batch. That comes with $\infrac{1}{\FF_p}$ of
  security loss. Another $\infrac{1}{\FF_p}$ security loss comes from the fact
  that the verifier does not verify each of the commitments openings separately,
  but does so in batches as well.
...
$\epsid(\secpar)$ and the discrete logarithm advantage is bounded by
$\epsdlog(\secpar)$, $\plonkprot$ is
$(\epsss(\secpar), 3, 3 \noofc + 1)$-forking sound for
$\epsss(\secpar) \leq \epsid(\secpar) + \epsdlog(\secpar)$ against algebraic
adversaries.
\end{lemma}

\begin{proof}
  We show that the adversary who breaks forking soundness can be used to break
  hiding properties of the polynomial commitment scheme. The proof goes by game
  hops. Let $\tree$ be the tree produced by $\tdv$ by rewinding $\adv$. Note
  that since the tree branches after Round 3, commitments $\gone{\p{a} (\chi),
    \p{b} (\chi), \p{c} (\chi), \p{z} (\chi), \p{t_{lo}} (\chi), \p{t_{mid}}
    (\chi), \p{t_{hi}} (\chi)}$, challenges $\alpha, \beta, \gamma$ are the
  same. 

  \ncase{Game 0} In this game the adversary $\adv$ wins if $\tdv$ fails to
  output tree $\tree$ such that $\extt$ is able to extract a witness out of
  it and none of commitments
  $\gone{\p{a} (\chi), \p{b} (\chi), \p{c} (\chi), \p{z} (\chi), \p{t_{lo}}
    (\chi), \p{t_{mid}} (\chi), \p{t_{hi}} (\chi)}$ come from a simulated proof.

  \ncase{Probability that $\adv$ wins Game 0 is negligible}

  \ncase{Game 1} In this game the adversary additionally wins if at least one of
  the commitments $\p{a} (\chi), \p{b} (\chi), \p{c} (\chi), \p{z} (\chi)$ comes
  from a simulated proof. W.l.o.g assume that this commitment is $\p{c} (\chi)$
  \michals{9.9}{Need to cover other cases as well, i.e.~the when $\adv$ uses
    simulated $\p{t_{lo}}$ etc}

  \ncase{$\game{0} \mapsto \game{1}$} We show a reduction $\rdv$ that uses
  $\adv$ and $\tdv$ to break hiding property \michals{9.9}{Define hiding
    property -- w.r.t.~masking, adversary can get evaluation at number of points
    (here -- 1)} of the commitment scheme. $\rdv$ proceeds as follows:
  \begin{enumerate}
  \item Given polynomial commitment SRS $\srs_{\PCOM}$, produce $\plonk$'s SRS
    $\srs$.
  \item Pick random polynomials $\p{p} (X), \p{p'} (X) \in \FF^{< |\HHH|} [X]$
    and send them to the polynomial commitment challenger $\cdv$.
  ...
  \item If index $s$ has not been guessed correctly, rewind $\adv$ to the start
    and pick a new $s$. Since $S = \poly$ probability that the correct $s$ will
    be guessed in polynomial time is overwhelming. That is, the reduction works
    in expected polynomial time.
  \item Since $\tree$ contains $\noofc + 1$ evaluations of $\p{c}$, the
    polynomial can be reconstructed. Hence $\rdv$ learns whether it equals to
    $\p{p}$ or $\p{p'}$.
  ...
  \item Guess the simulation query index $s$ the polynomial(s) come from and
    whether the polynomial is $\p{t_{lo}} (\chi), \p{t_{mid}} (\chi)$,
    $\p{t_{hi}} (\chi)$. W.l.o.g.~assume it is $\p{t_{lo}}$.
  ...
  \item Let $\tdv'$ be an algorithm that behaves exactly as $\tdv$, except when
    ...
    \item Before polynomials $\p{t}$ are computed, pick random $\chz$ and get
      evaluation $p_\chz = \p{p_b} (\chz)$. \michals{9.9}{If we give hiding
        adversary possibility to evaluate polynomials, we need to use masking}
    \item Let $\ev{t_{lo}}$ be the evaluation of the real $\p{t_{lo}} (\chz)$.
  ...
  \item If index $s$ has not been guessed correctly, rewind $\adv$ to the start
    ...
    be guessed in polynomial time is overwhelming. That is, the reduction works
    in expected polynomial time.
  \item Since $\tree$ contains $\noofc + 1$ evaluations of $\p{t_{lo}}$, the
    polynomial can be reconstructed. Hence $\rdv$ learns whether it equals to
    $\p{p_0}$ or $\p{p_1}$.
  \item $\rdv$ returns its guessing bit to $\cdv$. 
  \end{enumerate}

  \ncase{$\game{1} \mapsto \game{2}$} Note that since the adversary did not win
  in Game 1, all commitments s $\p{a} (\chi), \p{b} (\chi), \p{c} (\chi), \p{z}
  (\chi)$ are fresh, that is, they are not a commitments returned by the
  simulator \michals{9.9}{what if adversary got say, $p_1, p_2$ from the
    simulator and output $p_3 = p_1 + p_2$? USE AGM}
\end{proof}
...
\begin{proof}
  ...
  \ncase{$\game{0} \mapsto \game{1}$} Assume the adversary wins in Game 1, but
...
\begin{proof}
  ...
  \ncase{$\game{0} \mapsto \game{1}$} Probability that the adversary wins in
