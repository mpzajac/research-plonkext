...
\begin{figure}
  ...
  \caption{Extractor and tree finding algorithm. $\validtree(\tree)$ is
    ...
    $\REL(\inp, \wit) = 1$.
    % \michals{18.02}{The problem with current write up ---
    %   The adversary seeing $h'_j$ may decide to stop current trial to make a
    %   proof and start making the proof over. In that case, he would continue
    %   using ro queries from sequence $(h'_j, \ldots, h'_q)$. Hence, he would
    %   produce a proof that may be totally different from the previous one, i.e.
    %   even the instance may differ, thus the proof cannoit be joined to the
    %   tree. (Even when we force the adversary to use the same instance, he can
    %   change all the following messages. In that case the proof probably cannot
    %   be joined to the tree as well.)}
  }
...
analised probability that a tree finding algorithm is able to produce a required
