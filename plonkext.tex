...
using its private input called \emph{witness} and
...
\begin{lemma}[General forking lemma II]
	...
	Denote by $\ig$ a randomised instance generator. We denote by $\waccProb$ the probability
	\[
  ...
  to $\genforking$?} 
	...
	\[
		\frac{\waccProb^m}{q^{m - 1}} - \waccProb \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right)
	\]
...
\begin{proof}
	...
	First let denote by $\waccProb(y)$ and $\frkProb(y)$ the following probabilities
	\begin{align*}
		\waccProb(y) & =  \condprob{i \neq 0}{h_1, \ldots, h_q \sample H;\ (i, s) \gets \adv(y, h_1, \ldots, h_q)}\,.\\
	...
	\begin{equation}\label{eq:frkProb_y}
		\frkProb(y) \geq 
		\frac{\waccProb(y)^m}{q^{m - 1}} - \waccProb(y) \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right)
		\end{equation}
	Then with the expectation taken over $y \sample \ig$, we have
	\begin{align}
		\frkProb & = \expected{\frkProb(y)} \geq
		\expected{\frac{\waccProb(y)^m}{q^{m - 1}} -  \waccProb(y) \cdot \left(1 -
		\frac{h!}{(h - m)! \cdot h^m}\right)} \label{eq:use_eq1}\\
						 & = \frac{\expected{\waccProb(y)^m}}{q^{m - 1}} -
						 \expected{\waccProb(y)} \cdot  \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right) \\
		& \geq \frac{\expected{\waccProb(y)}^m}{q^{m - 1}} -
		\expected{\waccProb(y)} \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right) \label{eq:by_lemma_jensen}\\
		& = \frac{\waccProb^m}{q^{m - 1}} -  \waccProb \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right)\label{eq:by_accProb}\,.
	\end{align}
	Where \cref{eq:use_eq1} comes from \cref{eq:frkProb_y};
	\cref{eq:by_lemma_jensen} comes from \cref{lem:jensen}; and
	\cref{eq:by_accProb} holds by the fact that $\expected{\waccProb(y)} =
	\waccProb$.
...
$\genforking_{\adv}^{m}$ we have
	\begin{align*}
		...
		& = \prob{i_j = i_{j'} \land
		i_j \geq 1 \text{ for } (j, j') \in J} - \waccProb(y) \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right)\,.
	\end{align*}
	It remains to show that $\prob{i_j = i_{j'} \land i_j \geq 1 \text{ for }
	(j, j') \in J} \geq \infrac{\waccProb(y)^m}{q^{m - 1}}$.
...
h_{\iota - 1})$ to \[
	...
	\end{align*}
	Importantly, $\sum_{\iota = 1}^q \expected{X_{\iota}} = \waccProb(y)$.
	...
	\begin{gather}
		...
		\frac{1}{q^{m - 1}} \cdot \waccProb(y)^{m} \leq \sum_{i = 1}^{q} \expected{X_i}^m\,.
	\end{gather}
	Finally, we get
	\[
		\frkProb(y) \geq \frac{\waccProb(y)^m}{q^{m - 1}} - 
		 \waccProb(y) \cdot \left(1 - \frac{h!}{(h - m)! \cdot h^m}\right)\,.
	\]
	\qed
\end{proof}
