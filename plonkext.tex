...
\markulf{Removed paragraph on sigma protocols}
...
\oursubsub{Simulation sound NIZKs.}
...
% \begin{figure}
% 		\fbox{
% 		\procedure{$\zdv (y,h_1^{1}, \ldots, h_{q}^{1}; \rho)$}		
% 		{
%       (\rho_\sim, \rho_\adv)
% 		\rho \sample \RND{\zdv}\\
% 		(i, s_1) \gets \zdv(y, h_1^{1}, \ldots, h_{q}^{1}; \rho)\\
...
\begin{proof}
  ...
  % $\env$ that internally runs a simulation soundness adversary $\adv$, provides
  % it with access to a random oracle and simulator, and when necessary rewinds
  % it. The games differ by various breaking points, i.e.~points where the
...
\[
	\extProb \geq \frac{1}{q^{n - 1}} (\accProb - \epsur(\secpar))^{n} -\eps(\secpar)\,.
\] 
thus $\psfs$ is forking simulation extractable with extraction error $\epsur(\secpar)$.
\qed
\end{proof}
...
%Due to page limit, we omit description of \plonk{} here and refer to Supp.~Mat.~\ref{sec:plonk_explained}
...
% who outputs a valid proof for $\inp_\advse$, to break the $\ur{2}$ property of
% $\plonkprot$.
% 
...
% $\rdvur$ breaks $\ur{2}$-ness of $\plonkprot$, what may happen with some
...
% acceptable and does not break $\ur{2}$-ness of $\plonkprot$. From the
...
% that does not break $\ur{2}$-ness of $\plonkprot$, then $\accProb \leq
