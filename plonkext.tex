...
$\PCOMs$, presented in \cref{fig:pcoms}.
%
\begin{figure}[t!]
...
\begin{lemma}[General forking lemma, cf.~\cite{INDOCRYPT:FKMV12,CCS:BelNev06}]
	...
	\begin{figure}[t]
		...
		\procedure{$\forking_\zdv (y)$}
		{
			...
			(i, s) \gets \zdv(y, h_1, \ldots, h_q; r)\\
			\pcif i = 0\ \pcreturn (0, \bot, \bot)\\
			h'_{i}, \ldots, h'_{q} \sample H\\
			(i', s') \gets \zdv(y, h_1, \ldots, h_{i - 1}, h'_{i}, \ldots,  h'_{q}; r)\\
...
\begin{lemma}[General forking lemma II]
	...
	1}{y \gets \ig;\ h_1, \ldots, h_{q} \sample H;\ (b, \vec{s}) \gets \genforking_{\zdv}^{m}(y)}$ is at least 
	\[
	...
	\begin{figure}[t]
		...
		{
			r \sample \RND{\zdv}\\
			(i, s_1) \gets \zdv(y, h_1^{1}, \ldots, h_{q}^{1}; r)\\
...
Let $\rdvur$ run $\advse$ internally as a black-box:
  ...
  Assuming that $\advse$ does not break the unique response property is
	...
	extractor can extract from it a valid witness is futile. \markulf{13/11/20}{We
  need multiple proofs with the same first messages but different challenges.} The unique
  ...
  instances) and the list of random oracle queries and responses $Q_\ro$. 
	Then, $\ext$ starts a forking algorithm $\genforking^{n_j + 1}_\zdv(y)$ for $y =
  (\advse, r, \inp_\advse, \zkproof_\advse, Q, Q_\ro)$ however
	with minor modification---instead of randomly picking $h^1_1, \ldots, h^1_q$
	inside $\genforking$ we set $h^1_1, \ldots, h^1_q$ to be the consecutive
	queries from list $Q_\ro$. In fact, that change does not affect any
	probability of success of $\genforking$ as elements in $Q_\ro$ are random
	oracle responses and thus are random as required. 
	We run $\advse$ internally in $\zdv$. \markulf{13/11/20}{They are random but
    correlated with $Q_\ro$, maybe we have to say something about this.}
	
	To assure that the adversary
	$\advse$ produces the in the first execution of $\zdv$ the same transcript
	as it produces with $r$, $Q$, $Q_\ro$, and outputs the same $\inp_\advse,
	\zkproof_\advse$ algorithm $\zdv$ provides $\advse$ with randomness $r$ and
	on its queries to the random oracle and simulator answers with responses
	already contained in $Q_\ro$ and $Q$. Note, that since the view of the
	...
	first run is done, $\zdv$ outputs the index of a random oracle query that
	...
	$s_1$. Then a new random oracle responses are picked and the adversary is
	rewound to the point just prior he gets answer for query on
	$(\zkproof_\advse\range{0}{j - 1})$. The adversary gets a random oracle
	response from a new set of responses $h^2_1, \ldots, h^2_q$. If the
	adversary needs a simulated proof for an instance $\inp$ that is not in $Q$
	then $\zdv$ picks the simulated proof by its own. Eventually, $\zdv$ outputs
	index $i_2$ of a query that was used by the adversary to compute
	...
	$n_j + 1$ transcripts are collected.

	If the unique response property does
	not hold then in the $j$-th execution of $\zdv$ the adversary could reuse a challenge that it learnt from observing
	proofs in $Q$. In that case, $\zdv$ would not be able to output $i_j$ and
	have to output $0$, what would make extractor fail.

	\markulf{03.11.2020}{Recall here why we need UR to use forking lemma.}
	\michals{4.11}{Check the explanation above}\markulf{12/11/2020}{This is a good
    informal explanation, but I was looking for a formal reason why the forking
    lemma only works with that condition. Suspect it has to do with hash values
    having to come from adversary}
