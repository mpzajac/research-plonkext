...
Nevertheless, the Fiat--Shamir heuristic is a popular design tool when it comes
to constructing \emph{zero knowledge succinct arguments of knowledge}
(zkSNARKs). zkSNARKs have seen indisputable progress in the recent years
e.g.~\cite{AC:Groth10a,TCC:Lipmaa12,EC:GGPR13,SP:PHGR13,AC:Lipmaa13,AC:DFGK14,EC:Groth16,SP:BBBPWM18}
to name a few. Their succinctness make them especially useful for deployment in
real systems,
e.g.~\cite{REPO:Zcash20,ARXIV:RonZaj19,REPO:Zeth20,REPO:Celo20,REPO:Aztec20}. However,
many works, including \sonic{}~\cite{CCS:MBKM19}
\plonk{}~\cite{EPRINT:GabWilCio19}, and \marlin~\cite{EC:CHMMVW20}, design
interactive protocols and prove them secure. Unfortunately, they then only
\emph{conjecture} the security for their non-interactive variants by employing
the Fiat--Shamir heuristic.  However, proper security analysis of zkSNARKs
utilizing the heuristic is of great importance especially now, when they are
used to secure cryptoassets of immense value. Moreover, the aforementioned
zkSNARKs not only utilize the Fiat--Shamir transformation, but also a structured
reference string (SRS). According to our best knowledge security of the
transformation in case of SRS-based zero knowledge proofs has not yet been
shown. \michals{22.05}{What about some works by Tibor Jeager?}

\hl{Write about FS analysis for e.g. Bulletproofs}

Unfortunately, conjecturing security by calling Fiat--Shamir transformation is
no the only security weakness of zkSNARKs. And while security of the transform
may be interesting only for a narrow group of cryptography theory enthusiasts,
other are more practice-oriented. First of all, the most efficient zkSNARKs
require a trusted third party to generate the SRS and provide no security if the
SRS has been produced maliciously. This issue has been tackled by, e.g.~\plonk{}
and \sonic{} by using an updatable SRS framework. This framework, instead of
using a single SRS-generating entity, allows SRS to be generated by a set of
...
and zkSNARKs that are universal and
...
inefficient extraction gives a tight simulation soundness.
...
that $\plonk$ and $\sonic$ fulfil requirements of our framework and in fact are
...
definition). 
%
With the probabilities in \cref{def:simext} holding regardless the extractor
is unbounded or not, we obtain the following equality
$ \ssndProb = \accProb - \frkProb$.
\end{remark}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    - \epsur - \epsss,\right).
  \end{align*}
  Furthermore, the extractor $\ext$ from \cref{rem:simext_to_simsnd} is unbounded, we can
  ...
  simulation-extractability adversary $\adv$. hence $n = 1$. Thus,
    \begin{align*}
      \ssndProb & \leq \accProb - \left(\accProb - \epsur -
                  (\accProb - \epsur) \cdot \left( 1 -
                  \left(\frac{2^\secpar - 1}{2^\secpar}\right)\right)
                  - \epsur - \epsss\right) \\
                & = \accProb - \left(\accProb - \epsur -
                  (\accProb - \epsur) \cdot \frac{1}{2^\secpar}
                  - \epsur - \epsss\right) \\
                & = 2\epsur + \epsss
                  (\accProb - \epsur) \cdot \frac{1}{2^\secpar}.
    \end{align*}
