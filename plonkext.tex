...
with short (usually constant-length) proofs. This property makes zkSNARKs
...
for secure real-life deployment of zkSNARKs requires answering to the number of
...
proposed $\textsf{Marlin}$ with comparable efficiency to $\plonk$.
%
...
be UC-secure if it does not have this property. In the context of zkSNARKs it is
...
\item[Evaluation binding] a $\ppt$ adversary $\adv$ which outputs a commitment
  $\vec{c}$ and evaluation points $\vec{z}$ has at most negligible chances open
...
\item[Soundness] We say that $\proofsystem$ for $\RELGEN$ is \emph{sound} if no
  ...
  proof for a false statement, i.e~for $\inp \not\in\LANG$. More precisely, for
  all $\REL \in \RELGEN(\secparam)$
	\[
		\condprob{\ip{\adv(\REL, \crs, \inp)}{\verifier(\REL, \crs, \inp)} = 1}{\crs
      \gets \kgen(\REL), \inp \gets \adv(\REL, \crs); \inp \not\in \LANG_\REL}
    \leq \negl\,;
	\]
...
allows the simulator to simulate. In this paper we distinguish simulators that
requires a trapdoor to be able to provide an indistinguishable proof and those
