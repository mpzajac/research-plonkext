...
\begin{equation}
  ...
  \begin{aligned}
    ...
    \p{z_A} (\beta_1) \p{z_B} (\beta_1) - \p{z_C} (\beta_1) & = \p{h_0} (\beta_1) \ZERO_\HHH (\beta_1)
  \end{aligned}
\end{equation}
...
$\simulator$.
\end{proof}
% Note that the challenges are picked randomly, hence at this aspect the simulated
% proof is not distinguishable from a real proof.
...
\begin{corollary}
  $\marlinprotfs$ is forking simulation-extractable.
\end{corollary}
