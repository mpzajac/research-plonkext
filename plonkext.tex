...
\begin{theorem}[Simulation-extractable multi-round protocols]
	Let $\ps$ be an interactive $2 \mu$-round protocol, that has $\ur{k}$ property with security $\epsur$. Assume that $\ps$ is $(1, \ldots, 1, n_j, 1, \ldots, 1)$-special sound, where $j > k$.
...
\begin{proof}		
	...
	\begin{itemize}
		...
		$\zkproof_{\simulator}\range{1}{3}$ such that $\zkproof_{\advse}\range{1}{3} = \zkproof_{\simulator}\range{1}{3}$ and a random oracle query $\zkproof_{\simulator}[3].\ch$ on $\zkproof_{\simulator}\range{1}{3}$.
		...
		\begin{align*}
			...
			\zkproof_2 = (\zkproof_{\simulator}\range{1}{3}, \zkproof_{\simulator}[3].\ch, \zkproof_{\advse}\range{4}{5})
		\end{align*}
		\end{itemize}  
		If $\zkproof_1 = \zkproof_2$, then $\advse$ fails to break simulation extractability, as $\zkproof_2 \in Q$.
		On the other hand, if the proofs are not equal, then $\rdvur$ breaks $\ur{3}$-ness of $\ps$, what may happen with some negligible probability $\epsur$ only, hence
		\[
	...
	\]
	Denote by $\accProb$ the probability that $\advse$ outputs a proof such that is acceptable and does not break $\ur{3}$-ness of $\ps$. 
