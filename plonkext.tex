...
\begin{theorem}[Simulation extractability]
	...
	\[
		1 - \frkProb (1 - \eps_{\ur{3}})(1 - \epsbatch)(1 - \eps_\dlog).
	\]
	\michals{16.09}{Should include the simulation error above?}
	% The simulation error for $\plonkprotfs$ is \hl{\ldots}
	% where $q_\ro$ is the total number of queries required by the adversary $\advse$.
\end{theorem}
\begin{proof}
...
A crucial observation is that the adversary $\advse$ may have learned $\zkproof_{\advse}\range{1}{3}$ by querying the simulator on $\inp_{\advse}$ or might have computed it itself. We denote the first event by $\event{E}$ and the second by $\nevent{E}$. 
%
When $\nevent{E}$ happens the reduction utilises the generalised forking lemma and rewinds the adversary and programs the random multiple times to get $(1, 1, 1, \numberofconstrains + 3, 1)$-tree of acceptable transcripts $\tree$. 
We denote the probability that the reduction is succesfull in generating the tree $\tree$ by $\event{F}$.
If $\event{F}$ does not happen, what is denoted by $\nevent{F}$ then we grant $\advse$ victory.
%
Additionally, we divide event $\nevent{E}$ into two disjunctive subevents: $\nevent{E}_0$ and $\nevent{E}_1$. 
...
As all these events are mutually exclusive and exhaustive, we have
\begin{multline*}
	\eps = \prob{\advse \text{ wins}} = \prob{\advse \text{ wins}, \event{E}} + \prob{\advse \text{ wins}, \nevent{E}_0, \event{F}} + \prob{\adv \text{ wins}, \nevent{E}_1, \event{F}} + \\
	\prob{\adv \text{ wins}, \nevent{E}, \nevent{F}}\,.
\end{multline*}
