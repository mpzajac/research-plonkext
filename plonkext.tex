...
\begin{proof}		
	...
	$(\inp_\advse, \zkproof_\simulator\range{1}{k}) = (\inp_\advse,
	\zkproof_\advse\range{1}{k})$. That is, the adversary in its final proof reuses a part of a simulated proof it saw before and the proof is acceptable.
...
Let $\rdvur$ run $\advse$ internally as a black-box:
	\begin{itemize}
		...
		$\zkproof_{\simulator}\range{0}{k}$ such that
		$\zkproof_{\advse}\range{0}{k} = \zkproof_{\simulator}\range{0}{k}$ and a random oracle query $\zkproof_{\simulator}[k].\ch$ on
		$\zkproof_{\simulator}\range{0}{k}$.
		\item $\rdvur$ returns two proofs for $\inp_\advse$:
		\begin{align*}
			\zkproof_1 = (\zkproof_{\simulator}\range{1}{k},
			\zkproof_{\simulator}[k].\ch, \zkproof_{\simulator}\range{k +
			1}{\mu})\\
			\zkproof_2 = (\zkproof_{\simulator}\range{1}{k},
			\zkproof_{\simulator}[k].\ch, \zkproof_{\advse}\range{k + 1}{\mu})
		\end{align*}
		\end{itemize}  
	...
	\emph{fresh}.}. Otherwise, the game would be already interrupted by the
	error  
