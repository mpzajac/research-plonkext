...
\begin{theorem}[Forking simulation-extractable multi-message protocols]
  ...
  property with security $\epsur$, is $(\epss, n, k)$-forking sound .
  % for
...
$\plonkprot$ is $(\epsss, (1, 1, 3 \noofc + 1, 1))$-forking sound
for $\epsss = \epsid + \epsdlog$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
\label{lem:plonkprot_ss}
$\plonkprot$ is $(\epsss, (1, 1, 3\noofc + 1, 1))$-computational forking sound
for
 \[
...
and forking sound. We now 
make use of \cref{thm:se} and show that
$\plonkprot_\fs$ is forking simulation-extractable as defined in \cref{def:simext}.
...
forking simulation-extractable with extraction error $\eta = \epsur$. The extraction
...
\begin{lemma}
  ...
  $\adv$ breaks forking soundness of $\sonicprot$ is upper-bounded
  as
	\[
