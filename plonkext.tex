...
$y = (\REL, \crs, \advse, r, \inp_{\advse}, \zkproof_{\advse}, Q)$ where we set
...
\[
	\Pr
		\left[
			\begin{aligned}
				& \verify(\crs, c, z, s, o) = 1, \\
				& \verify(\crs, c, z, \tilde{s}, \tilde{o}) = 1
			\end{aligned}
		\,\left|\,
		\vphantom{\begin{aligned}
			& \verify(\crs, c, z, s, o),\\
...
are $\multconstr$-elements long and correspond to the $Q$ linear constrains of
...
\begin{description}
	...
	\[
		\beta = \ro(\trans[0..1], 0)\,, \qquad \gamma = \ro(\trans[0..1], 1)\,.
	\]
	...
	\item[Round 3]
	Get the challenge $\alpha = \ro(\trans[0..2])$, compute the quotient polynomial 
	\begin{align*}
	& \p{t}(X)  = \\
	...
	\item[Round 4]
	Get the challenge $\chz \in \FF_p$, $\chz = \ro(\trans[0..3])$.
	...
	\item[Round 5]
	Compute the opening challenge $v \in \FF_p$, $v = \ro(\trans[0..4])$.
...
\begin{proof}		
  ...
  $\genforking^{N}_\zdv(y,h_1, \ldots, h_q)$ for $y = (\REL, \crs, \advse, r,
...
%\subsection{Building tree of acceptable transcripts and extracting the witness}
We denote by $\validtree(\tree)$ an algorithm that takes an $(n_1, \ldots,
n_\mu)$ tree of transcripts and reutrns $1$ if it is valid and $0$ otherwise.
Let $\findwitness(\tree)$ an algorithm that given a valid $(n_1,
\ldots, n_\mu)$-tree of accepting transcripts for statements $\inp$ extracts a
witness $\wit$ such that $\REL(\inp, \wit) = 1$.

\begin{figure}
  \centering
  \fbox{
    \procedure{$\ext_\adv(\REL, \crs, \inp; r)$}{
      (\inp, \zkproof, \tree) \gets \treefind(1)\\
      %\pcif \tree \text{ is not valid $(n_1, \ldots, n_\mu)$-tree of accepting
      % transcripts} \pcthen \\
      \pcif \validtree(\tree) = 0 \pcthen \\
      \pcind \pcreturn (\zkproof, \bot)\\
      \pcelse \wit \gets \findwitness(\REL, \crs, \inp, \tree) \\
      \pcreturn \wit
    }
  }
  \vspace*{1cm}
  \fbox{
    \procedure[linenumbering]{$\treefind(i, (h_1, \ldots, h_q))$}{
      \pcif i = \mu + 1 \pcthen\\
      \pcind (\inp, \zkproof) \gets \adv^{\ro}(\REL, \crs)\\
      \pcind \pcif \verifier(\REL, \crs, \inp, \zkproof) = 1 \pcthen\\
  ...
  \caption{Extractor and tree finding algorithm. \michals{18.02}{The problem
      with current write up --- The adversary seeing $h'_j$ may decide to stop
      current trial to make a proof and start making the proof over. In that
      case, he would continue using ro queries from sequence $(h'_j, \ldots,
      h'_q)$. Hence, he would produce a proof that may be totally different from
    the previous one, i.e. even the instance may differ, thus the proof cannoit
    be joined to the tree. (Even when we force the adversary to use the same
    instance, he can change all the following messages. In that case the proof
    probably cannot be joined to the tree as well.)}}
  \label{fig:extractor_and_tree_finding_algorithm}
\end{figure}

\paragraph{Bootle et al.'s tree finding algorithm and Fiat--Shamir
  transformation}
We note that although the result of building a tree of transcripts presented in
\cite{EC:BCCGP16} is dubbed ``forking lemma'' it differs from forking lemmas
known from e.g.~\cite{JC:PoiSte00,CCS:BelNev06}.
%
First of all forking lemmas in these papers analyse probability of building a
tree of acceptable transcripts for Fiat--Shamir based non-interactive proof
systems, while the protocol presented by Bootle et al.~works for interactive
proof systems. \xout{(It can however be adjusted to work with NIZKs as well as
  presented on \mbox{\cref{fig:extractor_and_tree_finding_algorithm}}.)}

It is also not obvious how the result of Bootle et al.~can be used to show
security of non-interactive protocols. At
...
non-interactive proof system. Unfortunately, there is a crucial problem with
such approach. Namely, the adversary is not fixed to show a proof for any
concrete instance and can change the proven instance on the fly, e.g.~after
seeing a number of random oracle responses. More precisely, at
\cref{fig:extractor_and_tree_finding_algorithm}, line \ref{line:branching}, there is a first
branch of the tree already built and the algorithm tries to find another one by
presenting the adversary with a new set of challenges. The problem is that when
$\adv$ sees $h'_j$ then it may decide to start the proof over \emph{with a new
  instance}. In that case the resulting proof cannot be joined to the existing
one (to make a tree) simply because it is for a different instance. Obviously,
the adversary cannot change the instances indefinitely, so there is a chance
that the tree will be finally build; however, it is far fgrom obvious and
requires additional analysis.

This problem could be solved as follows \michals{18.2}{Describe how guessing the
  first adversary's ro query can solve the problem.}
\michals{18.2}{Maybe we could guess only the first ro query?! That would be splendid.}
...
\end{document}
  ...
  \michals{15.09}{As MK noted, this proof holds only for simulation soundness.
    For extractability we use special soundness (cf the next section)}
