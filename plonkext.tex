...
\cite{C:GroMal17}, see \cite{EPRINT:AtaBag19}.  \markulf{03.10.2020}{Cite weakSE e-print
  paper?}
...
\markulf{03.10.2020}{I rephrased this a bit, as it seemed to hurt us as well.
...
\markulf{03.11.2020}{This requires probability ensembles and $\secpar$ dependent
...
\markulf{03.10.2020}{Make sure we mean the right thing. In the following
...
\begin{lemma}[General forking lemma II]
	...
	\]
	Let $\genforking_\adv$ \markulf{03.11.2020}{Add a $m$ parameter as superscript
  to $\genforking$?} denote the algorithm described in \cref{fig:genforking_lemma} then the probability $\frkProb := \condprob{b = 1}{y \gets \ig;\ (b, \vec{s}) \gets \genforking_\adv(y)}$ is at least 
	\[
...
real-proof polynomial $\p{t}(X)$ is random since the polynomials
...
\markulf{03.11.2020}{cut: follow \cite{INDOCRYPT:FKMV12}. Don't give those guys too much credit, instead: } make use
of \cref{thm:se} and show that $\plonkprot_\fs$ is simulation-extractable as defined in \cref{def:simext}.
