...
\item Now we need to show that proof output by $\adv$ for $\srs_n$ can be
  ...
  except the query that gives challenge $\chz'$\michals{14.09}{Check!}. Unique
  response property assures that the output proof will not share the first 3
  rounds with some simulated proof. Hence we do not need to worry about mismatch
  between the programmed random oracle and the real random oracle. (That is, the
  fact that $\bdv$ programmed RO to have challenge $\chz'$ instead of $\chz$
  will not be noticed.)  Given proof
