...
\begin{definition}[Forking special soundness]
  % Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
...
%
  ...
  $\inp$. We say that $\ps$ is $(\epsss, (n_1, \ldots, n_\mu))$-forking special
  sound if there is an extractor $\extt$ that given $\tree$ extracts $\wit$
  such that $\REL(\inp, \wit) = 1$ with probability at least $1 - \epsss.$
\end{definition}

\begin{definition}[$k$-round forking special soundness]
  % Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  % $(2 \mu + 1)$-message proof system for a relation $\REL$. Let $\tree$ be an
  % $(n_1, \ldots, n_\mu)$-tree of acceptable transcripts output by a $\ppt$ tree
  % building algorithm $\tdv$ which plays the role of the verifier $\ps.\verifier$
...
%
  ...
  % $\REL(\inp, \wit) = 1$ with probability at least $1 - \epsss$.
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$. Let $\tree$ be a PPT
  algorithm that rewinds $\adv^{H,Sim}$ to produce a $(1, n_k, 1)$-tree of
...
\begin{definition}
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$, let $\adv$, called
  adversary, and $\tdv$, called tree creator, be $\ppt$ algorithms. $\tdv$ has
  oracle access to $\adv$ and provides it with (oracle) access to random oracle
  $\ro$ and $\ps$ simulator $\simulator$ -- more precisely $\tdv$ has an
  internal procedure $\bdv$ that provided $\srs$ and random oracle queries'
  responses $h_1, \ldots, h_1$ gives $\adv$ access to the random oracle and
  simulates proof for it. In the end, $\bdv$ returns the proof $\adv$
  returns. Eventually, $\tdv$ returns a $(n_1, \ldots, n_\mu)$ tree of
  acceptable transcripts $\tree$.
  ...
  \begin{figure}
    \centering
      ...
      {h_1^{1}, \ldots,
      h_{q}^1 \sample H\\
      (i, \inp, \zkproof_1) \gets \bdv(\adv, \srs, h_1^{1}, \ldots, h_{q}^{1})\\
        % i_1 \gets i\\
        \pcif \verifier(\srs, \inp, \zkproof_i) = 0\ \pcreturn (\bot)\\
        \pcfor j \in \range{2}{m}\\
        \pcind h_{1}^{j}, \ldots, h_{i - 1}^{j} \gets h_{1}^{j - 1}, \ldots,
        h_{i - 1}^{j - 1}\\
        \pcind h_{i}^{j}, \ldots, h_{q}^{j} \sample H\\
        \pcind (i_j, \zkproof_j) \gets \bdv(\adv, \srs, h_1^{j}, \ldots, h_{i -
          1}^{j}, h_{i}^{j},
        \ldots, h_{q}^{j})\\
        \pcind \pcif \verifier(\srs, \inp, \zkproof_i) = 0\ \pcreturn (\bot)\\
        %\pcind \pcif i_j = 0 \lor i_j \neq i\ \pcreturn (0, \bot)\\
        % \pcif \exists (j, j') \in \range{1}{m}^2, j \neq j' : (h_{i}^{j} =
        % h_{i}^{j'})\
        % 
  ...
  We say that $\ps$ is $(n_1, \ldots, n_k, \ldots, n_\mu)$-forking special sound if
  for any adversary probability
  \[
    \Pr\left[
      \begin{aligned}
...
to present the main result of this paper---special simulation soundness and
special soundness extractability of multi-round protocols.
