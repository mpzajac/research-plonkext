...
\begin{lemma}
	...
	If a polynomial commitment scheme $\PCOM$ is evaluation binding, then $\plonkprot$ is $\ur{3}$.\footnote{An attentive reader may note that $\plonkprot$ is $\ur{2}$. However, in the case presented here, less is required.}
\end{lemma}
\begin{proof}
	...
	Since \cref{lem:kzg_unique_op}, one of the openings has to be incorrect and should be rejected by the polynomial commitment verifier. 
...
\begin{proof}
	...
	\michals{28.08}{Need to add this to the unique resp prop part}
  ...
  \markulf{27.08}{I struggle to understand how this would work when simulated RO
	...
	% That is, if this happens then $\plonkprot$ is not $\ur{3}$---
	...
	\task{28.08}{How is that related to freshness of queries?}
