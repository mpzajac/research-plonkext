...
\begin{lemma}[General forking lemma II]
	...
	1}{y \gets \ig;\ (b, \vec{s}) \gets \genforking_{\zdv}^{m}(y)}$ is at least 
	\[
	...
	\begin{figure}[t]
		\centering
		\fbox{
		\procedure{$\genforking_{\zdv}^{m} (y)$}
		{
			r \sample \RND{\zdv}\\
			h_1^{1}, \ldots, h_{q}^{1} \sample H\\
			(i_1, s_1) \gets \zdv(y, h_1^{1}, \ldots, h_{q}^{1}; r)\\
...
Let $\rdvur$ run $\advse$ internally as a black-box:
  ...
  Assuming that $\advse$ does not break the unique response property is
	...
	$\advse$'s code, its randomness $r$, the output instance and proof
	$\inp_\advse, \zkproof_\advse$ as well as the list $Q$ of simulated proofs
	$\advse$ required and list of random oracle queries and responses $Q_\ro$. 
  ...
  \advse, r, \inp_\advse, \zkproof_\advse, Q, Q_\ro$ however
	...
	queries from list $Q_\ro$. In fact, that change does not change any
	probability of success of $\genforking$ as elements in $Q_\ro$ are random
	oracle responses, thus are random as required. 
