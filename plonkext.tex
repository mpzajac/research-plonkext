...
\begin{proof}[Proof idea]
	...
	At first $\reduction$ obtains $\crs$ from $\plonkprot.\kgen$ and passes it to $\advse$. Assume that 
	\begin{itemize}
		\item the adversary makes $q_\simulator$ queries to the simulator obtaining $q_\simulator$ simulated proofs;
		\item the adversary copies first $r$ round of some simulated proof in its final simulation-extractability breaking proof. (Note that from the unique response property $r < 3$.)
	\end{itemize}
