...
\begin{description}
	...
	\begin{multline*}
		\left\{\ip{\prover(\REL, \inp, \wit)}{\verifier(\REL, \inp)} \,\left|\, (\inp, \wit) \gets \adv(\REL)\vphantom{\simulator^\adv}\right.\right\} \approx_\secpar 
		\left\{\simulator^{\verifier}(\REL, \inp)\,\left|\, (\inp, \wit) \gets \adv(\REL)\vphantom{\simulator^\adv}\right.\right\}\,.
	\end{multline*}
\end{description}
...
Faust et al.~\cite{INDOCRYPT:FKMV12} show that every sigma protocol that is specially sound and has a unique response property is simulation-extractable. 
%
