...
\begin{description}
	...
	\begin{align}
		...
		\,.
	\end{align}
	The simulator computes $\gone{\p{W}_{\chz}(\chi), \p{W}_{\chz \omega}(\chi)}$ such that the verification equation holds and outputs the proof
	$\zkproof = \left(
		\begin{aligned}
		...
		\end{aligned}
	\right)$ 
	More precisely the verifier computes 
	$\gone{D} = v \gone{r} + u \gone {z}$ that is
	\[
	\gone{D} = 
	\]
% 	a ̄bv · [qM]1 + a ̄v · [qL]1 + bv · [qR]1 + c ̄v · [qO]1 + v · [qC]1 􏰀 ̄2􏰁
% [D]1 := + (a ̄+βz+γ)(b+βk1z+γ)(c ̄+βk2z+γ)αv+L1(z)α v+u ·[z]1  ̄
% −(a ̄+β ̄s +γ)(b+β ̄s +γ)αvβz ̄ [s ] σ1 σ2 ω σ31
\end{description}
