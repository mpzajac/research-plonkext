...
\begin{description}
	...
	\begin{multline*}
	  \smallset{\ip{\prover(\REL, \inp, \wit)}{\adv(\REL, \inp, \wit)} \left| (\inp, \wit) \gets \adv(\REL)\right.} \approx_\secpar \\
		\smallset{\ip{\simulator^{\adv}(\REL, \inp)}{\adv(\REL, \inp, \wit)} \left| (\inp, \wit) \gets \adv(\REL)\right.}\,.
	\end{multline*}
	%
...
Many proof systems additionally compounds of a setup algorithm $\kcrs$ that on input $\REL$ outputs a common reference string (CRS) $\crs$. For the sake of succinctness we denote by $\trans$ a transcript of a conversation between the prover and the verifier extended with verifier's input $(\REL, \inp)$ (or $(\REL, \crs, \inp)$ in case of a proof system with a setup).
...
\begin{definition}[Witness-extended emulation \cite{EC:GroIsh08}]
	...
	We say a public coin proof system $\proofsystem = (\kcrs, \prover, \verifier, \simulator)$ has \emph{witness-extended emulation} if for all $\REL \in \IM(\RELGEN)$, and deterministic adversary $\adv_1$ there exists an expected polynomial time extractor $\ext$ such that for all NUPPT adversaries $\adv_0$ holds
		...
		\begin{aligned}
				& \crs \gets \kcrs(\REL), (\inp, \aux_{\adv_1}) \gets \adv_0, \\
		...
		\right]
		 ...
		 \Pr\left[
			...
			\begin{aligned}
				& \crs \gets \kcrs(\REL), (\inp, \aux_{\adv_1}) \gets \adv_0, \\ 
				& (\trans, \wit )\gets \ext^{\ip{\adv_1(\REL, \crs, \inp, \aux_\prover)}{\verifier(\REL, \crs, \inp)}}
			\end{aligned}
