...
First of all, we prove that $\plonk$ and $\sonic$ are simulation-extractable. Although we do
not change these protocols at all, we had to solve a number of problems
in order to show that. The idea of the proof is presented below. 

Note that these protocols, as originally presented in
\cite{EPRINT:GabWilCio19} and \cite{CCS:MBMK19}, are
non-interactive protocols built using the Fiat--Shamir transform on an
interactive proof of knowledge. In the following, we denote the underlying
protocols by $\plonkprot$ (for $\plonk$) and $\sonicprot$ (for $\sonic$) and
the resulting, non-interactive ones by $\plonkprotfs$ and $\sonicprotfs$,
respectively. 

\subsubsection{Special soundness of $\plonkprot$ and $\sonicprot$.} 
