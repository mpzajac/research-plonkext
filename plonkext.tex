...
\begin{abstract}
	...
	\michals{14.05}{Since Sonic is witness-extended emulation then it should be simulation extractable as well. Task: check Marlin}
\end{abstract}
...
Many proof systems additionally compounds of a setup algorithm $\kcrs$ that on input $\REL$ outputs a common reference string (CRS) $\crs$. 
...
Interactive proofs run in a multiprotcol environment are often required to fulfil a stronger notion of soundness called \emph{witness-extended emulation}~\cite{JC:Lindell03}. We follow \cite{EC:GroIsh08} in their formulation of this property since it considers proof systems that utilises a CRS. 

\begin{definition}[Witness-extended emulation \cite{EC:GroIsh08}]
	\label{def:witextem}
	We say a public coin proof system $\proofsystem = (\kcrs, \prover, \verifier, \simulator)$ has \emph{witness-extended emulation} if for all $\REL \in \IM(\RELGEN)$, $\inp, \aux_\adv$ and deterministic adversary $\adv$ there exists an expected polynomial time extractor $\ext$ such that for all NUPPT distinguishers $\ddv$ holds
		\begin{multline*}
		\Pr\left[\ddv (\trans, \inp, \aux_{\adv}) = 1 \,\left|\,
		\begin{aligned}
				& \crs \gets \kcrs(\REL), (\inp, \aux_{\prover^{*}}) \gets \adv, \\
				& \trans \gets \ip{\prover^{*}(\REL, \crs, \inp, \aux_\prover)}{\verifier(\REL, \crs, \inp)}
			\end{aligned}
		\right.
		\right]
		 \approx_\epsilon \\
		 \Pr\left[
			...
			\begin{aligned}
				& \crs \gets \kcrs(\REL), (\inp, \aux_{\prover^{*}}) \gets \adv, \\ 
				& (\trans, \wit )\gets \ext^{\ip{\prover^{*}(\REL, \crs, \inp, \aux_\prover)}{\verifier(\REL, \crs, \inp)}}
			\end{aligned}
...
\end{definition} 
Although in the definition above the adversary $\adv$ is deterministic it obtains an auxiliary input $\aux_\adv$ that may be used by $\adv$ as a source of randomness. \cite{EC:GroIsh08} notes that since the protocol is public-coin, verifier's randomness is part of the transcript. Thus combining $\REL, \crs, \inp, \aux_\adv$ and an accepting transcript $\trans$ gives a view of both the prover and the verifier and thus gives a witness.

\comment{\begin{definition}[Witness-extended emulation \cite{EC:GroIsh08}]
	We say a public coin proof system $\proofsystem = (\kcrs, \prover, \verifier, \simulator)$ has \emph{witness-extended emulation} if for all $\REL \in \IM(\RELGEN)$, deterministic polynomial time $\prover^{*}$ there exists an expected polynomial time extractor $\ext$ such that for all NUPPT adversaries $\adv$ holds
		\begin{multline*}
		\Pr\left[\adv (\trans) = 1 \,\left|\,
		\begin{aligned}
				& \crs \gets \kcrs(\REL), (\inp, \aux_{\prover^{*}}) \gets \adv, \\
				& \trans \gets \ip{\prover^{*}(\REL, \crs, \inp, \aux_\prover)}{\verifier(\REL, \crs, \inp)}
			\end{aligned}
...
\end{figure}
\end{lemma}
\section{Simulation extractability from special soundness}
...
\end{theorem}
\michals{14.05}{Need to make this theorem more general as currently it works only for 3-move $\Sigma$-protocols!}
