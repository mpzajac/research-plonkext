...
$\plonk{}$ is simulation-sound.  \task{15.05}{Check Sonic and Bulletproofs}
\end{abstract}
...
\begin{figure}
	\begin{pcvstack}[center,boxed]
		\begin{pchstack}
			\procedure{$\kgen(\pp, \secparam)$}
			{
				\chi \sample \FF^2_p \\
				\pcreturn \gone{1, \ldots, \chi^{\numberofconstrains + 2}}, \gtwo{\chi}\\
			...
			{ 
				\pcreturn \gone{\vec{c}} = \gone{\vec{\p{f}}(\chi)}
			}
		\end{pchstack}
		% \pcvspace
		\begin{pchstack}
			...
			{
			\pcfor i \in \range{1}{\abs{\vec{z}}} \pcdo\\
			\pcind \p{o}_i(X) \gets \sum_{j = 1}^{t_i} \gamma_i^{j - 1} \frac{\p{f}_{i,j}(X) - \p{f}_{i, j}(z_i)}{X - z_i}\\
				\pcreturn \vec{o} = \gone{\vec{\p{o}}(\chi)}
			}
			...
			{
				\vec{r} \gets \FF_p^{\abs{\vec{z}}}\\
				\pcfor i \in \range{1}{\abs{\vec{z}}} \pcdo \\
				\pcind \pcif 
					\sum_{i = 1}^{\abs{\vec{z}}} r_i \cdot \gone{\sum_{j = 1}^{t_j} \gamma_i^{j - 1} c_{i, j} - \sum{j = 1}^{t_j} s_{i, j}} \bullet \gtwo{1} + \\
...
\begin{figure}
	\begin{pcvstack}[center,boxed]
		\begin{pchstack}
			\procedure{$\kgen(\pp, \secparam)$}
			{
				\alpha, \chi \sample \FF^2_p \\
				\pcreturn \\
				\hphantom{\p{o}(X) \gets \frac{\p{f}(X) - \p{f}(z)}{X - z}}
			}
			...
			{
				\p{c}(X) \gets \alpha \cdot X^{\dconst - \maxconst} \p{f}(X) \\
				\pcreturn \gone{c} = \gone{\p{c}(\chi)}
			}
		\end{pchstack}
		% \pcvspace
		\begin{pchstack}
			...
			{
				\p{o}(X) \gets \frac{\p{f}(X) - \p{f}(z)}{X - z}\\
				\pcreturn \gone{\p{o}(\chi)}
			}
			...
			{
				\pcif \gone{\p{o}(\chi)} \bullet \gtwo{\alpha \chi} + \gone{s - z \p{o}(\chi)} \bullet \gtwo{\alpha} = \\
				\pcind \gone{c} \bullet \gtwo{\chi^{- \dconst + \maxconst}} \pcthen  \pcreturn 1\\
				\pcelse \pcreturn 0.
			}
		\end{pchstack}
	\end{pcvstack}
...
\subsubsection{Unique opening property of $\PCOMs$}
\begin{lemma}
...
\begin{lemma}
		...
		\[
				\epsss \leq \epss + \epsdlog\,,
		\]
		where $\epss$ is a soundness error of the protocol, and $\epsdlog$ is a probability that a $\ppt$ algorithm can break the
		$(\dconst, \dconst)$-ldlog assumption.
\end{lemma}
\begin{proof}
		The proof goes similarly to the proof of \cref{lem:plonkprot_ss}.

		Let $\adv$ be an adversary that produces a $(1, 1, \multconstr + \linconstr + 1)$-tree of acceptable
		...
		i.e.~$\vereq(X) = 0$. In that case we show an extractor $\extss$ that
		...
		show a reduction $\rdvdlog$ that uses $\adv$ to break the $(\dconst,
		\dconst)$-dlog
		assumption with at least the same probability.

		\ncase{When $\event{E}$ happens}
		Since the protocol $\sonicprot$ is sound, for an acceptable proof
		$\zkproof$ for a statement $\inp$ there exists a witness $\wit$ such that
		$\REL(\inp, \wit)$ holds except some negligible probability $\epss$. 
		\michals{21.X}{Update the ss proof for Plonk with $\epss$} Note that
		...
		Since the adversary is algebraic, all group elements included in the tree of
  	transcripts are extended by their representation as a combination of the input
  	$\GRP_1$ or $\GRP_2$-elements i.e.~$\gone{1, \chi, \ldots, \chi^{\numberofconstrains +
    2}}$. Hence all coefficients of the verification equation polynomial
  	$\vereq(X)$ are known and $\rdvdlog$ can find its zero points. Since
  	$\vereq(\chi) = 0$, the targeted discrete log value $\chi$ is among them.
		\qed		
...
\begin{theorem}[Simulation extractability of $\sonicprotfs$]
	...
	extraction error \hl{...}. The extraction probability $\extProb$ is at least
	\hl{...}.
\end{theorem}
...
% \paragraph{\fbox{\normalfont{$\plonk$ prover $\prover(\REL, \crs, \inp, \wit = (\wit_i)_{i \in \range{1}{3 \cdot \numberofconstrains}})$}}}
