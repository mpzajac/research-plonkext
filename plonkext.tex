...
\begin{definition}[Simulation soundness]
  ...
  We note that \cref{def:simsnd} can be expressed as a special case of the
  simulation-extractability where a witness for a statement proven in $\ps$ from
  \cref{def:simext} can be extracted by an \emph{inefficient} extractor from a
  single acceptable transcript (i.e.~$(1, \ldots, 1)$-tree of accepting
  transcripts). Really, the extractor given a statement and transcript output by the adversary
  can simply check all possible witnesses to determine whether the statement is
