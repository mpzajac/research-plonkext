...
\begin{definition}[$(\eps, k,n)$-forking soundness]
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  ...
  \end{figure}
%
  We say that $\ps$ is $(\eps, k,n)$-forking sound if
...
event is bounded by the $(\noofc + 2, 1)$-dlog security $\epsdlog$,
...
\begin{proof}
  ...
  idealised verification equation, i.e.~for that proof $\vereq_{\inp, zkproof}(X) = 0$.
  Alternatively, event $\nevent{E}_1$ covers a case when for $\zkproof_\advss$
  it holds that $\vereq_{\inp, zkproof}(\chi) = 0$, but $\vereq_{\inp, zkproof}(X) \neq 0$, where $\chi$ is
  ...
  equation $\vereq_{\inp, zkproof}(\chi)$ holds. That is, the verifier plugs-in elements of
  $\zkproof$ into $\vereq_{\inp, zkproof}(\chi)$ and checks whether it equals $0$. That is what
  is called a \emph{real check} in \cite{EPRINT:GabWilCio19}.  On the other hand
  there is an \emph{idealised check}, which verifies whether $\vereq_{\inp, zkproof}(X) = 0$
  ...
  \item $\rdvur$ returns two proofs for $\inp_\advss$:
    ...
    \end{align*}
  \end{itemize}
  If $\zkproof_1 = \zkproof_2$, then $\advss$ fails to break forking simulation
  extractability, as $\zkproof_2 \in Q$.  On the other hand, if the proofs are
  ...
  Since $\vereq_{\inp, zkproof}(X) \neq 0$, but $\vereq_{\inp, zkproof}(\chi) = 0$ and $\rdvdlog$ knows
  ...
  $\vereq_{\inp, zkproof}(X)$. This way, it knows all coefficients of $\vereq_{\inp, zkproof}(X)$. Thus it can
