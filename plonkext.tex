...
1}, b_{\mu - 1}, a_\mu)$, where $a_i$ comes from $\prover$ and $b_i$
...
\begin{itemize}
  ...
  \item $\prover_\fs$ behaves as $\prover$ except after sending message
    ...
    a_{j - 1}, b_{j - 1}, a_j)$. (Importantly, $\zkproof[0..\mu] =
    (\inp, \zkproof)$).
...
sound. In this part we generalise the forking lemm, special
...
\begin{align}
  ...
  \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)} \label{eq:use_eq1}\\
	& \geq \frac{\expected{\accProb(y)}^m}{q^{q'(m - 1)}} -
	\expected{\accProb(y)} \cdot \left(1 - \frac{h!}{(h - mq')! \cdot
  h^{mq'}}\right) \label{eq:by_lemma_jensen}\\
	& = \frac{\accProb^m}{q^{q'(m - 1)}} -  \accProb \cdot \left(1 -
  \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)\label{eq:by_accProb}\,.
\end{align}
...
output by the prover, We say that $\proofsystem$ has \emph{unique responses
  on $I$} if for any $\ppt$ adversary $\adv$:
\[
	\prob{
		\begin{aligned}
		...
		& a_{i} = a'_{i} \text{ for some } i \in I
		\end{aligned}
		\ \left|\  
	\vphantom{\begin{aligned}
...
\]
\michals{31.12.20}{Do I need to assume that the first $i$ elements is the same?}
...
that are the same on some $i$-th message, where $i \in I$. 
...
\end{description}
	...
	\section{Generalised forking lemma for single branching point}
    \subsection{Generalised forking lemma.}
First of all, although dubbed ``general'', \cref{lem:forking_lemma} is not
general enough for our purpose as it is useful only for protocols where witness
can be extracted from just two transcripts. To be able to extract a witness
...
\begin{lemma}[General forking lemma II]
  ...
  \cref{fig:genforking_lemma} then the probability $\frkProb := \condprob{b =
    ...
    \pcif \exists (j, j') \in \range{1}{m}^2 : (h_{i}^{j} = h_{i}^{j'})\
	\pcreturn (0, \bot)\\
		\pcelse \pcreturn (1, \vec{s})
	}}
	\caption{Generalised forking algorithm $\genforking_{\zdv}^{m}$}
...
is run on the same random tape and with the same answers from the simulator and
random oracle. After $\advse$ finishes its acceptable proof $\zkproof_{\advse}$,
algorithm $\zdv$ outputs $(i, \zkproof_{\advse})$, where $i$ is the index of a random
oracle query required to get the challenge after $k$-th message from the prover,
where $k$ is the index of a message where the tree of
transcripts branches. 
%We also note that we differ here from the description of the
%$\genforking_\zdv$ algorithm provided
%e.g.~in~\cite{INDOCRYPT:FKMV12,CCS:BelNev06} where the algorithm also picks the first set of random oracle
%queries' responses $h^1_1, \ldots, h^1_q$. We however note that this difference
