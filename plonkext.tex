...
\paragraph{Special soundness of $\plonkprot$.}
As presented in \cite{EPRINT:GabWilCio19}, \plonk{} is a non-interactive protocol, which relies on an interactive proof of knowledge and the Fiat--Shamir transform. Thus, we denote the underlying protocol by $\plonkprot$ and the resulting, non-interactive one by $\plonkprot_\fs$. 
...
Secondly, the protocol has a trapdoor what allows an adversary who knows it to produce multiple proofs even without knowingg the witness.  
