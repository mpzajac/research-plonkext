...
\begin{definition}[Witness-extended emulation]
  ...
  expected polynomial time extractor $\ext$ \markulf{09.07.20}{Do we want
    expected PPT, how crucial is the non-uniformity?} such that for all NUPPT adversaries $\adv_0$ holds
		\begin{multline*}
		...
		\right]
		 ...
		 \Pr\left[
			...
			\right]\,,
\end{multline*}
\markulf{09.07.20}{fix left of $|$}where the extractor $\ext$ can rewind the conversation between the prover and the verifier to a particular round and run it again with a fresh randomness for the verifier.
\end{definition}
...
is specially sound and has a unique response property is simulation-extractable \markulf{}{typo?}. Let $\sigmaprot_\fs = (\prover_\fs, \verifier_\fs, \simulator_\fs)$ be a NIZK obtained by applying the Fiat--Shamir transform to $\sigmaprot$.
...
Consider $\prod_{i = 1}^{\mu} n_i$ accepting transfers with challenges as a tree
with depth $\mu$ and $\prod_{i = 1}^{\mu} n_i$ leaves.	Each node of depth $i <
\mu$ has exactly $n_i$ children, each labelled with a distinct value for the
$i$-th challenge $h_i$. We refer to that as a $(n_1, \ldots, n_\mu)$-tree of
accepting transcripts. \markulf{09.07.20}{Generalize special soundness in
  similar fashion}
...
\markulf{}{interesting} Note that to answer this question we shall quantify over all possible simulators. That would not be a problem though in the setting we consider.

\begin{definition}[$k$-out-of-$n$ simulatable proof system]
	Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ for $\RELGEN$ be
  a $n$-round \markulf{09.07.20}{2n-1 round} interactive proof. Let $(\inp, a_0, b_0, \ldots, b_{n - 2}, a_{n - 1})$ be a proof for a statement $\inp$ where $a_i$-s, $i \in \range{0}{n - 1}$ are messages send by the prover and $b_i$-s, $i \in \range{0}{n - 2}$ by the verifier.
