...
\begin{figure}
  ...
  \fbox{
    \procedure{$\treefind(i)$}{
      \pcif i = \mu + 1 \pcthen\\
      \pcind (\inp, \zkproof) \gets \adv(\REL, \crs)\\
      ...
      (\inp, \zkproof, \tree) \gets \treefind(i + 1)\\
      \pcif \tree = \bot \pcthen \pcreturn (\inp, \zkproof, \bot)\\
      \counter \gets 1\\
      \pcwhile \counter < n_i\\
      \pcind \text{rewind $\adv$ just before it gets it computes the $i$-th challenge}\\
      \pcind(\inp, \zkproof', \tree') \gets \pcind \treefind(i + 1)\\
      \pcind \pcif \tree' \neq \bot \pcthen \\
  ...
  \caption{Extractor and tree finding algorithm}
...
proof systems. (It can however be adjusted to work with NIZKs as well as
presented on \cref{fig:extractor_and_tree_finding_algorithm}.)
