...
accepted and does not break $\ur{k}$-ness of $\ps$.  Assuming that $\advse$ does
...
response property takes care of that, hence that event has already been covered
by the abort condition in $\game{1}$. If an adversary would like to reuse
...
We describe our extractor $\extse$ here. The extractor takes as input relation
...
adversary's transcript, denoted by $s_1$ in $\genforking$'s description.
...
new transcript $s_2$. $\zdv$ is run with different random oracle responses till
a tree $\tree$ of $n + 1$ valid transcripts is built. Then $\extse$ runs
internally the tree extractor $\exttree(\tree)$ and outputs what it returns.
...
$\exttree(\tree)$ fails to extract the witness. Denote that event by $\event{\errss}$.
