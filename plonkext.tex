...
\begin{theorem}
	$\plonkprotfs$ is simulation-extractable agains algebraic adversaries.
\end{theorem}
\begin{proof}[Proof idea]
	...
	\begin{itemize}
		...
		\item the adversary copies first $r$ round of some simulated proof in its final simulation-extractability breaking proof including the statement proven by $\simulator$. (Note that from the unique response property $0 \leq r < 3$.)
	\end{itemize}
	The reduction guesses query $i_\simulator$ and $r$ with probability $1/(3 q_\simulator)$. Assume its guess is correct.
	
	For $\advse$ queries for the simulated proof, except query $i_\simulator$ if $r > 0$, the reduction provides the adversary simulated proofs. For that particular query $\rdv$ starts its interaction with $\plonkprot.\verifier$:
	\begin{itemize}
	...
	\end{itemize}
	When $\advse$ decides to continue generating its malicious proof the reduction resumes its communication with $\plonkprot.\verifier$. That is, for each $\advse$ message it responds with the corresponding challenge from the verifier. In the end, the adversary returns a proof $\zkproof$.
	
	Assume the proof $\zkproof$ is acceptable. 
	...
	\end{multline}
	\end{proof}
	In that case \hl{show how to extract the witness}
	...
	\end{align}
	Since \cref{eq:ver_eq_poly} does not hold, $\p{V}(X)$ is not a zero polynomial. On the other hand, since \cref{eq:ver_eq} holds, $\p{V}(\chi) = 0$. Thus, one can factor $\p{V}$ and find $\chi$.
