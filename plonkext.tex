...
secure. Unfortunately, they then only \emph{conjecture} the security for their
...
\begin{remark}
  ...
  terms from the simulation-extractability definition. More precisely, allowing
  the extractor $\extse$ from \cref{def:simext} to be \emph{unbounded} and,
  given statement $\inp$ always finds a witness $\wit$ if there is any, we have
  $ \ssndProb = \accProb - \frkProb$. Note that such an unbounded extractor is
  justified in the light of simulation soundness definition. More precisely, we
  can equivalently define $\ssndProb$ as
\[
      \ssndProb =
      \Pr\left[
        \begin{aligned}
...
$\inp$ (the rest is given for the consistency with the simulation extractability
definition).
\end{remark}

In \cref{cor:simext_to_ssnd} we show that under some conditions this is enough
...
As noted in \cref{sec:simext_def} if we allow an unbounded extractor, then 
