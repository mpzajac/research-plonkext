...
\begin{description}
	...
	\begin{align*}
		...
		\begin{aligned}
			...
			% &   \\
			& - (\p{a}(\chz) + \beta \p{S_{\sigma 1}} + \gamma) (\p{b}(\chz) + \beta \p{S_{\sigma 2}} + \gamma) \alpha  \beta \p{z}(\chz \omega) \gone{\p{S_{\sigma 3}}(\chi)})
		\end{aligned}
		\right) + \\
		& + u \gone{\p{z}(\chi)}
	...
	\end{align*}
\begin{align*}
	\gone{F} & = \gone{\p{t_{lo}}(\chi)} + \chz^\numberofconstrains \gone{\p{t_{mid}}(\chi)} + \chz^{2 \numberofconstrains} \gone{\p{t_{hi}}(\chi)} \\
	& + \gone{D} +
