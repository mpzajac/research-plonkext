...
% The more
% rigorous approach is to prove security of the Fiat--Shamir heuristic in the
% random oracle model.
...
% This in turn raises the question: \emph{Are the security models of current
...
% shown secure in the structured reference string (SRS) model where a trusted
% third party provides the SRS for prover and verifier
% by the parties necessarily comes with a trapdoor which can be used to break
% (knowledge) soundness. That is, a malicious \emph{prover} equipped with the SRS
% trapdoor could convince a \emph{verifier} to accept a false statement or a
% statement for which the prover does not know the witness. Hence secure real-life
% deployment of zkSNARKs requires the trapdoor to remain secret.
...
inefficient. On the other hand, the extractor considered in this paper is
black-box as it only needs an oracle access to the adversary. Importantly, it
does not depend on any knowledge assumption.  (However, it also depends on a
number of properties required from the protocol. In case of $\plonk$ and
$\sonic$, these properties are often proven in the AGM.) We also note that when
an extractor depends on rewinding the adversary, it is expected to have
extractor's success probability dependent on the adversary's probability of
returning a valid proof.
...
\changedm{ We propose a new notion of \emph{updatable simulation extractability}
  which assures that the proof system is secure even when an adversary is
  allowed to update the SRS \emph{and} get simulated proofs for the statements
  it requires.
  ...
  \begin{definition}[Updatable simulation extractability]
    ...
    \emph{updatable simulation-extractable} with \emph{extraction error} $\nu$
    if for any $\ppt$ adversary $\adv$ that is given oracle access to a random
    oracle $\ro$, simulator $\simulator_\fs$, and SRS update oracle $\oraclesrs$
    defined at \cref{fig:oraclesrs}, and produces an accepting transcript of
...
\begin{figure}
  	\centering
		\fbox{
		\procedure{$\oraclesrs(\intent, \srs_n, (\rho_i)_{i = 1}^{n})$}		
		{
      \pcif \srs \neq \bot \pcthen \pcreturn 0 \\
      \pcif \intent = \setup \pcthen \\
      \pcind (\srs', \rho') \sample \kgen(\REL)\\
      \pcind Q \gets Q \cup \smallset{\rho'}\\
      \pcind \pcreturn (\srs', \rho')\\
      \pcif \intent = \update \pcthen\\
      \pcind b \gets \verifysrs(\REL, \srs_n, (\rho_i)_{i = 1}^n)\\
      \pcind \pcif b = 0 \pcthen \pcreturn 0 \\
      \pcind (\srs', \rho') \sample \srsupdate(\REL, \srs_n, (\rho)_{i = 1}^n)
      \\
      \pcind Q \gets Q \cup \smallset{\rho'}\\
      \pcind \pcreturn (\srs', \rho') \\
      \pcif \intent = \final \\
      \pcind b \gets \verifysrs(\REL, \srs_n, (\rho_i)_{i = 1}^n)\\
      \pcind \pcif b = 0 \lor Q \cap \smallset{\rho_i}_i = \emptyset \pcthen \pcreturn
      0 \\
      \pcind \srs \gets \srs_n\\
      \pcind \pcreturn \srs\\
      \pcelse \pcreturn 0
    }}
  \caption{SRS updates oracle}
  \label{fig:oraclesrs}
\end{figure}
}
