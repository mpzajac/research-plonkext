...
\plonk{}~\cite{EPRINT:GabWilCio19}, and \marlin~\cite{EC:CHMMVW20} are designed and proven secure as interactive protocols. Security is then only
...
transform for SRS-based zero knowledge proofs has not yet been
...
Security conjectures via the Fiat--Shamir transform is
...
\begin{proof}
  ...
  where $\epss(\secpar)$ is probability of breaking soundness of the underlying
  \emph{interactive} protocol $\ps$. Note that
  $\abs{\prob{\game{1}} - \prob{\game{2}}}$ is probability that $\adv$
  outputs an acceptable proof for a false statement, which does not break the
  unique response property (proofs breaking that property has been excluded by
  ...
  \item On input $h_{i_j}$ for the $i_j$-th,
    ...
    $\zkproof[j].\ch$. $\adv'$ sets $\ro(h_{i_j}) = \zkproof[j].\ch$.
  \item On query $\inp_\simulator$ to $\simulator$ it runs a simulator
    $\ps.\simulator$ internally and returns $\zkproof_\simulator$. If the random
    oracle query with input $\zkproof_\simulator[j]$, $1 \leq j < k$, of the
    simulator is the $i_j$-th query, generate $\zkproof_\simulator[j].\ch$ by
    invoking $\ps.\verifier$ on $\zkproof_\simulator[j]$ and programming
    $\ro(h_{i_j}) = \zkproof_\simulator[j].\ch$.
  \item Answers $\ps.\verifier$'s challenge $\zkproof[j].\ch$ using the
    answer given by $\adv$, i.e.~$\zkproof_\adv[j + 1]$.
  \end{itemize}
  ...
  $\adv'$ correctly guesses the indices of $\adv$ random oracle queries, what
  ...
  Note that in case of $\game{2}$ the adversary cannot win. Thus the probability
