...
the trapdoor $\td$ that is generated along $\srs$. In this paper we distinguish
...
\begin{definition}[$k$-programmable ZK]
  ...
  \item produces proofs indistinguishable from proofs output by an honest
    prover;
  \item $\simulator_\fs$ can program random oracle \emph{only} to compute
    challenges from $k$ to $\mu + 1$.
  \end{compactenum}
\end{definition}
We note that $\plonkprot$ is $2$-programmable ZK and $\sonicprot$ is
$1$-programmable what follows directly from proofs of their zero knowledge in
