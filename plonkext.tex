...
in $\p{P}_i$ for $i \in \smallset{1, 2, T}, j_i = \tau_i$. There is an oracle
...
\item[Group operations in $\GRP_1, \GRP_2, \GRP_T$:] On input
  ...
  $op = \msg{add}, \msg{sub}$, sets $\tau'_i \gets \tau_i + 1$, computes
  $p_{i, \tau'_i} = p_{i, j} \pm p_{i, j'}$ respectively to $op$. If there is
  another polynomial $p_{i, k} \in L_i$ such that $p_{i, k} = p_{\tau'_i}$, then the
  oracle returns encoding of $p_{i, k}$. Otherwise it sets the encoding
  $\xi_{i, \tau'_i}$ to a new unused random string, adds
  $(p_{i, \tau'_i}, \xi_{i, \tau'_i})$ to $L_i$, and returns $\xi_{i, \tau'_i}$.
\item[Bilinear pairing:] On input $(\xi_{1, j}, \xi_{2, j'})$ the oracle
  sets $\tau' \gets \tau_T + 1$ and computes $r_{\tau'} \gets p_{i, j} \cdot
  p_{i, j'}$. If $r_{\tau'} \in L_T$ then return encoding found in the list
  $L_T$, else pick a new unused random string and set $\xi_{T, \tau'}$ to it. Return
...
\begin{theorem}[Security of the \"uber assumption]
  ...
  $\p{P}_1, \p{P}_2$, then for any $\adv$ that makes up to $q$ queries to the
  GGM oracle holds:
  \[
    \Pr\left[
    \adv\left(
...
\begin{proof}
  ...
  \]
  for $i \in \smallset{1, 2, T}$. Invariant $\tau$ states that $\tau_0 + \tau_1
  = \tau + m_1 + m_2 + m$.

  After at most $q$ queries to the oracle, the adversary returns a bit $b'$. At
  that point challenger $\cdv$ chooses random
  $x_1, \ldots, x_n, y_1 \ldots, y_m$, random bit $b$, and sets
  $\p{F}_b \gets \p{F}(x_1, \ldots, x_n)$
  ...
  in equalities between polynomial evaluations that are not equalities between
  the polynomials. That is, the simulation is perfect unless for some $i, j, j'$
  holds
  \[
    p_{i, j}(x_1, \ldots, x_n) - p_{i, j'}(x_1, \ldots, x_n) = 0,
  \]
  for $p_{i, j}(X_1, \ldots, X_n) \neq p_{i, j'}(X_1, \ldots, X_n)$.
  Denote by $\bad$ an event that at least one of the three conditions
  holds. When $\bad$ happens, the answer $\cdv$ gives to $adv$ differs from an
  answer that a real oracle would give.
  \end{itemize}
  \end{proof}
