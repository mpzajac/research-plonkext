...
\begin{lemma}
	Let $\PCOM$ be a batched varsion of a KZG polynomial commitment \cite{AC:KatZavGol10} as described in \cite{EPRINT:GabWilCio19}. 
	Then for every PPT adversary $\adv$	
	\begin{multline*}
		\condprob{
		\PCOM.\verify(\crs, \vec{z}, \vec{s}, \vec{o})
		}
		{\crs \gets \PCOM.\kcrs(\secparam), 
			(\vec{z}, \vec{s}, \vec{o}) \gets \adv(\crs)
		}
		 \leq \negl.
	\end{multline*}
\end{lemma}
...
\begin{proof}
	...
	%We build an adversary $\bdv$ that breaks binding of the commitment scheme. To that end, we consider 2 cases, depending on which round $\adv$ is able to provide at least two outputs such that the resulting transcripts are acceptable.
...
\end{lemma}
\begin{proof}
	First, for the sake of clarity, we assume here that $\plonk$'s CRS consists only of monomials $\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$ for some trapdoor $\chi$. This is allowed as the rest of the CRS, as described in \cite{EPRINT:GabWilCio19}, can be computed from  $\gone{1, \chi, \ldots, \chi^(\numberofconstrains + 2)}$.
