...
\section{Witness extraction from multiround protocols}
...
Besides that, we additionally require that a protocol $\proofsystem$ has \emph{unique responses after $i$}. We call such a protocol an $\ur{i}$-protocol.
We call a $(2\mu + 1)$-move interactive protocol $\proofsystem = \ip{\prover}{\verifier}$ to be an $\ur{i}$ protocol if after the $i$-th round of the protocol (i.e.~after the $2i - 1$ message) all the responses of the prover are fixed. More precisely:
...
\end{definition}
For example $\Sigma$-protocols are often $\ur{1}$-protocols i.e.~when a conversation between the prover and the verifier compounds of an instance $\inp$ and three messages $(a, z, \gamma)$, then given $(a, z)$ it is infeasible for the prover to produce $\gamma, \gamma'$, such that $\gamma \neq \gamma'$ and $\verifier(\REL, \inp, a, z, \gamma) = \verifier(\REL, \inp, a, z, \gamma) = 1$.

\begin{lemma}
	Let $\proofsystem = \ip{\prover}{\verifier}$ be an interactive $\ur{i}$-protocol, such that there exists an extractor $\ext$ that extracts the witness from a $(1, \ldots, 1, n_i, \ldots, n_\mu)$-tree of accepting transcripts. 
