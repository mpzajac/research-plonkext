...
\michals{15.03}{Explain} Assume $\CRKT$ is a fan-in two arithmetic circuit,
which fan-out is unlimited and has $\numberofconstrains$ gates and $m$ wires
($\numberofconstrains \leq m \leq 2\numberofconstrains$). \plonk's constraint
...
We say that vector $\vx \in \FF^m$ satisfies constraint system if for all $i
\in \range{1}{\numberofconstrains}$
\[
...
\paragraph{$\plonk$ CRS generating algorithm $\kgen(\REL)$:}
...
Let $H = \smallset{\omega^i}_{i = 1}^{\numberofconstrains }$ be a (multiplicative) $\numberofconstrains$-element subgroup
of a field $\FF$ compound of $\numberofconstrains$-th roots of unity in $\FF$.  During the
preprocessing phase polynomials $\p{S_{id j}}, \p{S_{\sigma j}}$, for
$j \in \range{1}{3}$, are computed. The polynomials are such that:
\begin{align*}
  \p{S_{id j}}(\omega^i) & = (j - 1) \cdot \numberofconstrains + i,\\
  \p{S_{\sigma 1}}(X) & = X,\\
  \p{S_{\sigma 2}}(X) & = k_1 \cdot X,\\
  \p{S_{\sigma 3}}(X) & = k_2 \cdot X.
\end{align*}
Elements $k_1, k_2$ are chosen such that $H, k_1 \cdot H, k_2 \cdot H$ are
distinct cosets in $\FF^*$ (thus consist of $3 \cdot \numberofconstrains $ distinct
elements)\footnote{As noted in \cite{EPRINT:GabWilCio19}, $k_1$ may be a
  quadratic residue and $k_2$ a quadratic non-residue.}.

Furthermore, we define polynomials $\p{q_L}, \p{q_R}, \p{q_O}, \p{q_M}, \p{q_C}$
such that for $i \in \range{1}{\numberofconstrains }$
\begin{align*}
  \p{q_L}(\omega^i) & = \vql_i, \\
  \p{q_R}(\omega^i) & = \vqr_i, \\
  \p{q_M}(\omega^i) & = \vqm_i, \\
...
\paragraph{$\plonk$ prover
  $\prover(\REL, \crs, \inp, \wit = (\wit_i)_{i \in \range{1}{3 \cdot
      \numberofconstrains}})$.}
\begin{description}
...
\subsubsection{Algorithms rolled out}
\paragraph{$\sonic$ CRS generation $\kgen(\REL)$.} The CRS generating algorithm picks
randomly $\alpha, \chi \sample \FF_p$ and outputs
	\[
      \crs = \left( \gone{\smallset{\chi^i}_{i = -\dconst}^{\dconst},
        ...
        \gtar{\alpha} \right)
	\]
\paragraph{$\sonic$ prover $\prover(\REL, \crs, \inp, \wit=\va, \vb, \vc)$.}
\begin{description}
\item[Round 1] The prover picks randomly randomisers
  ...
  $\gone{r} \gets \com(\crs, \multconstr, \pr(X, 1))$.  Then it gets challenge $y$ from
  the verifier.
\item[Round 2] $\prover$ commits to $\pt(X, y)$ and outputs
  $\gone{t} \gets \com(\crs, \dconst, \pt(X, y))$. Then it gets a challenge $z$ from
  the verifier.
\item[Round 3] The prover computes commitment openings. That is, it outputs
  \begin{align*}
    \gone{o_a} & = \open(\crs, z, \pr(z, 1), \pr(X, 1)) \\
    \gone{o_b} & = \open(\crs, yz, \pr(yz, 1), \pr(X, 1)) \\
    \gone{o_t} & = \open(\crs, z, \pt(z, y), \pt(X, y)) 
  \end{align*}
...
\item[Round 4] In the next round the prover computes
  $\gone{c} \gets \com(\crs, \dconst, \p{s}(u, x)) \cdot \gone{\p{s}(u, x)}$ and
  compute commitments' openings
  \begin{align*}
    \gone{w} & = \open(\crs, u, \p{s}(X, y)), \\
    \gone{q_y} & = \open(\crs, y, \p{s}(u, Y)),
  \end{align*}
...
\item[Round 5] In the final round, $\prover$ computes opening
  $\gone{q_z} = \open(\crs, z, \p{s}(u, X))$ and outputs $\gone{q_z}$ and $\p{s}(u, z)$.
\end{description}

\paragraph{$\sonic$ verifier $\verifier(\REL, \crs, \inp, \zkproof)$.} The verifier
...
\begin{equation*}
  \begin{split}
    &\PCOMs.\verifier(\crs, \multconstr, \gone{r}, z, a', \gone{o_a}), \\
    &\PCOMs.\verifier(\crs, \multconstr, \gone{r}, yz, b', \gone{o_b}),\\
    &\PCOMs.\verifier(\crs, \dconst, \gone{t}, z, t', \gone{o_t}),\\
  \end{split}
  \qquad
  \begin{split}
    &\PCOMs.\verifier(\crs, \dconst, \gone{s}, u, s, \gone{w}),\\
    &\PCOMs.\verifier(\crs, \dconst, \gone{c}, y, s, \gone{q_y}),\\
    &\PCOMs.\verifier(\crs, \dconst, \gone{c}, z, s_z, \gone{q_z}),
  \end{split}
\end{equation*}
...
\[
  \Pr \left[
    \begin{aligned}
      & \verify(\crs, \gone{c}, z, s, \gone{o}) = 1, \\
      & \verify(\crs, \gone{c}, z, \tilde{s}, \gone{\tilde{o}}) = 1
    \end{aligned}
    \,\left|\, \vphantom{\begin{aligned}
          & \verify(\crs, \gone{c}, z, s, \gone{o}),\\
          & \verify(\crs, \gone{c}, z, \tilde{s}, \gone{\tilde{o}}) \\
          &o \neq \tilde{o})
		\end{aligned}}
      \begin{aligned}
        & \crs \gets \kgen(\secparam), \\
        & (\gone{c}, z, s, \tilde{s}, \gone{o}, \gone{\tilde{o}}) \gets \adv(\crs)
      \end{aligned}
    \right.\right]
  % \leq \negl.
\]
...
\begin{description}
	\item[Round 1]
		...
		i}$. Commits to $\pr(X, 1)$ and outputs $R \gets \com(\crs, \multconstr,
		\pr(X, 1))$.
		Then it gets challenge $y$ from the verifier.
      \item[Round 2] $\prover$ commits to $\pt(X, y)$ and outputs
        $T \gets \com(\crs, \dconst, \pt(X, y))$. Then it gets a challenge $z$
        from the verifier.
	\item[Round 3] The prover computes commitment openings. That is, it outputs
		\begin{align*}
