...
point $z$ chosen by $\receiver$.\hamid{The definition of polynomial commitment and how the algorithms $(\kgen, \com, \open, \verify)$ are defined is not given neither here nor in the appendix. I think it would be nice to define these algorithms somewhere even informally. My suggestion is as follows:
\begin{description}
	\item[$\kgen(1^\secpar, \maxdeg)$.] The key generation algorithm $\kgen(1^\secpar, \maxdeg)$ takes in a security parameter $1^\secpar$ and a parameter $\maxdeg$ which determines the maximal degree of the committed polynomial. It outputs a structured reference string $\srs$ (including a commitment key).
	\item[$\com(\srs, \p{f})$.] The commitment algorithm $\com(\srs, \p{f})$ takes in $\srs$ and a polynomial $\p{f}$ with maximum degree $\maxdeg$, and outputs a commitment $c$.
	\item[$\open(\srs, z, s, \p{f})$.] The opening algorithm $\open(\srs, z, s \p{f})$ takes as input $\srs$, an evaluation point $z$, a value $s$ and the polynomial $\p{f}$. It outputs an opening $o$.
	\item[$\verify(\srs, c, z, s, o)$.] The verification algorithm takes in $\srs$, a commitment $c$, an evaluation point $z$, a value $s$ and an opening $o$. It outputs 1 if $o$ is a valid opening for $(c, z, s)$ and 0 otherwise.
\end{description}}

$\plonk$ and $\sonic$ use variants of the KZG
  ...
  all $\REL \in \RELGEN(\secparam)$
	\[
      \condprob{\ip{\adv(\REL, \srs, \inp)}{\verifier(\REL, \srs, \inp)} =
        ...
        \LANG_\REL} \leq \negl\,;
	\]
	\hamid{I suggest to have all conditions on the left, like this:
\[
\condprob{\ip{\adv(\REL, \srs, \inp)}{\verifier(\REL, \srs, \inp)} =
	1 \land \inp \not\in
	\LANG_\REL}{\srs \gets \kgen(\REL), \inp \gets \adv(\REL, \srs)} \leq \negl\,;
\]	
}
%\end{description}
...
%  \]
 ...
 there exists a $\ppt$ simulator $\simulator$ such that \hamid{Shouldn't be for any
 	$\REL \in \RELGEN(\secparam)$, and adversary $\adv$
 	there exists a $\ppt$ simulator $\simulator$ such that for any $(\inp, \wit) \in \REL$...?}
	\begin{multline*}
...
\emph{computational} if they are indistinguishable for any $\nuppt$ \hamid{This acronym for "non-uniform PPT" is not defined in the paper.}
distinguisher.
...
Formally,
%\begin{description}
\ourpar{Special soundness.}\hamid{The last (short) sentence looks a little unclear.} A sigma protocol $\sigmaprot$ is \emph{specially-sound}
  if for any adversary $\adv$ the probability
	\[
		...
		\right.\right]
\]
\hamid{I would suggest this (although I agree this looks uglier):
\[
\Pr\left[
\begin{aligned}
& \verifier(\REL, \inp, (a, b, z)) = %\\
...
\begin{definition}[Forking simulation-extractable NIZK, \cite{INDOCRYPT:FKMV12}]
  ...
  $\ps$ with probability $\accProb$, that is \hamid{I think a part of the sentence saying ``there exists an extractor $\ext_\se$'' is missing.}
	\[
		\accProb = \Pr \left[
		\begin{aligned}
...
%
 ...
 % \smallset{\gone{\chi^{\alpha}}}$
  ...
  \item From the challenger get the challenge commitment $c$.
  \item Let $S$ be the upper bound on the number of simulator oracles queries
    the adversary can make. \michals{12.9}{Note -- new bound!}
  \item Guess which simulator's response is going to be used by $\adv$ in its
    proof. Let $s$ be the index of this response.
  \item Guess which of the simulated polynomials in response $s$ will be
    used. Let $i$ be the index of this polynomial.
