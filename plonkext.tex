...
\section{Simulation extractability}
\subsection{Special-soundness of $\plonkprot$}
Note that the special soundness property as usually defined holds for all, even computationally unbounded, adversaries. Since a simulation trapdoor for \plonk{} exists, it is not special sound in that regard. However, we provide a weaker, yet sufficient, definition of \emph{computational special soundness}. More precisely, we state that an adversary that is able to answer correctly multiple challenges either knows the witness or can be used to break a compuational assumption. 

\newcommand{\compass}{\mathtt{C}}
% \newcommand{\tree}{T}
\begin{definition}[Computational special soundness under $\compass$]
	Let $\compass = (\cdv, c)$ be a falsifiable assumption and $\proofsystem$ be an $k$-round zero-knowledge proof system for a relation $\REL$. 
	...
	\begin{compactitem}
		...
		\item there exists a reduction $\rdv$ that black-box uses $\adv$ to break $\compass$.
	\end{compactitem}
\end{definition}
...
\begin{proof}
	...
	First, $\event{E}$ holds when all of the transcripts were generated honestly;
	second, $\nevent{E}$ holds when there is a transcript that is acceptable, yet the adversary deviated from the protocol.
...
\begin{theorem}[Simulation extractability]
	...
	\]
	\michals{16.09}{Should inlice the simulation error above?}
