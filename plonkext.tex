...
\begin{proof}
	...
	We build two reductions $\rdvse$ and $\rdvdlog$, which given the tree of transcripts $\tree$ try to reveal the witness ($\rdvse$) or break the dlog assumption ($\rdvdlog$). We show that the probability that one of them succeeds is non-negligible.
	
	\ncase{When $\event{E}$ happens} Since all of the transcripts have been generated honestly then there is a witness $\wit$, such that $\REL(\inp, \wit)$. 
	Note that witness-carrying polynomials $\p{a}(\chi), \p{b}(\chi), \p{c}(\chi)$ have degree $(\numberofconstrains + 2)$ and since $\adv$ answered honestly on $(\numberofconstrains + 3)$ different challenges $\chz$ then $(\numberofconstrains + 3)$ evaluations of these polynomials (at different points) are known. Thus the polynomials can be interpolated and the witness exposed.
	
	\ncase{When $\nevent{E}$ happens}
	Consider a transcript that is acceptable, yet has not been generated honestly.
	In this case, from the soundness of the polynomial protocol, with overwhelming probability, $\vereq(X) \neq 0$, but $\vereq(\chi) = 0$. 
\end{proof}
