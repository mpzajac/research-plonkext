...
interactive proof systems. \xout{(It can however be adjusted to work with NIZKs
  as well as presented on
  \mbox{\cref{fig:extractor_and_tree_finding_algorithm}}.)}
...
seeing a number of random oracle responses.

More precisely, at \cref{fig:extractor_and_tree_finding_algorithm}, line
\ref{line:branching}, there is a first branch of the tree already built and the
algorithm tries to find another one by presenting the adversary with a new set
of challenges. The problem is that when $\adv$ sees $h'_j$ then it may decide to
start the proof over \emph{with a new instance}. In that case the resulting
proof cannot be joined to the existing one (to make a tree) simply because it is
for a different instance. Obviously, the adversary cannot change the instances
indefinitely, so there is a chance that the tree will be finally build; however,
it is far from obvious and requires additional analysis.

Alternatively, the adversary may not pick a new instance after seeing $h'_j$,
but ``rewind'' its own proof. Say, that $\adv$ has prepared first $k$ messages
...
The generalised forking lemma proposed in this paper solves the issues above.
First of all, the 
