...
\michals{25.08}{I think the following results could be quite easily generalise to polynomial protocols, not just plonk.}
\begin{lemma}
	\label{lem:plonk_ur}
	$\plonkprot$ has unique response property after Round 2.
\end{lemma}
\begin{proof}
	We assume that the contrary is true and analyse possible cases depending on which round a malicious prover $\adv$ provides at least two acceptable outputs.
	\case{1}
	In Round 3 the prover is tasked to output a commitment to the quotient polynomial $\p{t}$ that assures that all constrains---extended permutation, constrains of the circuits, etc.---are satisfied. Since the polynomial commitment is deterministic, there is only one value that can be produced by the prover.
	...
	%
	In that case either $\gone{\vec{t}}$ or $\gone{\vec{t'}}$ is not a commitment to the quotient polynomial and it should be rejected by the verifier or the proof system is not sound.
	\case{2}
	...
	Clearly, at least one of $\vec{r_4}$, $\vec{r'_4}$ has to be incorrect, thus it should be rejected of the verifier of the proof system.
	\case{3} 
	In the last round of the protocol the prover provides openings for the polynomial commitments done before. 
	Assume $\adv$ is able to produce two different polynomial commitment opennings pairs: 
	$\vec{r_5} = (\ev{\p{W_\chz}}, \ev{\p{W_{\chz \omega}}})$ and 
	$\vec{r'_5} = (\ev{\p{W_\chz}}', \ev{\p{W_{\chz \omega}}}')$.
	As previously one of the openings has to be incorrect and should be rejected by the verifier if the proof system be sound.
	\qed
\end{proof}
...
The proof idea is following. First the reduction takes the CRS from a challenger $\cdv_\plonkprot$ or $\cdv_\dlog$, where the former represents game where the reduction tries to break soundness of $\plonkprot$ and the latter game of breaking the dlog assumption. We run the reductions in parallel and use $\rdv$ to denote either $\rdvks$ or $\rdvdlog$.
...
Note that $\plonkprotfs$ consists $6$ calls to a random oracle and for the adversary to be successful, it has to perform at least that number of queries performing its simulation extractability-breaking proof. The reduction tries to guess which queries, out of all random oracle queries submitted by the adversary, are going to be used in the final proof. Denote the queries by $h_{i_1}, \ldots, h_{i_6}$. 
...
In the end, the adversary outputs a proof $\zkproof$ that should be acceptable by $\plonkprotfs.\verifier$, thus the proof produced by $\rdv$ with challenges from $\plonkprot.\verifier$ should be acceptable as well. Using the fact that the adversary is algebraic, the reduction knows coefficients of the CRS elements used in the output proof. For the adversary to be successful the proof cannot only be acceptable, it should also be impossible to extract a correct witness from it. We consider two cases. In the first, the proof is acceptable, but the statement is false. This allows $\rdv$ to break the dlog assumption. Another case is when the proof is acceptable, statement is valid, but the witness cannot be extracted. This proof allows $\rdv$ to break knowledge soundness of $\plonkprot$.
...
\end{theorem}
\michals{25.08}{I think the proof should be modified s.t. the reduction only guesses ro queries $h_1, ..., h_6$ instead of the queries plus $i_\simulator$}
\begin{proof}
	\michals{26.05}{The following is 2 days old and should be modified a bit}
	The proof goes by contradiction. Assume there exists an adversary $\advse$ that breaks simulation extractability with non-negligible probability $\eps$. We use $\advse$ to build two reductions $\rdvks$ and $\rdvdlog$, where the former uses $\advse$ to break knowledge soundness of $\plonkprot$ and the latter to break the $(\numberofconstrains + 2)$-dlog assumption.
	Since the reductions behave almost identically, we present them together and use $\rdv$ to denote both them. 
	Note that both reductions run in parallel and have access to independent copies of $\advse$.
	
	The reduction $\rdvks$ starts with obtaining $\crs$ from $\plonkprot.\kgen$ and passing it to $\advse$. Similarly, $\rdvdlog$ gets its challenge from the $(\numberofconstrains + 2)$-dlog challenger $\cdv_\dlog$.
	Assume that 
	\begin{itemize}
		\item the adversary makes $q_\simulator$ queries to the simulator $\plonkprotfs.\simulator$ obtaining $q_\simulator$ simulated proofs;
		\item the adversary copies first $r$ round of some simulated proof in its final simulation-extractability breaking proof including the statement $\inp$ proven by $\plonkprotfs.\simulator$; note that from the unique response property $0 \leq r < 3$;
		\item the adversary makes at most $q_\ro$ random oracle queries---including the queries that the simulator has to perform to be able to provide a simulated proof; we denote by $h$ a list of random oracle queries $h_{i_1}, \ldots, h_{i_6}$ that are used in the simulation-extractability breaking proof $\zkproof$ returned by $\advse$.
	\end{itemize}
	The reduction guesses indices of queries $i_\simulator, h_{i_1}, \ldots, h_{i_6}$ and $r$ with probability 
	\[
	\frac{1}{3 \cdot q_\simulator \cdot  q_\ro^6}.
	\]
	Assume the guess is correct.
	
	For $\advse$'s queries for a simulated proof, except query $i_\simulator$ if $r > 0$, the reduction provides the adversary the required proofs. However, when the simulator is about to make a random oracle query $h_{i_j}$ from the list $h$ it submits the query to $\plonkprot.\verifier$ and returns as $h_{i_j}$ the verifier's challenge (we assume that in $\plonkprot$ the first message sent by the prover to the verifier includes the proven statement $\inp$).
	
	For query $i_\simulator$ the reduction $\rdv$ starts its interaction with $\plonkprot.\verifier$:
	\begin{itemize}
		\item In Round 1, $\rdv$ randomly picks polynomials $\p{a}, \p{b}, \p{c}$.
		\item In Round 2, $\rdv$ gets challenge $\beta, \gamma$ from $\plonkprot.\verifier$, creates $\p{z}$ exactly as the simulator would---including $\beta, \gamma$ in $\p{z}$---and returns $\gone{\p{z}(\chi)}$.
		\item If $r = 1$, $\rdv$ pauses its interaction with the verifier and continues generating the proof as $\plonkprotfs.\simulator$ would generate.
		\item If $r = 2$, $\rdv$ continues talking with the verifier and gets in Round 3 challenge $\alpha$. After that the reduction pauses its interaction with the verifier and continues its simulated proof playing $\plonkprotfs.\simulator$ for the rest of the rounds.
	\end{itemize}
	(Communication with the verifier is resumes when the adversary makes a query from $h$).
	\michals{24.08}{The proof should hold for a bit better parameter, when we only guess the random oracle queries, putting aside guessing the simulator query}
	% When $\advse$ decides to continue generating its malicious proof the reduction resumes its communication with $\plonkprot.\verifier$. 
	% That is, for each $\advse$ message it responds with the corresponding challenge from the verifier. 
	In the end, the adversary returns a proof $\zkproof$. We assume that $\plonkprot.\verifier$ accepts $\zkproof$.
	% 	\michals{21.08}{Maybe it is enough to show a reduction to breaking k. soundness of  $\plonkprotfs$. What is the role of the $\plonkprot.\verifier$?}
	% \michals{24.08}{If we stick to the non-interactive setting, then we need to program the oracle to be able to produce an acceptable simulated proof. I.e. we cannot simulate on our own, but need to rely on simulator's superpowers.}
	
	Let 
	\begin{align*}
		\zkproof = & \gone{\p{a}(\chi), \p{b}(\chi), \p{c}(\chi), \p{z}(\chi),
		\p{W_\chz}(\chi), \p{W_{\chz \omega}}(\chi), \p{t_{lo}}(\chi),
		\p{t_{mid}}(\chi), \p{t_{hi}}(\chi)}, \\
		& \p{a}(\chz), \p{b}(\chz), \p{c}(\chz), \p{S_{\sigma 1}}(\chz),
		\p{S_{\sigma 2}}(\chz), \p{r}(\chz), \p{z}(\chz \omega)\,.
	\end{align*}
	Since $\advse$ is algebraic, each of the group elements it outputs is accompanied with coefficients for each of group elements that are input to $\advse$. 
	That is, the CRS $\crs$ and simulated proofs provided by $\rdv$.
	Hence, each proof element in $\GRP_1$ can be written as 
	\[
		\gone{\iota} = \vec{\iota} \cdot (\underbrace{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}_{\crs} \| \vec{\zkproof_\simulator}_1 \| \ldots \| \vec{\zkproof_\simulator}_{q_\simulator}),\] 
	where $\vec{\zkproof_\simulator}_1, \ldots, \vec{\zkproof_\simulator}_{q_\simulator}$ are vectors representing group elements in the simulated proofs obtained by the adversary.
	Importantly, from the way the simulator works, the reduction knows representation of each $\vec{\zkproof_\simulator}_1, \ldots, \vec{\zkproof_\simulator}_{q_\simulator}$ as a vector of  $(1, \chi, \ldots, \chi^{\numberofconstrains + 2})$.
	
	Consider the following two cases. First, that the verification equation for polynomial evaluations hold, so does the verification equation on polynomials themselves. That is
	\begin{multline}
		\label{eq:ver_eq_poly}
		\left(
		\p{W_{\chz}}(X) + u \cdot \p{W_{\chz \omega}}(X)
		\right) \cdot 
		X = 
		\chz \cdot \p{W_{\chz}}(X) + u \chz \omega \cdot \p{W_{\chz \omega}}(X) + F(X) - E\,.
	\end{multline}
	\end{proof}
	In that case $\rdv$ reveals witness $\wit$ from $\advse$'s coefficients of $\p{a}(\chi), \p{b}(\chi), \p{c}(\chi)$. If for the extracted $\wit$ holds $\REL(\inp, \wit) = 1$, then $\advse$ failed to break simulation extractability of $\plonkprotfs$. On the other hand, if $\REL(\inp, \wit) = 0$, then $\rdv$ breaks knowledge soundness of $\plonkprot$. 
	
	The second case is when the verification equation of evaluations holds (as it is required for the adversary to be successful), but the polynomial equation does not. In that case $\advse$ can be used to break $(\numberofconstrains + 2)$-dlog assumption. That is, let 
	\begin{align}
		\p{V}(X) = \p{W_{\chz}}(X) + u \cdot \p{W_{\chz \omega}}(X)
		\cdot X - \chz \cdot \p{W_{\chz}}(X) - u \chz \omega \cdot \p{W_{\chz \omega}}(X) -  F(X) + E\,.
	\end{align}
	Since \cref{eq:ver_eq_poly} does not hold, $\p{V}(X)$ is not a zero polynomial. On the other hand, since \cref{eq:ver_eq} holds, $\p{V}(\chi) = 0$. Thus, one can factor $\p{V}$ and find $\chi$ breaking the dlog assumption.
	
\COMMENT
{\begin{lemma}
	\plonkint{} is special-sound against AGM adversaries.
\end{lemma}
\begin{proof}
	Proof outline. 
	In the following games an AGM adversary $\adv$ is tasked to break $m$-special-soundess property of $\plonkint$. The extractor $\ext$ which from $m$ accepting transcripts for a statement $\inp$ reveals the corresponding witness $\wit$ is the games' challenger $\cdv$. 
	We allow $\adv$ to ask about simulated proofs for chosen statements. These queries are handled by $\cdv$ as well.
	\paragraph{Game $\game{1}$:}
		The challenger picks trapdoor $\chi$, computes \plonk's CRS $
		\crs$ and gives it to an AGM adversary $\adv$. The challenger also provides the adversary with the CRS and plays the role of the verifier $\plonkint.\verifier$ of the proof system. 		
		Finally, the adversary outputs an instance $\inp$ and proof $\zkproof$. If the proof is acceptable by $\plonkint.\verifier$ then with overwhelming probability the extractor returns $\wit$ such that $(\inp, \wit) \in \REL$.
		
		\paragraph{Game $\game{2}$:}
		The game runs as the previous except that the challenger $\cdv$ additionally tries to break a $(\numberofconstrains + 2)$-dlog assumption. To that end it engages in a $(\numberofconstrains + 2)$-dlog game with a $(\numberofconstrains + 2)$-dlog challenger $\cdv_\dlog$ who provides $\cdv$ with $(\numberofconstrains + 2)$-dlog input $\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$. 
		The challenger $\cdv$ computes the rest of $\plonkint$ CRS and starts the game with the adversary $\adv$. (In this game we do not specify what $\cdv$ returns to $\cdv_\dlog$).
		
		\paragraph{Game $\game{3}$:}
		The game goes as previous except that the challenger $\cdv$ runs runs $\plonkintpoly.\verifier$ along with  $\plonkint.\verifier$. More precisely, $\cdv$ picks challenges that the verifiers would send and provide them to the adversary when necessary. 
		Since $\adv$ is algebraic for each polynomial commitment $\gone{f(\chi)}$ it sends it also provides $f$'s coefficients. That is, the only group elements $\adv$ has access to are the CRS $\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$ and the proofs it required from the simulator (operated by $\cdv$).
		 
		This allows $\cdv$ to recreate the polynomials committed by $\adv$ and transform its messages to messages that (potentially malicious) $\plonkintpoly.\prover$ would send. 
		
		In the end, the challenger checks verification equations of $\plonkint$ and $\plonkintpoly$. The latter is for polynomials, while to former for the polynomials' evaluations.
		Similarly as in \cite{EPRINT:GabWilCio19} we note that  $\plonkintpoly$'s polynomial verification equation accepts only true statements except negligible probability. Furthermore, we denote by $\bad$ the case when the verification equation for $\plonkint$ holds, while $\plonkintpoly$ does not. We show that if $\bad$ happens, then $\adv$ can be used to break $(\numberofconstrains + 2)$-dlog assumption.
		
		\paragraph{Game $\game{4}$:}
		
		% Show unique response property for the polynomial protocol (statistical unique response property)
	% \end{description}
	\qed
\end{proof}}
...
\comment{
\item[Round 5]
In this round the prover is asked to output values 
$\gone{\p{W}_{\chz}(\chi), \p{W}_{\chz \omega}(\chi)}$ 
assuring that required polynomials' evaluations are computed correctly. 
Since the simulator could not compute them honestly using the instance's witness, it has to use its power to make the verification equation hold.
To that end $\simulator$:
\begin{itemize}
	\item programs the random oracle in a way that $\ro(\trans)$ returns $v$ such that 
	$\gone{\p{W}_{\chz \omega}(\chi)}$ as
	\begin{align}
		...
		\right)\,.
	\end{align}
	Value $v$ is picked such that a verification equation 
	\item computes the opening of $\p{z}$ at $\chz \omega$, i.e.~ $\gone{\p{W}_{\chz \omega}(\chi)}$, honestly, i.e.
	\begin{align}
