...
sacrificing its efficiency. \markulf{12/11/2020}{Not actually related to soundness.}
...
and make it updatable.\markulf{12/11/2020}{arguably there is a transformation
...
knowledge that are zero-knowledge \markulf{12/11/2020}{Should this be HVZK, also
in abstract?} in the standard model and have a unique
...
$\PCOM$:
\begin{description}
	\item[Evaluation binding] a $\ppt$ adversary $\adv$ which outputs a commitment $c$
...
$\p{f}(z)$. \markulf{12/11/2020}{The definition below, which I believe is the
  standard one, does not really match up with the informal description above.
  There is no $f$ or $z$.}
	...
	$\vec{z},\vec{z}' \in \FF_p^l$ be the arguments the polynomials are evaluated at, $\vec{s},\vec{s}' \in \FF_p^k$ the evaluations, and 
