...
\let\accentvec\vec \documentclass[runningheads]{llncs}
...
not special-sound, the forking lemma, as stated in \cite{CCS:BelNev06}, cannot
...
protocol is proposed and its security and special-soundness analysed. Second,
...
problem by showing their protocol secure directly in the algebraic group model.
...
% Due to the lack of space, the assumptions
  ...
  commitment openings. Then for every $\ppt$ adversary $\adv$
	\[
		\Pr
			\left[
			\begin{aligned}
				& \verify(\crs, \vec{c}, \vec{z}, \vec{s}, \vec{o}) = 1,  \\ 
				& \verify(\crs, \vec{c}, \vec{z}, \vec{s}', \vec{o}') = 1, \\
				& \vec{s} \neq \vec{s}'
			\end{aligned}
...
conversation between $\prover$ with input $(\REL, \crs, \inp, \wit)$ and
...
transcript. We sometimes abuse notation and write $\verifier(\REL, \crs, \inp, \zkproof) = 1$ to
denote a fact that $\zkproof$ is accepted by the verifier. (This is especially
handy when the proof system is non-interactive, i.e.~the whole conversation
between the prover and verifier consists of a single message $\zkproof$ sent by
$\prover$).
  ...
  all $\REL \in \RELGEN(\secparam)$
	\[
      \condprob{\ip{\adv(\REL, \crs, \inp)}{\verifier(\REL, \crs, \inp)} =
        ...
        \LANG_\REL} \leq \negl\,;
	\]
\item[Zero knowledge] We call a proof system $\proofsystem$
  \emph{zero-knowledge} if for any $\REL \in \RELGEN(\secparam)$, $(\inp, \wit)
  \in \REL$, and adversary $\adv$ there exists a $\ppt$ simulator $\simulator$
  such that
	\begin{multline*}
...
\end{description}
A CRS $\crs$ may come with a secret string called \emph{trapdoor} $\td$ that
allows the simulator to produce a simulated proof. In that case algorithm
$\kgen(\REL)$ outputs $(\crs, \td)$ and $\td$ is given to the simulator. In this
paper we distinguish simulators that requires a trapdoor to simulate and those
that do not. We call the former \emph{CRS-simulators} and denote by
$\simulator_\td$ for trapdoor $\td$.
...
Formally,
\begin{description}
\item[Special soundness] A sigma protocol $\sigmaprot$ is \emph{specially-sound}
...
\begin{description} 
  \item[Unique response
  property] Let $\sigmaprot = (\prover, \verifier, \simulator)$ be a
  sigma-protocol for $\REL \in \RELGEN(\secparam)$ which proofs
  compound of three messages $(a, b, z)$. We say that $\sigmaprot$ is
  has a unique response property if for all $\ppt$ algorithms $\adv$
  holds \[ \condprob{\verifier (\REL, \inp, (a, b, z)) = \verifier
    (\REL, \inp, (a, b, z'))  = 1}{(\inp, a, b, z, z') \gets
  \adv(\REL), z \neq z'} \leq \negl\,.  \] 
\end{description} 
If this
  property holds even against unbounded adversaries, it is called
  \emph{strict}, cf.~\cite{INDOCRYPT:FKMV12}. Later on we call
  protocols that follows this notion \emph{ur-protocols}. For the sake
  of completeness we note that many sigma protocols, like
  e.g.~Schnorr's protocol \cite{C:Schnorr89}, fulfil this property.
...
zero-knowledge interactive proof system $\proofsystem = (\kgen,
\prover, \verifier, \simulator)$ for $\REL \in \RELGEN(\secparam)$.
Let $\zkproof$ be a proof performed by the prover $\prover$ and
verifier $\verifier$ compound of messages $(a_1, b_1, \ldots, a_{\mu -
1}, b_{\mu}, a_{\mu + 1})$, where $a_i$ comes from $\prover$ and $b_i$
comes from $\verifier$.  Denote by $\ro$ a random oracle. 
Let $\proofsystem_\fs = (\kgen_\fs, \prover_\fs, \verifier_\fs,
\simulator_\fs)$ be a proof system such that
\begin{itemize}
...
h_{i_j}^{j} \neq h_{i_{j'}}^{j'} \text{ for } (j, j') \in J}	\\
  ...
  & \qquad - \prob{i_j \geq 1 \land h_{i_j}^{j} = h_{i_{j'}}^{j'} \text{ for some } (j, j') \in J}\\
...
internally runs the forking algorithm $\genforking_\zdv$.  Algorithm $\zdv$
takes $(\REL, \crs, \advse,
%\inp_\advse,
%\zkproof_\advse, 
Q, r)$ as input $y$ and $Q_\ro$ as input $h_1^1, \ldots,
h_q^1$. 
(For the sake of completeness, we allow $\genforking_\zdv$ to
pick $h^1_{l + 1}, \ldots, h^1_q$ responses if $Q_\ro$ has only $l < q$
elements.)  

Next, $\zdv$ runs internally $\advse(\REL, \crs; r)$ and responds to its random
...
$i$ is the index of a random oracle query required to get the challenge after
...
but this time it provides a fresh random oracle responses $h^2_i, \ldots,
...
unique response property of the transformed sigma protocol. Fischlin's
...
model, but utilises CRS what also complicates things considerably.
...
output by the prover, We say that $\proofsystem$ has \emph{unique responses
  from $i$} if for any $\ppt$ adversary $\adv$:
\[
	\prob{
		\begin{aligned}
    ...
    a'_{\mu + 1})
		\gets \adv^\ro(\REL, \crs), \\
    & \vec{a} \neq \vec{a'}, a_1, \ldots, a_{i + 1} = a'_1,
    \ldots, a_{i + 1}, \\
...
simulation trapdoor for $\plonkprot$ exists, the protocol is not special sound
in that regard. This comes since an unbounded adversary could reveal the
trapdoor and build a number of simulated proofs for a fake statement. Hence, we
provide a weaker, yet sufficient, definition of \emph{computational special
  soundness}. More precisely, we state that an adversary that is able to answer
correctly multiple challenges either knows the witness or can be used to break
...
verifier zero-knowledge in the standard model\footnote{Crucially, we require
  ...
  additional knowledge, as e.g~knowledge of a CRS trapdoor.}, has $\ur{k}$
property with security $\epsur$, is $(1, \ldots, 1, n_k, 1, \ldots, n_\mu)$-special
sound.
% Assume that the simulator $\simulator$ does not get as input any trapdoor
% related to the CRS.
...
\[
	\extProb \geq \frac{1}{q^{n_k - 1}} (\accProb - \epsur)^{n_k} -\eps\,,
\]
...
\begin{proof}		
  ...
  a transcripts of $\advse$'s operations, is tasked to extract a witness
...
\item When $\advse$ makes a fake proof $\zkproof_{\advse}$ for $\inp_{\advse}$,
  ...
  aborts also when it fails to build a $(1, \ldots, 1, n_k, 1, \ldots, 1)$-tree
...
% That is, assume that is not true and for some query
...
fresh challenge after round $k + 1$.%\michals{15.01}{$k + 1$ vs $k$}
%
Otherwise, if $\ur{k}$-ness does not hold and the adversary
...
$\ext$ starts a forking algorithm $\genforking^{n_k + 1}_\zdv(y,h_1, \ldots,
...
adversary's transcript, in $\genforking$ description denoted by $s_1$. Then new
random oracle responses are picked (for the later queries) and the adversary is
rewound to the point just prior it gets the response to RO query
$\zkproof_{\advse}[0..k]$. The adversary gets a random oracle response from a
new set of responses $h^2_i, \ldots, h^2_q$. If the adversary requests a
...
adversary to compute $\ro(\zkproof_{\advse}[0..k])$, and a new transcript form
the adversary $s_2$. $\zdv$ is run with different random oracle responses till
$n_k + 1$ valid transcripts are collected.

If the unique response property does not hold then in some $j$-th execution of
...
\[
	...
	\waccProb \cdot \left(\frac{\waccProb^{n_k - 1}}{q^{n_k - 1}} +
	\frac{(2^\secpar) !}{(2^\secpar - n_k)! \cdot
(2^\secpar)^{n_k}} - 1\right)\,.
\]
For the sake of simplicity we loose this approximation a bit and state
\[
	\prob{\event{\errfrk}} \leq 1 -
	\left(\frac{\waccProb^{n_k}}{q^{n_k - 1}} +
		\waccProb \cdot \left(\frac{2^\secpar - n_k}{2^\secpar}\right)^{n_k} -
	\waccProb\right)\,.
\]
...
\]
Since $\ps$ is special-sound the probability that $\env$ fails in extracting the
witness is upper-bounded by some negligible $\eps_\ss$.
...
\[
	...
	\left(\frac{\waccProb^{n_k}}{q^{n_k - 1}} + \waccProb \cdot
	\left(\frac{2^\secpar - n_k}{2^\secpar}\right)^{n_k} - \waccProb\right) + \epsur + \epsss\,.
\]
Thus the probability that extractor $\extss$ succeeds is at least
\[
	\frac{\waccProb^{n_k}}{q^{n_k - 1}} + 
	\waccProb \cdot
	\left( \frac{2^\secpar - n_k}{2^\secpar}\right)^{n_k} -
...
\ncase{Step 1} First, consider a case where the commitment is limited to commit
to multiple polynomials which are evaluated at the same point $z$. As noted in
\cite[Lemma 2.2]{EPRINT:GabWilCio19} it is enough to upper bound the probability
of the adversary succeeding using the idealised verification equation---which
considers equality between polynomials---instead of the real verification
equation---which consider equality of the polynomials' evaluations. This holds
since an adversary that manages to provide a commitment opening that holds for
the real verifier, but does not hold for the idealised verifier can be used to
break the dlog assumption and reveal the secret trapdoor used to produce the
commitment's CRS.

For polynomials $\vec{f} = f_1, \ldots, f_k$, evaluation point $z$, evaluation
result $\vec{s} = s_1, \ldots, s_k$, random $\gamma$, and opening $o(X)$ the
...
\]
does not is negligible~\cite{EPRINT:GabWilCio19}. This brings the proof back to
Step 1 above. \qed
\end{proof}
...
\end{lemma}
\begin{proof}[sketch]
  Let $\adv(\REL,\crs = (\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}},
  \gtwo{\chi}))$ be an adversary trying to break the $\ur{2}$-ness of
  $\plonkprot$. It is sufficient to observe that the first 2 rounds of the
  protocol determines, along with the verifiers challenges, the rest of it. In
  Round 3 the adversary outputs a commitment to polynomial $\pt(X)$ which
  ...
  Using AGM and arguments similar to \cite{CCS:MBKM19}, we argue that $\PCOMp$
  is a commitment of knowledge. That is, a AGM adversary $\adv$ that outputs a
  commitment $c'$ which it can later open, knows a polynomial $\p{f}$ such that
  $\gone{\p{f}(\chi)} = c'$. Thus if $c' \neq c$ and the commitment scheme is
  evaluation binding then $\adv$ when picking $c'$ picks it as a commitment to
  $\p{f}$ which evaluates at $\chz$ to $\pt(\chz)$. The probability of that is
  negligible as there can only be degree of $\pt$ many overlapping points and
  ...
  if both evaluations are acceptable by the $\PCOM.\verify$ then the evaluation
  binding property of $\PCOM$ is broken. This happens with probability
...
%polynomial $\pt'(X)$
%Since the commitment is deterministic, there is
%only one correct commitment for a 
\end{proof}
\COMMENT{
\begin{lemma}
\label{lem:plonkprot_ur}
...
\end{lemma}
\begin{proof}
Let $\adv$ be an adversary that breaks $\ur{2}$-ness of
$\plonkprot$. 
...
For the first case we show that $\adv$ breaks the evaluation binding property of
$\PCOM$, while for the second case we show that it breaks the unique opening
property of $\PCOM$.
...
breaks $(1, 1, 1, \numberofconstrains + 3, 1)$-computational special soundness
...
\end{lemma}
\begin{proof}
  Let $\crs$ be $\plonkprot$'s CRS and denote by $\crs_1$ all CRS's
  $\GRP_1$-elements; that is, $\crs_1 = \gone{1, \chi, \ldots,
    \chi^{\numberofconstrains + 2}}$. Let $\adv$ be an algebraic adversary that
  for a statement $\inp$ produces a $(1, 1, 1, \numberofconstrains + 3, 1)$-tree
  ...
  witness in their coefficients, have degree $(\numberofconstrains + 2)$ and
  since $\adv$ answered honestly on $(\numberofconstrains + 3)$ different
  challenges $\chz$ then $(\numberofconstrains + 3)$ evaluations of these
  ...
  representation as a combination of the input $\GRP_1$-elements i.e.~$\gone{1,
    \chi, \ldots, \chi^{\numberofconstrains + 2}}$. Hence all coefficients of
...
\begin{lemma}
  ...
  not require a CRS trapdoor.\footnote{The simulator works as a simulator for
    ...
    the CRS simulator.} More precisely, assume that $\plonk$'s CRS simulator
...
\begin{proof}
  ...
  \ngame{0} In this game $\adv(\REL, \crs)$ picks an instance--witness pair
  $(\inp, \wit)$ and gets a real proof $\zkproof$ for it.

  \ngame{1} In this game for $\adv(\REL, \crs)$ picks an instance--witness pair
  ...
  randomisers $b_1, \ldots b_9$, sets $\wit_i$, for $i \in \range{1}{3
    \numberofconstrains}$, computes polynomials $\pa(X), \pb(X), \pc(X)$ and
  outputs $\gone{\pa(\chi), \pb(\chi), \pc(\chi)}$. Then it picks Round 1
  challenge $\beta, \gamma$ honestly.
  ...
  that this evaluation is possible only since $\simulator_\chi$ knows the
  trapdoor.
...
\ngame{2} In this game $\adv(\REL, \crs)$ picks an instance--witness pair
...
sets $\wit_i = 0$ for $i \in \range{1}{3\numberofconstrains}$. Then $\simulator$
...
variables that occur in polynomial $\pt(\vB)$ except the challenge element
$\pf(\vB) = B_1 B_3 B_5 B_7$. Variables $\vB$ are evaluated to randomly picked
$\vb = b_1, \ldots b_9$.
...
% Probability $\frkProb$ is a lower bound for a successful witness extraction
% from two transcripts. Let $\trans_1 = (\inp, a, b, z)$ and $\trans_2 = (\inp,
% a, b', z')$ be the transcripts. Both $\trans_1$ and $\trans_2$ have to be
% \emph{acceptable}, i.e.~$i > 0$ and the probability that $\zdv$ accepts a
% transcript is denoted by $\accProb$. Index $i$ can be interpreted as an index
% of $h_i$ which was sent as a challenge for $(\inp, a)$, i.e~the index of a
% random oracle query that has been used to produce prover's message in the
% point the proof branches (regarding the random oracle queries). This index has
% to be guessed by the security reduction. For the sake of extractability, both
% transcripts have to have the same index $i$, i.e.~the same instance $\inp$ and
% the first message $a$, but the actual challenges $b = h_i$ and $b' = h'_{i}$
% have to differ.
...
% \michals{22.X}{Do we need to multiply
...
%
	Let $\adv$ be an adversary that produces a $(1, 1, \multconstr + \linconstr +
  1, 1, 1)$-tree of acceptable transcripts $\tree$ for a statement $\inp$. We
  consider two disjunctive events $\event{E}$ and $\nevent{E}$. The first
  corresponds to a case when all transcripts in $\tree$ are acceptable for the
  ideal verifier, i.e.~$\vec{\vereq}(X) = \vec{0}$. In that case we show an
  extractor $\extss$ that from $\tree$ extracts a valid witness $\wit$. The
  second, corresponds to a case when $\tree$ contains a transcript that is
  acceptable by the real verifier but is not acceptable by the ideal verifier.
  In that case we show a reduction $\rdvldlog$ that uses $\adv$ to break the
  $(\dconst, \dconst)$-$\ldlog$ assumption with at least the same probability.
...
of the randomisers polynomial $\pr(X, Y)$ computed by the simulator is
indistinguishable from a polynomial provided by an honest user for a
distinguisher that only learns $\pR,\pT,\pa,\pb$ from the proof (the other proof
elements are fixed by these elements and the public information).
...
$\p{t}(X, y)$ is a polynomial that has $0$ as a coefficient next to $X^0$.
I.e.~$\pt(0, y) = 0$. As noted in \cite{CCS:MBKM19}, the coefficient next to
...
% \end{align*}
(Vectors $\vec{u_q}, \vec{v_q}, \vec{w_q}$ are $\multconstr$-elements long and
correspond to the $Q$ linear constrains of the proof system. Field element
$k_{q}$ is the instance value). When the proven instance is correct, $\pt(0,Y)$
is a zero polynomial. See \cite{CCS:MBKM19} for details. Also, when
\cref{eq:ab_eq_c} holds, that polynomial simplifies to
\begin{equation}
	\label{eq:x0_simpl}
...
i.e.~non-Laurent, polynomial. Also, $t(0,Y)$ is a polynomial of degree
$(\linconstr + \multconstr)$ with $0$ coefficients for $Y^{i}$, for $i \in
\range{0}{\multconstr}$. Also, since $\vec{a}, \vec{b}$ were picked at random,
$\pt(0,Y) / Y^{\multconstr + 1}$ is a degree-$(\linconstr - 1)$ polynomial of
random coefficients. Recall, that the view of the adversary is independent of
$\vec{a}, \vec{b}$ because of randomisers $c_{\multconstr + 1}, \ldots,
c_{\multconstr + 4}$.
...
\begin{remark} 
  ...
  subversion zero knowledge is to utilise an extractor that extracts a CRS
  trapdoor from a CRS-generator. Unfortunately, a NIZK made subversion
...
\end{remark}
	...
	\begin{lemma}
      ...
      probability that $\p{f}(X)$ has roots in $\FF_p$ is at least
      $\infrac{1}{d!}$.
	\end{lemma}
	\begin{proof}
    First observe that there is $p^{d}$ canonical polynomials in $\FF_p[X]$.
    Each of the polynomials may have up to $d$ roots. Consider polynomials which
    are reducible to polynomials of degree $1$, i.e.~polynomials that have all
    $d$ roots. The roots can be picked in $\bar{C}^{p}_{d}$ ways, where
    $\bar{C}^{n}_{k}$ is the number of $k$-elements combinations with
...
\ncase{$\plonk$ CRS generating algorithm $\kgen(\REL)$}
...
\cite{EPRINT:GabWilCio19}, can be computed using these CRS elements thus we leave
...
\end{align*}
	...
	\ncase{$\sonic$ CRS generation $\kgen(\REL)$}
...
\begin{lemma}[General forking lemma II]
	...
	Fix $q \in \ZZ$ and set $H$ of size $h \geq mq'$. Let $\zdv$ be a $\ppt$
  algorithm that on input $y, h_1, \ldots, h_q$ returns $(\vec{i}, s)$ where
  $\vec{i} = (\vec{i}[1], \ldots, \vec{i}[q']) \in
  \range{0}{q}^{q'}$ and $s$ is called a side output. Denote by $\ig$ a randomised
  instance generator. We denote by $\accProb$ the probability
	\[
      \condprob{\vec{i} \neq \vec{0}}{ y \gets \ig;\ h_1, \ldots, h_q \sample H;\ (\vec{i}, s)
		\gets \zdv(y, h_1, \ldots, h_q)}\,.
	\]
	Let $\genforking_{\zdv}^{m}$ denote the algorithm described in
  \cref{fig:genforking_lemma} then the probability $\frkProb := \condprob{b =
    ...
    \genforking_{\zdv}^{m}(y, h_1, \ldots, h_q)}$ is at least
	\[
		\frac{\accProb^m}{q^{q'(m - 1)}} - \accProb \cdot \left(1 -
    \frac{h!}{(h - mq')! \cdot h^{mq'}}\right).
	\]
	...
	\begin{figure}[t]
        ...
        \pccomment{$\vec{i} = i[1], \ldots, i[q']$}\\
    \vec{i_1} \gets \vec{i}\\
		\pcif \vec{i} = \vec{0}\ \pcreturn (0, \bot)\\
		\pcfor j \in \range{2}{m}\\
		\pcind h_{1}^{j}, \ldots, h_{i[1] - 1}^{j} \gets h_{1}^{j - 1}, \ldots,
		h_{i[1] - 1}^{j - 1}\\
		\pcind h_{i[1]}^{j}, \ldots, h_{q}^{j} \sample H\\
		\pcind (\vec{i_j}, s_j) \gets \zdv(y, h_1^{j}, \ldots, h_{q}^{j}; \rho)\\
		\pcind \pcif \vec{i_j} = 0 \lor \vec{i_j} \neq \vec{i}\ \pcreturn (0, \bot)\\
    \pcif \exists (j, j') \in \range{1}{m}^2, k \in \range{1}{q'} : (h_{i[k]}^{j} = h_{i[k]}^{j'})\
	\pcreturn (0, \bot)\\
		\pcelse \pcreturn (1, \vec{s})
	}}
	\caption{Generalised forking algorithm $\genforking_{\zdv}^{m}$}
...
First let denote by $\accProb(y)$ and $\frkProb(y)$ the following probabilities
\begin{align*}
  \accProb(y) & =  \condprob{\forall k \in \range{1}{q'} : \vec{i}[k] \neq 0}{h_1, \ldots, h_q \sample H;\ (\vec{i}, s)
                \gets \zdv(y, h_1, \ldots, h_q)}\,.\\
  \frkProb(y) & = \condprob{b = 1}{(b, \vec{s}) \gets
                \genforking_{\zdv}^{m}(y, h_1, \ldots, h_q)}\,.
\end{align*}
...
\begin{equation}\label{eq:frkProb_y_gen}
	\frkProb(y) \geq 
	\frac{\accProb(y)^m}{q^{q'(m - 1)}} - \accProb(y) \cdot \left(1 -
  \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)
	\end{equation}
Then with the expectation taken over $y \sample \ig$, we have
\begin{align}
	\frkProb & = \expected{\frkProb(y)} \geq
	\expected{\frac{\accProb(y)^m}{q^{q'(m - 1)}} -  \accProb(y) \cdot \left(1 -
  \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)} \label{eq:use_eq1_gen}\\
	& \geq \frac{\expected{\accProb(y)}^m}{q^{q'(m - 1)}} -
	\expected{\accProb(y)} \cdot \left(1 - \frac{h!}{(h - mq')! \cdot
  h^{mq'}}\right) \label{eq:by_lemma_jensen_gen}\\
	& = \frac{\accProb^m}{q^{q'(m - 1)}} -  \accProb \cdot \left(1 -
  \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)\label{eq:by_accProb_gen}\,.
\end{align}
...
\begin{align*}
  \frkProb (y) & = \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j}[k] \neq 0 \land
                 ...
                 } (j, j') \in J, k, l \in \range{1}{q'}}	\\
               & \geq \prob{\vec{i_j} = \vec{i_{j'}} \land  \vec{i_j}[k] \neq 0
                 \text{ for } (j, j') \in J,  k \in \range{1}{q'}} \\
               & \quad - \prob{\vec{i_j}[k] \neq 0 \land
                 h_{\vec{i_j}[l]}^{j} = h_{\vec{i_{j'}}[l]}^{j'} \text{ for all
                 } k \in \range{1}{q'} \text{ and for some
                 } (j, j') \in J, l \in \range{1}{q'}}\\
               & = \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j}[k] \neq 0 \text{ for } (j, j') \in
                 J,  k \in \range{1}{q'}} - \\
               & \quad  \prob{\vec{i_j}[k] \neq 0 \text{ for } k \in \range{1}{q'}} \cdot 
                 \left(1 - \frac{h!}{(h - mq')! \cdot h^{mq'}}\right) \\ 
               & = \prob{\vec{i_j} = \vec{i_{j'}} \land
                 \vec{i_j}[k] \neq 0 \text{ for
                 } (j, j') \in J,  k \in \range{1}{q'}} - \\
               & \quad \accProb(y) \cdot \left(1 -
                 \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)\,.
\end{align*}

Probability that for some $(j, j') \in J$, $k \in \range{1}{q'}$ and $\vec{i_j} = \vec{i_{j'}}$ holds
$h_{\vec{i_j}[k]}^{j} \neq h_{\vec{i_{j'}}[k]}^{j'}$, equals 
\[
  \frac{h \cdot (h - 1)
\cdot \ldots \cdot (h - mq' - 1)}{h^{mq'}} = \frac{h!}{(h - mq')! \cdot h^{mq'}}.
\]
That is, it equals the number
of all $mq'$-element strings where each element is different divided by
the number of all $mq'$-element strings, where elements are taken from a
set of size $h$. 

It remains to show that $\prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j}[k] \neq 0 \text{ for }
(j, j') \in J, k \in \range{1}{q'}} \geq \infrac{\accProb(y)^m}{q^{m - 1}}$.
Let $\RND{\zdv}$ denote the set from which $\zdv$ picks its coins at random.
For each $\vec{\iota} \in \range{1}{q}^{q'}$ let $X_{\vec{\iota}} \colon \RND{\zdv} \times
H^{\vec{\iota}[1] - 1} \to [0, 1]$ be defined by setting $X_{\vec{\iota}}(\rho, h_1, \ldots,
h_{\vec{\iota}[1] - 1})$ to
\[
	\condprob{\vec{i} = \vec{\iota}}{h_{\vec{\iota}[1]}, \ldots, h_q \sample H; (\vec{i}, s) \gets \zdv(y, h_1, \ldots, h_q; \rho)} 
\] 
for all $\rho \in \RND{\zdv}$ and $h_1, \ldots, h_{\vec{\iota}[1] - 1} \in H$. Consider $X_{\vec{\iota}}$ as a random variable over the uniform distribution on its domain. 
Then
\begin{align*}
	& \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j}[k] \neq \vec{0} \text{ for
   } (j, j') \in J, k \in \range{1}{q'}} 
	 = \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \prob{\vec{i_1} = \vec{\iota} \land \ldots \land \vec{i_m} = \vec{\iota}} \\
	& = \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \prob{\vec{i_1} = \vec{\iota}} \cdot \condprob{\vec{i_2} = \vec{\iota}}{\vec{i_1} = \vec{\iota}} \cdot \ldots \cdot \condprob{\vec{i_m} = \vec{\iota}}{\vec{i_1} = \ldots = \vec{i_{m - 1}} = \vec{\iota}} \\
	& = \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \sum_{\rho, h_1, \ldots, h_{\vec{\iota}[1] - 1}} X_{\vec{\iota}}
	(\rho, h_1, \ldots, h_{\vec{\iota}[1] - 1})^{m} \cdot \frac{1}{\abs{\RND{\zdv}} \cdot \abs{H}^{\vec{\iota}[1] - 1}}
	= \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{\iota}}^m} \,.
\end{align*}
Importantly, $\sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{\iota}}} = \accProb(y)$.

By \cref{lem:jensen} we get
\[
	\sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{\iota}}^m} \geq \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_\iota}^m\,.
\]
Note that for e.g.~$X_{\vec{i}} = 1$, $\vec{i} \in \range{1}{q}^{q'}$ the inequality becomes equality, that is, it is tight.
 
We now use the H\"older inequality, cf.~\cref{lem:holder}, for $x_{\vec{i}} = \expected{X_{\vec{i}}}$, $y_{\vec{i}} = 1$, $p = m$, and $q = m/(m - 1)$ obtaining
\begin{gather}
	\left(\sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{i}}}\right)^{m}  \leq \left(\sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{i}}}^m\right) \cdot q^{q'(m - 1)}\\
	\frac{1}{q^{q'(m - 1)}} \cdot \accProb(y)^{m} \leq \sum_{\vec{\iota} \in \range{1}{q}^{q'}} \expected{X_{\vec{i}}}^m\,.
\end{gather}
Finally, we get
\[
	\frkProb(y) \geq \frac{\accProb(y)^m}{q^{q'(m - 1)}} - 
	 \accProb(y) \cdot \left(1 - \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)\,.
\]
\qed
\end{proof}
...
accepting transcripts, where $q'$ elements $n_j$, $j \in \range{1}{\mu}$, are
...
to denote probability of $\zdv$ outputing a non-zero $\vec{i}$ and probability
of $\advse$ outputing an acceptable proof we claim that these probabilities are
exactly the same what comes from how we define $\zdv$.)  Let $\advse$ produce an
acceptable proof $\zkproof_{\advse}$ for instance $\inp_{\advse}$; $r$ be
$\advse$'s randomness; $Q$ the list of queries submitted by $\advse$ along with
simulator's $\simulator_\zkproof$ answers; and $Q_\ro$ be the list of all random
oracle queries made by $\advse$.  All of these are given to the extractor $\ext$
that internally runs the forking algorithm $\genforking_\zdv$.  Algorithm $\zdv$
takes $(\REL, \crs, \advse,
%\inp_\advse,
%\zkproof_\advse, 
Q, r)$ as input $y$ and $Q_\ro$ as input $h_1^1, \ldots,
h_q^1$. 
(For the sake of completeness, we allow $\genforking_\zdv$ to
pick $h^1_{k + 1}, \ldots, h^1_q$ responses if $Q_\ro$ has only $k < q$
elements.)  

Next, $\zdv$ runs
internally $\advse(\REL, \crs; r)$ and responds to its random oracle and
simulator queries by using $Q_\ro$ and $Q$. Note that $\advse$ makes the
same queries as it did before it output $(\inp_{\advse}, \zkproof_{\advse})$ as it
is run on the same random tape and with the same answers from the simulator and
random oracle. After $\advse$ finishes its acceptable proof $\zkproof_{\advse}$,
algorithm $\zdv$ outputs $(\vec{i}, \zkproof_{\advse})$, where $\vec{i}$ is the
vector of indices of a random
oracle query required to get the challenge after $k$-th message from the prover,
for $k \in B$.
%We also note that we differ here from the description of the
%$\genforking_\zdv$ algorithm provided
%e.g.~in~\cite{INDOCRYPT:FKMV12,CCS:BelNev06} where the algorithm also picks the first set of random oracle
...
%random as well. 
Then, after the first run of $\advse$ is done, the extractor runs $\zdv$ again,
but this time it provides a fresh random oracle responses $h^2_{\vec{i}[1]}, \ldots,
h^2_q$. Note that this is equivalent to rewinding $\advse$ to a point just
before $\advse$ is about to ask its $\vec{i}[1]$-th random oracle
query. (We rewind to a point where the tree of accepting transcripts branches
for the first time.) Probability that the adversary produces an acceptable transcript with the
fresh random oracle responses is at least $\accProb$. This continues until the
required number of transcripts is obtained. 
...
We also note that the general forking lemma proposed in
\cref{lem:generalised_forking_lemma} is more general than we actually need to
...
$(1, \ldots, 1, n_k, 1, \ldots, 1)$-tree of acceptable transcripts, i.e.~for $q'
= 1$. Setting $q' = 1$ also considerably improves the probability bounds for
$\frkProb$, which is now
\[
...
sound and $q'$ elements from $\smallset{n_1, \ldots, n_\mu}$ are not $1$.
...
\]
for $N = \prod_{k = 1}^{\mu} n_k$ and some negligible $\eps$.	
\end{theorem}
\begin{proof}		
  The proof goes by game hoping. The games are controlled by an environment
  $\env$ that internally runs a simulation extractability adversary $\advse$,
  ...
  \ngame{1} This is identical to $\game{0}$ except that now the game is aborted
  if there is a simulated proof $\zkproof_\simulator$ for $\inp_{\advse}$ such
  that $(\inp_{\advse}, \zkproof_\simulator\range{1}{k + 1}) = (\inp_{\advse},
  \zkproof_{\advse}\range{1}{k + 1})$. That is, the adversary in its final proof
  reuses a part of a simulated proof it saw before and the proof is acceptable.
  Denote that event by $\event{\errur}$.
...
\michals{15.09}{As MK noted, this proof holds only for simulation soundness. For extractability we use special soundness (cf the next section)}
	...
	\begin{theorem}[Simulation soundness]
		...
		Assume that $(\numberofconstrains + 2, 1)$-$\dlog$ is
    $\eps_\dlog(\secpar)$-hard, $\plonkprot$ is sound and $\ur{2}$ with security
    $\epss(\secpar)$ and $\epsur(\secpar)$ respectively. Then the probability
    that an algebraic, $\ppt$ adversary $\advss$ breaks simulation soundness of
	...
	\begin{proof}
		...
		\eps := \Pr
			...
			\end{aligned}}
			\begin{aligned}
				& \crs \gets \plonkprotfs.\kgen(\REL, \secparam)\\
  ...
  $\rdvs$ and $\rdvdlog$.
	...
	We show how $\rdvur$ utilizes $\advss$, that makes use of $\inp_\advss,
	\zkproof_{\advss}\range{1}{3}$, to break the $\ur{2}$ property of $\plonkprot$. 
	This way we bound the probability $\prob{\adv \text{ wins}, \event{E}}$ by
	the probability of $\rdvur$ being able to win in the $\ur{2}$ game.
  ...
  \item When $\advss$ outputs a fake proof $\zkproof_{\advss}$ for
    ...
    $\zkproof_{\simulator}\range{1}{3}$ such that $\zkproof_{\advss}\range{1}{2}
    = \zkproof_{\simulator}\range{1}{3}$ and a random oracle query
    $\zkproof_{\simulator}[3].\ch$ on $\zkproof_{\simulator}\range{1}{3}$.
		...
		On the other hand, if the proofs are not equal, then $\rdvur$ breaks
		$\ur{2}$-ness of $\plonkprot$. Thus 
		\[
			\prob{\advss \text{ wins}, \event{E}} \leq \epsur(\secpar).
		\]

    \ncase{When $\nevent{E}_0$ happens} In this case the reduction $\rdvs$ uses
    $\advss$ to break soundness of $\plonkprot$ with probability $\epss /
    q_{\ro}^6$, where $q_\ro$ is the number of total random oracle queries
    performed by the adversary or by $\rdvs$ on behalf of the simulator. As
    previously, $\rdvs$ runs $\advss$ internally and simulates its environment
    by answering to its queries to $\plonkprotfs.\simulator$ and $\ro$. The
  ...
  \item On input $h$ for the $i$-th, $i \not\in \smallset{{i_1}, \ldots,
      {i_6}}$, random oracle query, $\rdvs$ returns randomly picked $ y$, sets
    $\ro(h) = y $ and stores $(h, y)$ in $Q_\ro$ if $h$ is sent to $\ro$ the
    first time. If that is not the case, $\rdv$ finds $h$ in $Q_\ro$ and returns
    the corresponding $y$.
  \item On input $h_{i_j}$ for the $i_j$-th, $i_j \in \smallset{{i_1}, \ldots,
      {i_6}}$, random oracle query, $\rdvs$ parses $h_{i_j}$ as a partial proof
    transcript $\zkproof[1..j]$ and runs $\plonkprot$ using $\zkproof[j]$ as a
    $\plonkprot.\prover$'s $j$-th message to $\plonkprot.\verifier$. The
    verifier responds with a challenge $\zkproof[j].\ch$. The reduction sets
    $\ro(h_{i_j}) = \zkproof[j].\ch$.
  \item On query $\inp_\simulator$ to $\simulator$ it runs a simulator
    ...
    the random oracle query with input $\zkproof_\simulator[j]$, $1 \leq j \leq
    2$, of the simulator is the $i_j$-th query, generate
    $\zkproof_\simulator[j].\ch$ by invoking $\plonkprot.\verifier$ on
    $\zkproof_\simulator[j]$ and programming $\ro(h_{i_j}) =
    \zkproof_\simulator[j].\ch$.
  ...
  protocol $\plonkprotfs$, which CRS is computed from the challenge $\gone{1,
    \chi, \ldots, \chi^{\numberofconstrains + 2}}, \gtwo{\chi}$ from the
  $(\numberofconstrains + 2, 1)$-$\dlog$ assumption challenger. Then it proceeds
