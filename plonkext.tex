...
\end{description}
	...
	\section{More generalised forking lemma}
    \subsection{Generalised forking lemma.}
First of all, although dubbed ``general'', \cref{lem:forking_lemma} is not
general enough for our purpose as it is useful only for protocols where witness
can be extracted from just two transcripts. To be able to extract a witness
...
First let denote by $\accProb(y)$ and $\frkProb(y)$ the following probabilities
\begin{align*}
\accProb(y) & =  \condprob{\vec{i} \neq \vec{0}}{h_1, \ldots, h_q \sample H;\ (\vec{i}, s)
\gets \zdv(y, h_1, \ldots, h_q)}\,.\\
	\frkProb(y) & = \condprob{b = 1}{(b, \vec{s}) \gets
\genforking_{\zdv}^{m}(y, h_1, \ldots, h_q)}\,.
\end{align*}
...
$\genforking_{\zdv}^{m}$ we have
\begin{align*}
  \frkProb (y) & = \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j} \neq \vec{0} \land
                 h_{\vec{i_j}[1]}^{j} \neq h_{\vec{i_{j'}}[1]}^{j'}, \ldots, h_{\vec{i_j}[q']}^{j} \neq h_{\vec{i_{j'}}[q']}^{j'} \text{ for } (j, j') \in J}	\\
               & \geq \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j} \neq \vec{0} \text{ for } (j, j') \in J} \\
               & \qquad - \prob{\vec{i_j} \neq \vec{0} \land
                 h_{\vec{i_j}[k]}^{j} = h_{\vec{i_{j'}}[k]}^{j'} \text{ for some
                 } (j, j') \in J, k \in \range{1}{q'}}\\
               & = \prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j} \neq \vec{0} \text{ for } (j, j') \in J} -
                 \prob{\vec{i_j} \neq 0} \cdot 
                 \left(1 - \frac{h!}{(h - mq')! \cdot h^{mq'}}\right) \\ 
               & = \prob{\vec{i_j} = \vec{i_{j'}} \land
                 \vec{i_j} \geq 1 \text{ for } (j, j') \in J} - \accProb(y) \cdot \left(1 -
                 \frac{h!}{(h - mq')! \cdot h^{mq'}}\right)\,.
\end{align*}

Probability that for some $(j, j') \in J$ and $\vec{i_j} = \vec{i_{j'}}$ holds
$h_{\vec{i_j}[k]}^{j} \neq h_{\vec{i_{j'}}[k]}^{j'}$, for $k \in \range{1}{q'}$ equals 
\[
  \frac{h \cdot (h - 1)
...
It remains to show that $\prob{\vec{i_j} = \vec{i_{j'}} \land \vec{i_j} \neq 0 \text{ for }
(j, j') \in J} \geq \infrac{\accProb(y)^m}{q^{m - 1}}$.
Let $\RND{\zdv}$ denote the set from which $\zdv$ picks its coins at random.
