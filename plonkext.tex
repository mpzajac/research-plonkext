...
\begin{proof}
	...
	\[
		\abs{\prob{\game{1}} - \prob{\game{2}}} \leq \epszk.
	\]
...
B_3 B_5 B_7$.
Variables $B_1, \ldots B_9$ are evaluated to randomly picked $b_1, \ldots
b_9$.

The reduction $\rdv$ learns $\gone{\pR}$ and challenge $\gone{w}$
where $w$ is either $b_1 b_3 b_4 b_7$ or a random value. Then it picks $\chi$,
$\chz$,
...
\end{description}
Is the adversary $\adv$'s output distribution differ in Game $\game{2}$ and
$\game{3}$ then the reduction uses it to distinguish between $w = b_1 b_3
b_5 b_7$ and $w$ being random, thus
\[	
	\abs{\prob{\game{2}} - \prob{\game{3}}} \leq \epsuber.
\]
Eventually 
\[
	\abs{\prob{\game{1}} - \prob{\game{3}}} \leq \epszk + \epsuber.
\]
\qed
\end{proof}
