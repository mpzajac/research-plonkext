...
proof systems,
e.g.~\cite{AC:Groth10a,TCC:Lipmaa12,EC:GGPR13,SP:PHGR13,AC:Lipmaa13,AC:DFGK14,EC:Groth16}
to name a few. Special attention has been devoted to design new
\emph{zero-knowledge succinct arguments of knowledge} (zkSNARKs) which are
...
e.g.~\hl{cite zerocash, zexe, celo}. This is turn risen a question---\emph{Are
 zkSNARKs' security models useful in the real life?} This question is
especially valid when we realise that all zkSNARKs that have constant-sized
proofs are shown secure in the common reference string (CRS) model where the
CRS used by the parties, i.e.~a prover that shows veracity of a statement
using his private input called \emph{witness} and
a verifier that verifies it, comes with a trapdoor which can be used to
convince the verifier to accept a false statement or to accept a statement
which the prover does not know the witness for. Hence crucial is a
question---How can the parties be sure that the trapdoor has not leaked? Who
is the party that generates the CRS? Can this party be trusted? The security
model requires that as otherwise that CRS-generating party may leak the
trapdoor. This questions are especially important in zero knowledge proofs
used in distributed systems, like e.g.~blockchains, where assumption that a
trusted party generates the CRS may be a step too far and subvert the
whole purpose of a decentralised system. 

First who tackled this question were Bellare et al.~\cite{AC:BelFucSca16} who
...
system remain sound in the same situation. Importantly, Bellare et al.~shown
that no system can be mutually subversion zero-knowledge and subversion-sound.
...
relation-dependent. That is, if one wants to show that two arithmetic circuits
have been evaluated correctly, it has to do that using two different CRS-s. We
say that such a zkSNARK is not universal. Since CRS generation is a
troublesome process it is desired to have it been performed once, not every
time a new circuit validity has to be shown. One of the first universal
zkSNARK was proposed by Groth et al.~\cite{C:GKMMM18}. This particular proof
system also introduced a new security property, i.e.~updatable soundness.
Since \cite{AC:BelFucSca16} impossibility result holds, one cannot wish for a
...
zkSNARK provers to \emph{update} the CRS, i.e.~take an old CRS and modify it
in a well-defined way to obtain a new CRS. If at least one of the CRS-updating
parties  is honest then the CRS assures soundness. Furthermore, the system
proposed is subversion zero-knowledge. Although inefficient, as the CRS length
is quadratic to the size of the proven statements, \cite{C:GKMMM18} sets a new
paradigm for designing zkSNARKs.

The first zkSNARK that had updatable and short CRS and was universal was
$\sonic$ proposed by Maller et al.~in \cite{CCS:MBKM19} (the scheme was later
improved by Gabizon in \cite{EPRINT:Gabizon19c}) 

\cite{EC:CHMMVW20} \cite{EPRINT:GabWilCio19} 
...
In the context of zkSNARKs it is important to mention such works the first
simulation-extractable zkSNARK by Groth and Maller \cite{C:GroMal17} and SE
zkSNARK for QAP by Lipmaa \cite{EPRINT:Lipmaa19a}. Kosba's et
...
succinctness of the proof system is not preserved as the witness is encrypted.
...
randomisable.  \markulf{03.11.2020}{Cite weakSE e-print
...
\begin{align}
	\label{eq:uber}
	\pR(\vB) & = \smallset{B_i \mid i \in A,\ A \in D} \cup \smallset{B_i B_j \mid i \in
A, j \in B,\ A \neq B,\ A, B \in D} \cup \\ 
			& \smallset{B_i B_j B_k \mid i \in A,\ j \in
B,\ k \in C,\ A \neq B \neq C \neq A,\ A, B, C \in D} \setminus \smallset{B_1
B_3 B_5 B_7}\,,\notag \\
	\pS(\vB) & = \emptyset \notag\,, \\
	\pT(\vB) & = \emptyset \notag\,, \\
...
where $w$ is either $\pf(\vb) = b_1 b_3 b_5 b_7$ or a random value. Then it picks $\chi$,
$\chz$,
computes the CRS $\crs$ from $\chi$ and sets
\[
	\gone{\tb_i} =  (\chi - \chz)(\chi - \ochz) \gone{b_i} + \xi_i (\chi - \chz)
	\gone{1} + \zeta_i (\chi - \ochz) \gone{1}, % \text{ for } i \in
%	\range{1}{9}, u_1 \sample \FF_p
\]
for $i \in \range{1}{9}$ and $\xi_i, \zeta_i \sample \FF_p$. 
Then it computes all $\gone{\tb_i \tb_j}, \gone{\tb_i \tb_j \tb_k}$ such that
$\gone{B_i B_j, B_i B_j B_k} \in \pR$
and sets $\gone{\tw} = \gone{\tb_1 \tb_3 \tb_5 \tb_7}$, where $\gone{b_1
b_3 b_5 b_7}$ is substituted by $\gone{w}$.
...
\begin{description}
	...
	\item[Round 3] The reduction computes $\gone{\pt(\chi)}$ using $\tw$ as it
		was equal $\tb_1 \tb_3 \tb_5 \tb_7$. That is, if $w = b_1 b_3 b_5 b_7$ then
		...
		as $\simulator$ would compute. The reduction computes and outputs
		$\gone{\ptlo(\chi), \ptmid(\chi), \pthi(\chi)}$ such that $\pt(X) =
...
\end{description}
Is the adversary $\adv$'s output distribution differ in Game $\game{1}$ and
$\game{2}$ then the reduction uses it to distinguish between $w = b_1 b_3
b_5 b_7$ and $w$ being random, thus
\[	
	\abs{\prob{\game{1}} - \prob{\game{2}}} \leq \epsuber.
\]
Eventually, 
\[
	\abs{\prob{\game{0}} - \prob{\game{2}}} \leq \epszk + \epsuber.
\]
\qed
\end{proof}
...
\end{equation}
for public $\vec{\p{u}}(Y), \vec{\p{v}}(Y), \vec{\p{w}}(Y), \p{k}(Y)$ defined
as in \cref{sec:sonic_constraint_system}.
% \begin{align*}
...
found and are random $\FF_p$ elements. Hence, the challenge $y$ picked by the simulator
