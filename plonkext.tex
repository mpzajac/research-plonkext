...
The proof goes by contradiction. Belowe we show some intuitions behind it.
Let $\advse$ be an adversary that breaks simulation extractability of $\plonkprotfs$ with some non-negligible probability $\eps$. 
We show a reduction $\rdv$ which uses $\advse$ to either break knowledge soundness of $\plonkprot$ or to break $(\numberofconstrains + 2)$-dlog assumption.

The proof idea is following. First the reduction takes the CRS from a challenger $\cdv_\plonkprot$ or $\cdv_\dlog$, where the former represents game where $\rdv$ tries to break soundness of $\plonkprot$ and the latter game of breaking the dlog assumption. 
...
\begin{proof}[Proof draft]
	...
	The reduction $\reduction$ starts with obtaining $\crs$ from $\plonkprot.\kgen$ and passing it to $\advse$. 
	Assume that 
	\begin{itemize}
