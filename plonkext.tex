...
\begin{itemize}
  ...
  \item $\prover_\fs$ behaves as $\prover$ except after sending message
    $a_i$, $i \in \range{1}{\mu - 1}$, the prover does not wait for
...
has \emph{unique responses after $i$}, and call it an $\ur{i}$-protocol, if it
...
that are the same on  first $i + 1$ messages. 
...
\item When $\advse$ makes a fake proof $\zkproof_{\advse}$ for $\inp_{\advse}$,
  ...
  \epsur\,. \)
    ...
    aborts also when it fails to build a $(1, \ldots, 1, n_k, 1, \ldots,
