...
\begin{description}
	\item[Round 1] 
	Sample $b_1, \ldots, b_9 \sample \FF_\pp$; compute $\p{a}(X), \p{b}(X), \p{c}(X)$ as 
	\begin{align*}
	...
	\item[Round 2]
	Get challenges $\beta, \gamma \in \FF_\pp$
	\[
	...
	\item[Round 4]
	Get the challenge $\chz \in \FF_\pp$, $\chz = \ro(\trans)$.
	...
	% \begin{align*}
	% 	\p{r}(X) & = \p{a}(\chz) \p{b}(\chz) \selmulti(X) + \p{a}(\chz) \selleft(X) + \p{b}(\chz) \selright(X) + \p{c}(\chz) \seloutput(X) + \selconst(X) \\
	% 	& (\p{a}(\chz) + \beta \chz + \gamma) (\p{b}(\chz) + \beta k_1 \chz + \gamma) (\p{c}(\chz) + \beta k_2 \chz + \gamma) \p{z}(X) \alpha \\
	% 	& - (\p{a}(\chz) + \beta \p{S_{\sigma 1}}(\chz) + \gamma) (\p{b}(\chz) + \beta \p{S_{\sigma 2}}(\chz) + \gamma)\beta \p{z}(\chz \omega) \p{S_{\sigma 3}}(X) \alpha \\
	% 	& + \p{z}(X) \lag_1 (X) \alpha^2\,.
	% \end{align*}
	\[
		\p{r}(X) = 
		\begin{aligned}
	...
	\item[Round 5]
	Compute the opening challenge $v \in \FF_\pp$, $v = \ro(\trans)$.
