...
gates using a single CRS. However, to make computation efficient, for each
circuit there is allowed a preprocessing phase which extend the CRS with
circuit-related polynomial evaluations.

For the sake of simplicity of security reductions presented in this paper, we
include in the CRS only these elements that cannot be computed without knowing
the secret trapdoor $\chi$. The rest of the CRS---the preprocessed input---can
be computed using these CRS elements thus we leave them to be computed by the
...
\begin{align*}
  \p{q_L}(X) & = \vql_i \lag_i(X), \\
  \p{q_R}(X) & = \vqr_i \lag_i(X), \\
  \p{q_M}(X) & = \vqm_i \lag_i(X), \\
  \p{q_O}(X) & = \vqo_i \lag_i(X), \\
  \p{q_C}(X) & = \vqc_i \lag_i(X).
\end{align*}
...
\begin{description}
	...
	\item[Step 4] Compute challenges $\beta, \gamma, \alpha, \alpha', \chz, v,
