...
\begin{theorem}
	If $(numberofconstrains + 2$-dlog assumption holds then $\plonkprotfs$ is simulation-extractable against algebraic adversaries.
\end{theorem}
\begin{proof}[Proof draft]
	The proof goes by contradiction.
	Let $\advse$ be an adversary that breaks simulation extractability of $\plonkprotfs$. We show a reduction $\rdv$ that breaks knowledge soundness of $\plonkprot$.
	...
	\end{itemize}
	The reduction guesses query $i_\simulator$ and $r$ with probability $1/(3 q_\simulator)$. Assume its guess is correct.
	
	For $\advse$'s queries for a simulated proof, except query $i_\simulator$ if $r > 0$, the reduction provides the adversary the required proofs. 
	...
	\begin{itemize}
		...
		\item If $r = 2$, $\rdv$ continues talking with the verifier and gets in Round 3 challenge $\alpha$. After that the reduction pauses its interaction with the verifier and continues its simulated proof playing $\simulator$ for the rest of the rounds.
	\end{itemize}
	When $\advse$ decides to continue generating its malicious proof the reduction resumes its communication with $\plonkprot.\verifier$. That is, for each $\advse$ message it responds with the corresponding challenge from the verifier. In the end, the adversary returns a proof $\zkproof$. We assume that $\plonkprot.\verifier$ accepts the proof $\zkproof$.
	...
	\end{align*}
	Since $\advse$ is algebraic, each of the group elements it output is accompanied with coefficients for each of group elements that are input to $\advse$. 
