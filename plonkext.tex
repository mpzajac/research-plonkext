...
indistinguishable from a real proof. To that end we show a reduction to the
BBG uber assumption as presented in \cref{def:uber_assumption}. That is, we
show that if the adversary was able to distinguish the proof, then it could be
...
$\pR, \pS, \pT \in \FF_p[B_1, \ldots, B_9]$, where
\begin{align*}
	\pR & = (\pa(\vB), \pb(\vB), \pc(\vB), \pz(\vB), \crs_1) 
\end{align*}
for $\vB = (B_1, \ldots, B_9)$ and all polynomials expressed as polynomials in
the randomisers, with $\chi$ being just a constant, e.g.~$\pa(\vB) = (B_1 \chi
+ B_2)\pZH + \sum_{i = 1}^{\numberofconstrains} \wit_i \lag_i (\chi)$.
