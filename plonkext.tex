...
\begin{proof}
  ...
  \begin{itemize}
  \item Polynomials $f^i$ sent by the prover;
  \item Challenges $\beta_i$ sent by the verifier;
  \item Evaluation challenge $\chz$ sent by the verifier;
  \item Polynomial evaluations at $\chz$ and proofs of correct evaluations sent by the prover.
  \end{itemize}
  In the following we denote elements sent w.r.t.~SRS $\srs$ by $f_i$,
  $\beta_i$, and $\chz$. Openings are denoted by $W$. For SRS $\srs'$
  we denote polynomials by $f'_i$, challenges by $\beta'_i$ and
  $\chz'$ and openings by $W'$.
  ...
  \item On $\bdv$ request to see simulated proof for
    $(\srs', \inp, \wit)$, $\adv$:
    \begin{enumerate}
    \item //Translate input
    \item //Translate proof
    \end{enumerate}
  \item On $\bdv$'s proof $(\srs', \inp', \zkproof')$, $\adv$
    \begin{enumerate}
    \item 
    \end{enumerate}
  \end{itemize}
  
  $\bdv$ proceeds as follows. For every commitment $c$ output by  
\end{proof}
\end{document}
