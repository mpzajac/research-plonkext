...
\end{lemma}
\begin{proof}
	We assume that the contrary is true and analyse possible cases depending on
  ...
  the prover. \markulf{27.08}{Unless he commits to two different polynomials?}
	\michals{27.08}{That's right. Point is that the quotient polynomial is a polynomial that assures that all constrains are fulfilled, thus if the adversary commit to a different polynomial, then some constrains won't be met.}
  ...
  proof system is not sound. \markulf{27.08}{This seems to rely on the fact that for a
    given statement (and the first two initial rounds), there is only a single
    quotient polynomial.} 
		\michals{27.08}{That's right}
	\case{2}
  ...
  it should be rejected of the verifier of the proof system.\markulf{28.08}{This
  argument seems to rely also on the binding property of the commitment scheme,
  or at least the knowledge soundness of the proof system.}
	\michals{27.08}{Yes, I think this could be called the binding property---the committer shouldn't be able to open the commitment to an incorrect value.}
	\case{3} 
  ...
  by the verifier if the proof system be sound. \markulf{27.08}{Given the commitment
    and the value, the opening is fixed?}
		\michals{27.08}{In the standard commitment it is. However batch opening gives some freedom -- i.e. the opening may be incorrect, but if a good batch coefficient is picked, then it may be accepted anyway. This happens with negligible probability only.}
	\qed
\end{proof}
...
\begin{proof}
  ...
  to provide a simulated proof.\markulf{27.08}{That's consistent with what I
  ...
  \markulf{27.08}{I struggle to understand how this would work when simulated RO
    queries are reused.}
		\michals{27.08}{Wouldn't including queries made by the simulator here solve the problem?}
