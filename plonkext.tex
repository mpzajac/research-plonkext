...
\begin{theorem}
Let $\PS$ be an AHP proof system instantiated using KZG commitment scheme. Let $\bdv$ be an adversary who can program random oracle. Then $\bdv$ can move proofs from SRS $\srs$ to $\srs'$ if it knows all partial trapdoors between them.
\end{theorem}
\begin{proof}
  Let $\srs$ be an SRS created by using trapdoor $\chi$ and let
  $\srs'$ be created using trapdoor $\alpha \chi$. Although $\chi$ may
  not be known to $\bdv$, $\alpha$ is known. Note that a zkSNARK
  compiled from a AHP consists of the following:
  \begin{itemize}
  ...
  \end{itemize}
  In the following we denote polynomials sent w.r.t.~SRS $\srs$ by
  $f_i$, challenges by $\beta_i$ and evaluation challenge by
  $\chz$. Openings are denoted by $W$. For SRS $\srs'$ we denote
  polynomials by $f'_i$, challenges by $\beta'_i$ and $\chz'$ and
  openings by $W'$.
  
  $\bdv$ moves proofs as follows. For every commitent $c$ output by  
\end{proof}
\end{document}
