...
%
  ...
  % $\REL(\inp, \wit) = 1$ with probability at least $1 - \epsss$.
  Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
  $(2 \mu + 1)$-message proof system for a relation $\REL$. Let $\tree$ be a PPT algorithm that rewinds $\adv^{H,Sim}$ to produce a
  $(1, n_k, 1)$-tree of transcripts such that none of the challenges in round $k$ were used in simulated proofs. We say that $\ps$ is $(\epsss, ((1, n_k, 1)))$-forking special
  sound if there is an extractor $\extt$ that given a tree produced by $\tree$ extracts $\wit$
  such that $\REL(\inp, \wit) = 1$ with probability at least $1 - \epsss.$
\end{definition}
