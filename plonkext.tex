...
\markulf{03.11.2020}{Better to introduce abbreviations such as NIZK and SNARK,
  UC ... even if they seem obvious to us.}
...
extractable NIZK is universally-composable (UC) secure
...
Recently, \cite{EPRINT:BKSV20} shown that the original Groth's proof
system from \cite{EC:Groth16} is weakly SE and randomisable.  
\markulf{03.11.2020}{Cite weakSE e-print
paper?}\michals{4.11.20}{Done}
...
\begin{definition}[$(\pR, \pS, \pT, \p{f})$-original uber assumption]
...
\begin{lemma}[General forking lemma II]
	...
	\]
	Let $\genforking_{\adv}^{m}$ \markulf{03.11.2020}{Add a $m$ parameter as superscript
  to $\genforking$?} 
	\michals{4.11}{Done}
...
\begin{lemma}
	$\plonk$ is honest verifier zero-knowledge and its simulator does not
...
\emph{requires} the CRS simulator.}	
\end{lemma}
...
\begin{proof}
	...
	\ngame{1} In this game $\adv(\REL, \crs)$ picks an instance--witness
	pair $(\inp, \wit)$ and gets a real proof $\zkproof$ for it.
	\ngame{2} In this game for $\adv(\REL, \crs)$ picks an instance--witness
	...
	proceeds as follows.
	\michals{7.11}{Put a description of the simulator here}
	...
	different bit in both games is negligible.

	\ngame{3} In this game $\adv(\REL, \crs)$ picks an
...
\ncase{$\game{2} \mapsto \game{3}$}
% We now argue that for any $\ppt$ adversary $\adv$ a proof provided by the simulator
% is indistinguishable from a real proof. This is done by the game-hop argument.
% That is, in the first game the adversary provides $(\inp, \wit) \in \REL$ and
% gets a real proof. In Game 2, it gets the proof simulated by a
% simulator $\simulator_\chi$ which, contrary to the simulator described here,
% is given the trapdoor $\chi$. Next, we show another game, Game 3, where the
% adversary is given proof simulated by the simulator $\simulator$ as described
% above. We show a reduction $\rdv$ which assures that the probability of the
% adversary winning in Game 2 and Game 3 differs only negligibly. More
% precisely, we show that any adversary $\adv$ that is able to distinguish these
% two games with non-negligible probability can be used to break \hl{the ... uber assumption}.
%
We now describe the reduction $\rdv$.
Let $D = \smallset{\smallset{1, 2}, \smallset{3, 4}, \smallset{5, 6},
\smallset{7, 8, 9}}$.
Let 
\begin{multline*}
	\pR = \smallset{b_i \mid i \in A,\ A \in D} \cup \smallset{b_i b_j \mid i \in
A, j \in B,\ A \neq B,\ A, B \in D} \cup \\ 
\smallset{b_i b_j b_k \mid i \in A,\ j \in
B,\ k \in C,\ A \neq B \neq C \neq A,\ A, B, C \in D} \setminus \smallset{b_1 b_3 b_5 b_7}.
\end{multline*}
%\cup \smallset{b_i b_j} \cup
%\smallset{b_{i'} b_{j'} b_{k'}}$,
%_{(i, j) \in
%\smallset{1, 2} \times \smallset{3, 4} \cup } b_1 b_3, b_1 b_4, b_1 b_5, b_1 b_6,
%	b_1 b_7 b_2 b_3, b_2 b_4,
%b_1 b_}$ \hl{continue}.
%where $(i, j)$ 
That is, the elements of $\pR$ are all singletons, pairs and triplets of $b_i$
elements that occur in polynomial $\pt$ except the challenge element $b_1
b_3 b_5 b_7$.
Furthermore we set $\pS = \pT = \emptyset$.

The reduction $\rdv$ learns $\gone{\pR}$ and challenge $\gone{w}$
where $w$ is either $b_1 b_3 b_4 b_7$ or a random value. Then it picks $\chi$, $\chz$,
computes the CRS $\crs$ from $\chi$ and sets
\[
	\gone{\tb_i} =  (\chi - \chz)(\chi - \ochz) \gone{b_i} + \xi_i (\chi - \chz)
	\gone{1} + \zeta (\chi - \ochz) \gone{1}, % \text{ for } i \in
%	\range{1}{9}, u_1 \sample \FF_p
\]
for $i \in \range{1}{9}$ and $\xi_i, \zeta_i \sample \FF_p$. Then it sets
$\gone{\tw}
= \gone{\tb_1 \tb_3 \tb_5 \tb_7}$, where $\gone{b_1 b_3 b_5 b_7}$ is
substituted by $\gone{w}$.
Then it computes all $\gone{\tb_i \tb_j}, \gone{\tb_i \tb_j \tb_k} \in \pR$.
...
\begin{description}
	\item[Round 1] $\rdv$ computes $\gone{\pa(\chi)}$ using as randomisers
		$\gone{\tb_1}, \gone{\tb_2}$ and setting $\wit_i = 0$, for $i \in
		\range{1}{3 \numberofconstrains}$. Similarly it computes $\gone{\pb(\chi)},
		\gone{\pc(\chi)}$.
		$\rdv$ publishes the obtained values and picks a Round 1 challenge $\beta,
		\gamma$ at random.
	\item[Round 2] $\rdv$ computes $\gone{\pz(\chi)}$ using $\tb_7, \tb_8,
		\tb_9$ and publishes it. Then it picks randomly the challenge $\alpha$.
	\item[Round 3] The reduction computes $\gone{\pt(\chi)}$ using $\tw$ as it
		was equal $\tb_A \tb_B \tb_C$. That is, if $w = b_A b_B b_C$ then
		$\pt(\chi)$ is as computed by the simulator $\simulator_\chi$, otherwise,
		if $w$ is random then $\pt(\chi)$ is random as well, thus it is computed
		as $\simulator$ would compute.\michals{7.11}{How to divide $\pt$ into
		$\pt_{low}, \pt_{mid}, \pt_{hi}$.?}
		Eventually, $\rdv$ outputs $\chz$.
	\item[Round 4] The reduction outputs $\pa(\chz), \pb(\chz), \pc(\chz),
		\p{S_{\sigma 1}}(\chz), \p{S_{\sigma 2} (\chz)}, \pt(\chz), \pz(\ochz)$.
		For the sake of concreteness, denote by $S = \smallset{\pa, \pb, \pc,
		\pt, \pz}$. Although for a polynomial $\p{p} \in S$, reduction $\rdv$ does not
		know $\p{p}(\chi)$ or even do not know all the coefficients of $\p{p}$,
		the polynomials in $S$ was computed such that the reduction always knows
		their evaluation at $\chz$ and $\ochz$.
...
\comment{
Now we argue why for a $\ppt$ adversary a proof provided by the simulator is
indistinguishable from a real proof. We do that indirectly and rely on
$\plonk$'s simulator that \emph{knows} the CRS trapdoor $\chi$. That is, we
show that a proof provided by our simulator $\simulator$ is
indistinguishable from a proof provided by $\simulator_\chi$,
cf.~\cref{sec:plonk_simulator}.
To that end we show a reduction to the
BBG uber assumption as presented in \cref{def:uber_assumption}. That is, we
show that if the adversary was able to distinguish the proofs, then it could be
used to break the assumption.

% \michals{6.11}{Need to take care of the fact that the distinguisher should
% know evaluations of the polynomials in some points}
Let $\cdv$ be the $(\pR, \pS, \pT, \p{f})$-uber assumption challenger for
$\pR, \pS, \pT \in \FF_p[\vec{B}]$, for $\vec{B} = (B_1, \ldots, B_9, A, B, \Gamma, \Delta, V)$, and
\begin{align*}
	\pR & = \smallset{\pta(\vB), \ptb(\vB), \ptc(\vB), \ptz(\vB), \crs_1,
	\pta_\chz(\vB), \ptb_\chz(\vB), \ptc_\chz(\vB), \ptz_{\omega \chz} (\vB)}\\
	\pS & = \smallset{\chi}\\
	\pT & = \smallset{1}
\end{align*}
The only difference between ``tilded'' and ``non-tilded'' polynomials are that
the former take randomisers $\vB$ and challenges $A, B, \Gamma, \Delta, V$ as
variables with $\chi$ just being a constant, while the latter are over variable $X$. Also ``tilded''
polynomials have set all $\wit_i$ inputs to $0$.

Polynomials are expressed as polynomials in
randomisers and challenges, where all $\wit_i$, $i \in
\range{1}{3\numberofconstrains}$ are set to $0$, with $\chi$ being just a
constant, e.g.~$\pta(\vB) = (B_1 \chi
+ B_2)\ptZH + \sum_{i = 1}^{\numberofconstrains} 0 \cdot \lag_i (\chi) = (B_1
\chi + B_2) \ptZH$.
For a polynomial $\p{p} \in \pR$ polynomial $\p{p}_x$ is a polynomial where
every occurrence of $\chi$ has been substituted by value $x$. E.g.~ 
\[
	\pta_\chz(\vB) = (B_1 \chz + B_2)\ptZH.
\]
}

\comment{
\michals{6.11}{OLD proof begins}
Now we argue why for a $\ppt$ adversary the proof provided by the simulator is
indistinguishable from the real proof. Observe that the simulator picks the
messages as an honest prover or verifier would except how the message output
in Round 3 are computed. We thus focus on showing that in both real and
simulated proof the outputs of that round are indistinguishable. For that end
we observe that 
\begin{equation}
	\label{eq:p_not_in_SPAN}
	\p{t} \not\in \SPAN\smallset{\p{f_i} \cdot \p{f_j} / \p{f_k}}
\end{equation}
for $\p{f_i}, \p{f_j}, \p{f_k} \in \smallset{\p{a}, \p{b}, \p{c}, \p{z},
	\p{S_{\sigma 1}}, \p{S_{\sigma 2}}, \p{S_{\sigma 3}}, \lag_1, \p{Z_{H}},
\selmulti, \selleft, \selright, \selconst, \seloutput, \pubinppoly}$. Hence,
	the requirements for the uber assumption, cf.~\cref{def:uber_assumption},
	are met and the simulator can replace an honestly computed
	$\gone{\p{t}(\chi)}$ by a random $\p{\tilde{t}}(\chi)$ and the change would
	not be noticed for a $\ppt$ adversary.  
	Furthermore, since $\p{\tilde{t}}(\chz) = \p{t}(\chz)$, the rest of the
	proof that utilises $\p{t}(\chz)$ can be computed honestly as well.
	\markulf{03.11.2020}{Not sure about the definition of random
  polynomials here. What is the argument?}
	\michals{5.11}{Corrected, moved to computational zk, check now}
\qed
\michals{6.11}{OLD proof ends}
}
\end{proof}
...
found and are random $\FF_p$ elements. Hence, the $y$ picked by the simulator
comes from the same distribution as if it was picked by an honest verifier.
