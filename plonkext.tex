...
\begin{proof}
	...
	Divide the adversary's proof $\zkproof$ into two parts. 
	The first, $\vec{a}^\heartsuit = (a_0, \ldots, a_i)$ and the second $\vec{a}^{\spadesuit} = (a_{i + 1}, \ldots, a_\mu)$.
	...
	Otherwise, if the query was made by the simulator $\simulator_{\fs, \zkproof}$ then, since $\proofsystem$ is an $\ur{i}$-protocol, a proof starting with $(\inp, \vec{a}^\heartsuit)$ is in the list $Q$ and cannot be used by the simulation-extractability adversary $\adv_\se$.
	
	After the oracle was queried with $(\inp, \vec{a}^\heartsuit)$, a new randomness for $\simulator_{\fs, \ro}$ is picked. 
	More precisely, assume that $(\inp, \vec{a}^\heartsuit)$ was $J$-th query to $\simulator_{\fs, \ro}$, then the sequence of answers $y_1, \ldots, y_Q$ changes to $y_1, \ldots, y_J, y'_{J + 1}, \ldots, y'_{Q}$, for some fresh $y'_{J + 1}, \ldots, y'_{Q}$ picked accordingly to $r$.
\end{proof}
