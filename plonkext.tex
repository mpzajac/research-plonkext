...
notion \emph{forking} simulation extractability. This is also to avoid confusion
...
work we often simply refer to it as simulation extractability.
...
success probability dependent on the adversary's probability of returning a
...
soundness for the same generic class of protocols.  On the up side, we show that
...
\begin{proof}
  ...
  different ways is at most $\epsop(\secpar)$.
...
\begin{proof}
  ...
  $\zkproof'$ for the same statement that.  To distinguish polynomials and
  commitments that an honest prover sends in the proof from the polynomials and
  ...
  with probability at most $7 \cdot (\epsbind + \infrac{2}{\abs{\FF_p}})$. The
  latter terms comes since the adversary does not provide an opening for each of
  the commitment separately, but does that in batch. That comes with
...
%
  ...
  hold. Hence probability that one of $\p{t^0}(X)$, $\p{t^1}(X)$ is computed
