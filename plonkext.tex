...
\begin{theorem}
  Let $\proofsystem$ be an FSE zkSNARK compiled from an AHP proof system with KZG as the commitment scheme. Then $\proofsystem$ is also USE.
\end{theorem}
\begin{proof}
  ...
  Note that a zkSNARK compiled from a AHP consists of the following:
  \begin{itemize}
  \item Commitments to polynomials $f_i$ sent by the prover; we denote the list of these by $C$.
  ...
  Idea for the proof goes as follows. Adversary $\adv$ internally runs adversary $\bdv$ and on its queries it answers as follows:
  \begin{itemize}
  %\item On a random oracle query on a partial transcript to get challenge $\beta_i$ $\adv$ passes the query to the random oracle $\ro$ and returns its answer.
  \item On a random oracle query on a partial transcript to get evaluation challenge $\chz$, $\adv$ passes the query, gets answer $\chz$ and returns to $\bdv$ challenge $\chz' \gets \alpha \chz$. 
  \item On other random oracle queries, $\adv$ passes the queries to $\ro$ and returns its answer. 
  ...
  \item On $\bdv$ request to see simulated proof for
    ...
    \item Return $\zkproof'$ 
    \end{enumerate}
  \item On $\bdv$'s proof $(\srs', \inp', \zkproof')$, $\adv$ proceeds as follows:
    \begin{enumerate}
    \item $(\inp, \wit) \gets \MoveInstanceBackward(\srs', \inp', \wit')$
  ...
  Since $\proofsystem$ is FSE there exists an extractor $\ext_\adv(r_\adv, Q_\adv, Q_\adv^\ro)$ for $\adv$ that returns witness $\wit$ such that $\REL(\srs, \inp, \wit) = 1$. We show existance of extractor $\ext_\bdv$. More precisely $\ext_\bdv (r_\bdv, Q_\bdv, Q^\ro_\bdv, Q^\srs_\bdv)$ proceeds as follows:
  \begin{itemize}
