...
\begin{proof}
  ...
  % $(1, 1, 3\noofc + 1, 1)$-tree of acceptable transcripts $\tree$. 
 ...
 % \item
  ...
  evaluations known, $\extt$ can recreate their coefficients by interpolation
  ...
  \begin{enumerate}
  \item builds $\plonkprot$'s SRS $\srs$ using the input $\dlog$ instance and
    start $\tdv(\adv, \srs)$;
  ...
  \ncase{$\game{1} \mapsto \game{2}$} Given adversary $\adv$ that wins in Game
  ...
  \ncase{$\game{2} \mapsto \game{3}$} Given adversary $\adv$ that wins in Game
