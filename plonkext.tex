...
\begin{theorem}[Simulation soundness]
	...
	Assume that $(\numberofconstrains + 2)$-dlog is $\eps_\dlog(\secpar)$-hard, $\plonkprot$ is sound and $\ur{3}$ with security $\eps_{\ss}(\secpar)$ and $\epsur(\secpar)$ respectively. 
	...
	\[
		\epsur(\secpar) + q_\ro^6 (\eps_{\dlog}(\secpar) + \eps_{\ss}(\secpar))\,,
	\]
...
\begin{itemize}
	...
	$\zkproof_{\simulator}\range{1}{3}$ such that $\zkproof_{\advss}\range{1}{2} = \zkproof_{\simulator}\range{1}{2}$ and a random oracle query $\zkproof_{\simulator}[2].\ch$ on $\zkproof_{\simulator}\range{1}{2}$.
	\item $\rdvur$ returns two proofs for $\inp_\advss$:
	\begin{align*}
		\zkproof_1 = (\inp_{\advss},\zkproof_{\simulator}\range{1}{2}, \zkproof_{\simulator}[2].\ch, \zkproof_{\simulator}\range{3}{5})\\
		\zkproof_2 = (\inp_{\advss}, \zkproof_{\simulator}\range{1}{2}, \zkproof_{\simulator}[2].\ch, \zkproof_{\advss}\range{3}{5})
	\end{align*}
	\end{itemize}  
	...
	\[
		\prob{\adv \text{ wins}, \event{E}} \leq \epsur(\secpar).
	\]
	
	% Before analyzing the events $\nevent{E}_0$ and $\nevent{E}_1$, we make the following observation:
	% Since $\advss$ is algebraic, it outputs a proof $\zkproof_\advss$ that can be written as 
	% \[
	...
	% \]
	% where $\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$ are all $\GRP_1$-elements from the CRS of $\plonkprotfs$, $\vec{M}$ is a matrix of coefficients output by $\advss$ aside the proof, and $\vec{\tilde{\zkproof}_\simulator}_i$ denote all $\GRP_1$-elements from the simulated proof ${\zkproof_\simulator}_i$. 
...
\ncase{When $\nevent{E}_0$ happens}
In this case the reduction $\rdvs$ uses $\advss$ to break soundness of $\plonkprot$ with probability $\eps_\ks / q_{\ro}^6$, where $q_\ro$ is the number of total random oracle queries performed by the adversary or by $\rdvs$ on behalf of the simulator.
...
\item It guesses indices $i_1, \ldots, i_6$ such that random oracle queries $h_{i_1}, \ldots, h_{i_6}$ are the queries used in $\zkproof_\advss$. This is done with probability at least $1/q_{\ro}^6$ (since there are $6$ challenges from the verifier in $\plonkprot$).
    ...
    $\zkproof_\advss[j + 1]$.\markulf{30.08}{I don't understand this last step.}
		\michals{30.08}{Here I just wanted to say that to respond to $\plonkprot/\verifier$'s challenges $\rdv$ uses the responses from $\advss$}
\end{itemize}
...
In that case $\rdvs$ extracts from $\vec{M'}$ coefficients of $1, \chi, \ldots, \chi^{\numberofconstrains + 2}$ for polynomials $\p{a}, \p{b}$, and $\p{c}$ and reveals the witness $\wit_\advss$ (as it is encoded in theses polynomials' coefficients).
If $\REL(\inp_\advss, \wit_\advss)$ holds then $\advss$ failed to break simulation-extractability of $\plonkprotfs$. On the other hand, if that is not the case, then $\rdvs$ breaks soundness of $\plonkprot$.
%
...
$\rdvs$ provides the simulated proofs for the adversary so it knows how the polynomials used in the proofs are built (i.e. how to express them using $\gone{1, \chi, ... \chi^{n + 2}}$}
...
\[
	\eps \leq \epsur(\secpar) + q_\ro^6 (\eps_{\dlog}(\secpar) + \eps_{\ks}(\secpar))\,,
\]
