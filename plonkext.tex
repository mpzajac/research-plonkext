...
\begin{proof}
	Proof outline. Games:
	\begin{enumerate}
		\item Witness extraction from Plonk with an AGM adversary 
		\item Witness extraction from Plonk with an AGM adversary AND check that polynomial equation holds (not only the verification equation on evaluations). If that's not the case, output $\bad$ and show that when $\bad$ is output the adversary can be used to break $q$-dlog assumption
		\item Show unique response property for the polynomial protocol (statistical unique response property)
	\end{enumerate}
	\qed
\end{proof}
