...
\begin{proof}
	...
	%  is not a part of a simulated proof output by $\simulator_\fs$.
	% Otherwise, since $\proofsystem$ is an $\ur{i}$-protocol, $\vec{a^\heartsuit}$ uniquely determines $\vec{a^\spadesuit}$, thus the output proof $\vec{a} = \vec{a^\heartsuit} \| \vec{a^\spadesuit}$ belongs to the list $Q$, thus cannot be used by $\adv_\se$ to break simulation extractability.
\end{proof}
