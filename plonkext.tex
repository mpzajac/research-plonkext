...
\begin{definition}[Independence of $\p{R}, \p{S}, \p{T}$]
	...
	\[
		\p{f} = \sum_{i = 1}^{r} \sum_{j = 1}^{s} a_{i, j} \p{r}_i \cdot \p{s}_j +
		\sum_{k = 1}^{t} b_k \p{t}_k.
	\]
	We say that $\p{f}$ is \emph{independent} if it is not dependent.
\end{definition}

\begin{definition}[BBG05 Uber assumption]
	\label{def:uber_assumption}
	Let $\p{f} \in \FF_p[X]$, $F = \smallset{\p{f_0} = 1, \ldots, \p{f_n}}$,
	$\p{f_i} \in \FF_p[X]$ and $\p{f} \not\in \SPAN (\smallset{\p{f}_i \cdot
		\p{f}_j / \p{f}_k}_{i, j, k})$. Let $\GRP$ be a bilinear group of order
		$p$. Then for any $\ppt$ adversary $\adv$, random $\chi$ and $y$
		\begin{multline*}
			\condprob{\adv(\gnone{1, \p{f_1}(\chi), \ldots, \p{f_n}(\chi),
			\p{f}(\chi)}) = 0}{\chi \sample \FF_p} \approx_\secpar\\
			\condprob{\adv(\gnone{1, \p{f_1}(\chi), \ldots, \p{f_n}(\chi),
			y})}{\chi \sample \FF_p, y \sample \FF_p}\,.
		\end{multline*}
\end{definition}
