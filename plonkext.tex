...
\end{theorem}
\begin{proof}
	We proceed by a contradiction. Suppose there exist a PPT adversary $\advse$ that breaks simulation extractability with non-negligible probability
	\[
	\eps := \Pr
		\left[
		\begin{aligned}
...
In such case, we are able to build two reductions $\rdvks$, $\rdvur$, which using $\advse$ as a black-box, violate either the soundness or unique response properties of the underlying interactive protocol $\plonkprot$.

In the following we denote by $\zkproof_{\advse}, \zkproof_{\simulator}$ proofs returned by the adversary and the simulator respectively. We use $\zkproof[i]$ to denote prover's message in the $i$-th round of the proof, $\zkproof[i].\ch$ to denote the challenge that is given to the prover after $\zkproof[i]$, and $\zkproof\range{i}{j}$ to denote all messages of the proof between round $i$ and $j$.

Without loss of generality for every acceptable proof $\zkproof_{\advse}$, we assume that whenever $\advse$ outputs in Round $i$, a message $m$, which results in a partial proof $\zkproof_{\advse}[:i]$, then $\zkproof_{\advse}[i]$ is a fresh query to the random oracle. 
In is straightforward to transform any adversary that violates this condition into an adversary that makes these additional queries to the random oracle and wins with the same probability.
...
Additionally, we divide even $\nevent{E}$ into two disjunctive subevents: $\nevent{E}_0$ and $\nevent{E}_1$. 
The event $\nevent{E}_0$ considers a case when the final proof provided by the adversary $\advse$ is accepted by the idealised verification equation, i.e.~for that proof $\vereq(X) \equiv 0$. 
...
\begin{itemize}
	\item The reduction answers both queries to the simulator. It also keeps lists $Q$, for the simulated proofs, and $Q_\ro$ for the random oracle queries. $\plonkprotfs.\simulator$ and random oracle queries.
	\item When $\advse$ outputs a fake proof $\zkproof_{\advse}$ for  $\inp_\advse$, $\rdvur$ looks through lists $Q$ and $Q_\ro$ until it finds 
	$\zkproof_{\simulator}\range{1}{3}$ such that $\zkproof_{\advse}\range{1}{2} = \zkproof_{\simulator}\range{1}{2}$ and a random oracle query on $\zkproof_{\simulator}\range{1}{2}$.
	\item $\rdvur$ returns two proofs for $\inp_\advse$:
	\begin{align*}
		\zkproof_1 = (\inp_{\advse},\zkproof_{\simulator}\range{1}{2}, \zkproof_{\simulator}\range{1}{2}.\ch, \zkproof_{\simulator}\range{3}{5})\\
		\zkproof_2 = (\inp_{\advse}, \zkproof_{\simulator}\range{1}{2}, \zkproof_{\simulator}\range{1}{2}.\ch, \zkproof_{\advse}\range{3}{5})
	\end{align*}
	\end{itemize}  
	...
	\]
	where $\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$ are all $\GRP_1$-elements from the CRS of $\plonkprotfs$, $\vec{M}$ is a matrix of coefficients output by $\advse$ aside the proof, $\vec{\tilde{\zkproof}_\simulator}_i$ denote all group $\GRP_1$ elements from the simulated proof ${\zkproof_\simulator}_i$. 
	Since the reduction provides the simulated proofs, $\rdv$ knows a matrix $\vec{M'}$ such that
	\begin{equation}
		\label{eq:M_prim}
...
\begin{itemize}
	...
	\item On query $h \not\in \smallset{h_{i_1}, \ldots, h_{i_6}}$, $\rdvks$ returns randomly picked $y$, sets $\ro(h) = y $ and stores $(h, y)$ in $Q_\ro$ if $h$ is sent to $\ro$ the first time. If that's not the case, $\rdv$ finds $h$ in $Q_\ro$ and returns the corresponding $y$.
...
Assume that the $\plonkprot.\verifier$ accepts $\zkproof_\advse$. We consider case when the idealised verification equation accepts. (Thus, the real verification accepts as well.) 
...
Then it proceeds as $\rdvks$ does, except in the last part, when the adversary provided its proof $\zkproof_\advse$, it uses the fact that the real verification equation holds, but the ideal verification equation does not to break the dlog assumption. 


Since $\vereq(X) \neq 0$, but $\vereq(\chi) = 0$ and $\rdvdlog$ knows $\vec{M'}$, as defined in \cref{eq:M_prim}, it can recreate all the polynomials submitted by $\advse$ as part of the proof and included in $\vereq(X)$. This way, it knows all coefficients of $\vereq(X)$. Thus it can factorise it and find its roots, one of the would be the required $\chi$. Hence the probability that 
\[
