...
State restoration attack security~\cite{TCC:BenChiSpo16,cryptoeprint:2020:1351} defines a stronger soundness notion for interactive protocols, that allows for a tighter reduction from interactive to non-interactive protocols. These works do, however, not consider simulation soundness and simulation extractability.
...
\ngame{4} This game is identical to $\game{3}$ except that it aborts if
...
\begin{lemma}
\label{lem:plonkprot_ss}
$\plonkprot$ is $(\epsss, (1, 1, \noofc + 3, 1))$-computational forking sound
for
 \[
