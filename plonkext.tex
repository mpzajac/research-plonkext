...
\begin{theorem}[Simulation soundness]
	\michals{29.08}{Proof done similarly to \cite{INDOCRYPT:FKMV12}}
...
\michals{30.08}{I thought that with this assumption we may just consider only queries from $\advse$}
...
\item It guesses indices $i_1, \ldots, i_6$ such that random oracle queries $h_{i_1}, \ldots, h_{i_6}$ are the queries used in $\zkproof_\advse$. This is done with probability at least $1/q_{\ro}^6$ (since there are $6$ challenges from the verifier in $\plonkprot$).
    ...
    $\zkproof_\advse[j + 1]$.\markulf{30.08}{I don't understand this last step.}
		\michals{30.08}{Here I just wanted to say that to respond to $\plonkprot/\verifier$'s challenges $\rdv$ uses the responses from $\advse$}
\end{itemize}
...
\end{gather*}
\markulf{30.08}{We might have to extract from $M$ rather than $M'$. :(}
\michals{30.08}{I think that is fine.
$\rdvks$ provides the simulated proofs for the adversary so it knows how the polynomials used in the proofs are built (i.e. how to express them using $\gone{1, \chi, ... \chi^{n + 2}}$}
...
\section{Simulation extractability} In this section we show that $\plonkprotfs$
is not only simulation-sound, but also simulation-extractable. To that end, we
proceed as follows. First we show that the proof scheme is (kind of)
special-sound. That is, given a number of acceptable transcripts which match on
the first 3 rounds of the protocol we can either reveal a correct witness for
the proven statement or use one of the transcripts to break the dlog assumption.
The latter requires use of the AGM. More precisely, we assume that each group
element that is published as a part of the transcripts comes with a vector of
coefficients that represents the element in the basis compound of the input
group elements, i.e.~$\plonkprotfs$'s CRS. \michals{17.09}{Need to check: if I
make a simulated proof, that uses only programmability of the RO and not
knowledge of the trapdoor -- is the output proof acceptable by the idealised
polynomial verification?}
\markulf{17/09}{In Groth-Maller, the basis is extended to include previously
  simulated proofs}

Given special-soundness of $\plonkprot$, we use the fact that it is also $\ur{3}$ and show, in a similar fashion to \cite{INDOCRYPT:FKMV12}, that it is simulation-extractable. That is, we build reductions that given a simulation extractability adversary $\advse$ either breaks the protocol's unique response property or breaks the dlog assumption, if extracting a valid witness from a tree of transcripts is impossible.

\subsection{Special soundness of $\plonkprot$}
Note that the special soundness property (as usually defined) holds for all--- even computationally unbounded---adversaries. Since a simulation trapdoor for \plonk{} exists, is not special sound in that regard---as an unbounded adversary could reveal the trapdoor and build a number of simulated proofs for a fake statement. 
...
\begin{definition}[Computational special soundness under $\compass$]
	Let $\compass = (\cdv, c)$ be a falsifiable assumption and $\proofsystem$ be an $2k$-round zero-knowledge proof system for a relation $\REL$. 
	We say that $\proofsystem$ is $(n_1, \ldots, n_k)$-\emph{special sound} if for every PPT adversary $\adv$ that produces an accepting $(n_1, \ldots, n_k)$ tree of transcripts $\tree$ for a statement $\inp$ 
	\begin{compactitem}
		\item either exists an extractor $\ext$ that given $\tree$ extracts $\wit$ such that $\REL(\inp, \wit) = 1$, or
		\item there exists a reduction $\rdv$ that black-box uses $\adv$ to break $\compass$.\michals{16.09}{I think the black-box part is not necessary, esp.~we use AGM}
	\end{compactitem}
\end{definition}
\markulf{17/09/2020}{I am confused about when we rely on AGM and when on
  rewinding?}


\begin{lemma}
	\label{lem:plonkprot_ss}
	$\plonkprot$ is $(1, 1, 1, \numberofconstrains + 3, 1)$-computationally special sound under $(\numberofconstrains + 2)$-dlog assumption and algebraic group model.
	\michals{16.09}{$\plonkprot$ is an interactive protocol, but maybe the
    adversary should have access to simulated proofs nevertheless? (Or we just
    assume that the simulation is done without using trapdoor but by
    reprogramming the ro and we don't need to care)}
  \markulf{17.09}{It seems important that in the interactive protocol, the
    adversary can create simulated proofs itself, by knowing the challenge in advance.}
\end{lemma}
\begin{proof}
	...
	First, $\event{E}$ holds when all of the transcripts were generated honestly. That is, all the transcripts are acceptable by the idealised verification equation, i.e.~$\vereq(X) = 0$ 
	Second, $\nevent{E}$ holds when there is a transcript that is acceptable, yet the adversary deviated from the protocol, i.e.~for some transcript $\vereq(\chi) = 0$, but $\vereq(X) \neq 0$.
	We build two reductions $\rdvss$ and $\rdvdlog$, which given the tree of transcripts $\tree$ either reveal the witness ($\rdvss$) or break the dlog assumption ($\rdvdlog$). We show that the probability that one of them succeeds is non-negligible.
	
	\ncase{When $\event{E}$ happens} Since all of the transcripts have been
	generated honestly then there exists a witness $\wit$, such that $\REL(\inp,
	\wit)$ holds.  Note that witness-carrying polynomials $\p{a}(X), \p{b}(X),
	\p{c}(X)$ have degree $(\numberofconstrains + 2)$ and since $\adv$ answered
	honestly on $(\numberofconstrains + 3)$ different challenges $\chz$ then
	$(\numberofconstrains + 3)$ evaluations of these polynomials (at different
	points) are known. Reduction $\rdvss$ interpolates the polynomials and reveals
	the corresponding witness $\wit$.  Note that if $\REL(\inp, \wit)$ does not
	hold then, from the soundness of the polynomial protocol, at least one of the
	transcripts has not been generated honestly (and $\nevent{E}$ happened).
	\michals{17.09}{$\rdvss$ is what is called in FKMV12 a "special extractor"--an extractor that extracts from a tree of transcripts.}
	
	\ncase{When $\nevent{E}$ happens} Consider a transcript that is acceptable,
	yet has not been generated honestly. Since the polynomial protocol is sound,
	$\vereq(X) \neq 0$, but $\vereq(\chi) = 0$, with probability at least $(1 -
	\epsbatch)$.  Here, $\epsbatch$ is (negligible) probability of making an
	acceptable transcript for a false statement because of use of batching in the
	verification equation.\markulf{17.09}{Why is batching special, wouldn't
    batching as a polynomial protocol also be perfect? We would have to treat
    the batching challenge as a formal variable? I guess that is not possible?}
	Furthermore, since the adversary is algebraic, all group elements included in the tree of transcripts are extended by their representation as a combination of the input $\GRP_1$-elements i.e.~$\gone{1, \chi, \ldots, \chi^{\numberofconstrains + 2}}$. Hence all coefficients of the verification equation polynomial $\vereq(X)$ are known and $\rdvdlog$ can find its zero points. Since $\vereq(\chi) = 0$, the targeted discrete log value $\chi$ is among them.
\end{proof}
\michals{17.09}{Need to reformulate the above --- $\event{E}$--idealised verification accepts (and we have $\epsbatch$ there) and $\nevent{E}$ some idealised verification doesn't accept, but the real one does.}
\subsection{From special-soundness and unique response property to simulation extractability}
Since \cref{lem:plonkprot_ur,lem:plonkprot_ss} hold, $\plonkprot$ is $\ur{3}$ and computationally special sound.

\begin{theorem}[Simulation extractability]
	Assume that $(\numberofconstrains + 2)$-dlog is $\eps_\dlog(\secpar)$-hard, $\plonkprot$ is $\ur{3}$ with security $\eps_{\ur{3}}(\secpar)$. 
	Let $\ro\colon \bin^* \to \bin^\secpar$ be a random oracle. 
	Let $\advse$ be a PPT adversary that can make up to $q$ random oracle queries and outputs an acceptable proof for $\plonkprotfs$ with probability at least $\accProb$
	...
	% \michals{16.09}{some numbers will come here}
	% \michals{16.09}{check what the extraction error is (in FKMV it's Q/h)}
	Then the extraction error $\eps$ is upper-bounded by
	% \begin{multline*}
	\[
		1 - \left(
			\frac{\accProb^{\numberofconstrains + 3}}{q^{\numberofconstrains + 2}} - \frac{\accProb \cdot (\numberofconstrains + 2)}{2^\secpar}
		\right)% +
		\cdot 
		% \left(
		% 	\frac{\accProb^{\numberofconstrains + 3}}{q^{\numberofconstrains + 2}} - \frac{\accProb \cdot (\numberofconstrains + 2)}{2^\secpar}
		% \right) \cdot  
		\left(
			1 - (\eps_{\ur{3}} + \epsbatch + \eps_\dlog)
		\right)
	\]
	% \end{multline*}
\end{theorem}
\begin{proof}
	Let $\advse$ be an adversary that breaks simulation extractability with non-neglibible probability.
	\michals{16.09}{Need to change that -- the extractor doens't work with some non-negligible probability (need amplification?)}
	\[
	\eps := \Pr
		\left[
		\begin{aligned}
...
We build reductions $\rdvur$, $\rdvdlog$ which using $\advse$ as a black-box, violate either the unique response property of the underlying interactive protocol $\plonkprot$, or the $(\numberofconstrains + 2)$-dlog assumption.
...
$\zkproof_{\advse}\range{1}{i}$ is a fresh query to the random oracle.
\markulf{30.08}{What does fresh mean here? What about messages coming from the simulator?} 
\michals{30.08}{I thought that with this assumption we may just consider only
  queries from $\advse$}
\markulf{19/09}{I would like to understand this requirement better in light of
  the new proof approach.}

It is straightforward to transform any adversary that violates this condition into an adversary that makes these additional queries to the random oracle and wins with the same probability.

Since we rely on the special soundness property, we denote by $\event{F}$ event when the reduction manages to build a $(1, 1, 1, \numberofconstrains + 3, 1)$-tree of acceptable transcripts $\tree$.
If $\event{F}$ does not happen, what is denoted by $\nevent{F}$ then we grant $\advse$ a victory.

A crucial observation is that the adversary $\advse$ may have learned $\zkproof_{\advse}\range{1}{3}$ by querying the simulator on $\inp_{\advse}$ or might have computed it itself. We denote the first event by $\event{E}$ and the second by $\nevent{E}$. 
%
Additionally, we divide event $\nevent{E}$ into two disjunctive subevents: $\nevent{E}_0$ and $\nevent{E}_1$. 
Event $\nevent{E}_0$ considers a case when all transcripts in $\tree$ has been computed honestly, i.e.~all of them are accepted by the idealised verification equation---for that proof $\vereq(X) = 0$.
Alternatively, event $\nevent{E}_1$ covers a case when for $\zkproof_\advse$ it
holds that $\vereq(\chi) = 0$, but $\vereq(X) \neq 0$, where $\chi$ is $\plonkprotfs$'s trapdoor.
%
As all these events are mutually exclusive and exhaustive, we have
\begin{multline*}
	\eps = \prob{\advse \text{ wins}} \leq 
	\prob{\nevent{F}} + \prob{\event{F}} \cdot (\condprob{\advse \text{ wins}, \event{E}}{\event{F}} + \\ 
	\condprob{\advse \text{ wins}, \nevent{E}_0}{\event{F}} + \condprob{\advse \text{ wins}, \nevent{E}_1}{\nevent{F}}).
	% \prob{\advse \text{ wins}, \event{E}} + \condprob{\advse \text{ wins}, \nevent{E}_0}{\event{F}}\prob{\event{F}} + \\
	% \condprob{\adv \text{ wins}, \nevent{E}_1}{\event{F}}\prob{\event{F}} +
	% \condprob{\adv \text{ wins}, \nevent{E}}{\nevent{F}}\prob{\nevent{F}}\,.
\end{multline*}

From the generalised forking lemma, cf.~\cref{lem:generalised_forking_lemma}, 
\[
	\prob{\event{F}} \geq \frac{\accProb^{\numberofconstrains + 3}}{q^{\numberofconstrains + 2}} - \frac{\accProb \cdot (\numberofconstrains + 2)}{2^\secpar}\,,
\]
where $\accProb$ is the probability that $\advse$ outputs an acceptable proof.
In the cases below, we assume that $\event{F}$ holds.

\ncase{When $\event{E}$ happens}
\michals{17.09}{I think this event could be made independent on $\event{F}$, but then the analysis is more messy}
We assume that $\inp_{\advse}$ was submitted to the simulator $\simulator$. 
We show how $\rdvur$ utilizes $\advse$, that makes use of $\inp_\advse, \zkproof_{\advse}\range{1}{3}$, to break the $\ur{3}$ property of $\plonkprot$. 
This way we bound the probability $\prob{\adv \text{ wins}, \event{E}}$ by the probability of $\rdvur$ being able to win in the $\ur{3}$ game.
...
\begin{itemize}
	...
	On the other hand, if the proofs are not equal, then $\rdvur$ breaks $\ur{3}$-ness of $\plonkprot$. Thus 
	\[
		\condprob{\advse \text{ wins}, \event{E}}{\event{F}} \leq \eps_{\ur{3}}(\secpar).
	\]
	
	\comment{
	Before analyzing the events $\nevent{E}_0$ and $\nevent{E}_1$, we note that a proof $\zkproof$ could be accepted only if the verification equation $\vereq(\chi)$ holds. That is, the verifier plugs-in elements of $\zkproof$ into $\vereq(\chi)$ and checks whether it equals $0$. That is what is called a \emph{real check} in \cite{EPRINT:GabWilCio19}. 
	On the other hand there is an \emph{idealised check}, which verifies whether $\vereq(X) = 0$ \emph{as a polynomial} (with proof elements being polynomials as well).
	
	Also, in these cases the reduction $\rdvdlog$ runs $\advse$ internally and is responsible for simulating its access to simulated proofs and random oracle. When $\advse$ outputs an accepting proof then it is rewound to the state just after Round 3, where $\rdvdlog$ programs anew the random oracle. This step is repeated up to a point when $\advse$ provides a $(1, 1, 1, \numberofconstrains + 3, 1)$-tree of acceptable transfers $\tree$.
	Assume that $\advse$ provides an acceptable proof with probability $\accProb$ and the random oracle has image of size $2^\secpar$, then by the generalised forking lemma \cref{lem:generalised_forking_lemma} probability that in $(\numberofconstrains + 3)$ rewinds of $\advse$ a $(1, 1, 1, \numberofconstrains + 3, 1)$-tree of acceptable transfers is built is at least
	\[
		\frac{\accProb^{\numberofconstrains + 3}}{q^{\numberofconstrains + 2}} - \frac{\accProb \cdot (\numberofconstrains + 2)}{2^\secpar}.
	\]
	}

	\markulf{17/09}{Both of these cases would be subsumed by computational special
  soundness}
\ncase{When $\nevent{E}_0$ happens}
Since in this case all of the transcripts from $\tree$ are acceptable by the polynomial verification equation, that is for each transcript $\vereq(X) = 0$, then the special extractor can reveal the witness corresponding to $\inp$ by interpolating polynomials $\p{a}(X), \p{b}(X), \p{c}(X)$. This happens except some negligible probability $\epsbatch$, which corresponds to the verification error that could occur due to use of batching in verification.
Hence
\[
	\condprob{\advse \text{ wins}, \nevent{E}_0}{\event{F}} \leq \epsbatch\,.
\]

\ncase{When $\nevent{E}_1$ happens}
In this case the adversary manages to provide a transcript such that $\vereq(X) \neq 0$, but $\vereq(\chi) = 0$.
As explained in e.g.~\cref{lem:plonkprot_ss}, in that case the reduction $\rdvdlog$ is able to factorize $\vereq(X)$, find its zero points and reveal $\chi$ effectively breaking the discrete logarithm assumption.
\[
	\condprob{\advse \text{ wins}, \nevent{E}_1}{\event{F}} \leq \eps_\dlog\,.
\]
\qed
\end{proof}
