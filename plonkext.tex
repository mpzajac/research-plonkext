...
\begin{lemma}
	\plonkint{} has unique response property after Round 2.
\end{lemma}
...
\begin{proof}
	...
	\paragraph{Game $\game{1}$:}
		...
		Similarly as in \cite{EPRINT:GabWilCio19} we argue that 
		\begin{itemize}
			\item $\plonkintpoly$'s verification equation accepts only true statements except negligible probability, and 
			\item  if the verification equation for $\plonkint$ holds, while $\plonkintpoly$ does not, then the $\adv$ can be used to break $(\numberofconstrains + 2)$-dlog assumption.
		\end{itemize}
		
		Witness extraction from Plonk with an AGM adversary AND check that polynomial equation holds (not only the verification equation on evaluations). If that's not the case, output $\bad$ and show that when $\bad$ is output the adversary can be used to break $q$-dlog assumption
		\paragraph{Game $\game{4}$} Show unique response property for the polynomial protocol (statistical unique response property)
	% \end{description}
	\qed
\end{proof}
