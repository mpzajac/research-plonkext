...
\begin{proof}[by game hoping]
	We build reductions $\rdvur$, $\rdvss$ which using $\advse$ as a black-box, violate either the unique response property of the underlying interactive protocol $\plonkprot$, or its computational special soundness. 
	We also provide a special extractor $\rdv_\ss$ that from a given tree of acceptable transcripts extracts a witness (effectively making $\advse$ lose). 
	...
	In the following we assume that a reduction $\rdv$, for $\rdv \in \smallset{\rdvur, \rdv_\ss}$, provides $\advse$ with access to a random oracle and simulator. That is, the reductions provide the adversary with simulated proofs. Also, it reqind the adversary when needed.
	...
	\ngame{0} \michals{17.09}{$\advse$ wins if there is no extractable witness for its proof}
	In this game the adversary $\advse$ wins if it provides an acceptable proof $\zkproof$ for a statement $\inp$ which it does not know a witness for.
	
	\ngame{1}\michals{17.09}{$\advse$ wins if there is no extractable witness for its proof, break when $\ur{3}$ broken}
	As in $\game{0}$, $\advse$ wins if it provides an acceptable proof for a statement it does not know a witness for, however the $\game{1}$ additionally aborts when $\advse$ breaks $\ur{3}$-ness of $\plonkprot$. Denote that event 
	by $\event{\errur}$.

	
	Assume that $\advse$ queried the simulator on $\inp_{\advse}$. 
	We show a reduction $\rdvur$ that utilizes $\advse$, which outputs $\inp_\advse, \zkproof_{\advse}\range{1}{3}$, to break the $\ur{3}$ property of $\plonkprot$. 
	...
	\begin{itemize}
		...
		On the other hand, if the proofs are not equal, then $\rdvur$ breaks $\ur{3}$-ness of $\plonkprot$. Thus
		\[
			\prob{\game{0}} = \prob{\game{1} \land \nevent{\errur}}
		\]
		
		
	\ngame{2}\michals{17.09}{$\advse$ wins if there is no extractable witness for its proof, break when $\ur{3}$ broken or generating tree of transcripts fails}
	
	\ngame{3}\michals{17.09}{$\advse$ wins if there is no extractable witness for its proof, break when $\ur{3}$ broken, generating tree of transcripts fails, or ...}
	\qed
\end{proof}
\begin{proof}[by separation of events]
