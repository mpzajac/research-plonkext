...
\ourpar{Soundness.}
  ...
  all $\REL \in \RELGEN(\secparam)$
	\[
      \condprob{\ip{\adv(\srs, \inp)}{\verifier(\srs, \inp)} =
        1}{\srs \gets \kgen(\REL), \inp \gets \adv(\srs); \inp \not\in
        \LANG_\REL} \leq \negl\,;
	\]
...
\condprob{\ip{\adv(\srs, \inp)}{\verifier(\srs, \inp)} =
	1 \land \inp \not\in
	\LANG_\REL}{\srs \gets \kgen(\REL), \inp \gets \adv(\srs)} \leq \negl\,;
\]	
} \markulf{12 Sep}{I agree with that.}
%\end{description}
