...
\michals{15.03}{Explain}
Assume $\CRKT$ is a fan-in two arithmetic circuit, which fan-out is unlimited
and has $n$ gates and $m$ wires ($n \leq m \leq 2n$). \plonk's constraint system
is defined as follows:
\begin{itemize}
\item Let $\vec{V} = (\va, \vb, \vc)$, where $\va, \vb, \vc
  \in \range{1}{m}^n$. Vectors $\va, \vb, \vc$ represents left,
  right and output gate wires. 
\item Vectors $\vec{Q} = (\vql, \vqr, \vqo, \vqm, \vqc) \in
  (\FF^n)^5$ are called \emph{selector vectors}. 
\end{itemize}
