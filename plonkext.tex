...
SRS which needs to be generated afresh for every new relation is subverted. This issue has been tackled, e.g., by~\plonk{}
and \sonic{} which feature an updatable and universal SRS~\cite{C:GKMMM18}. Instead of
using a single SRS-generating entity, this framework allows a universal SRS to be generated by a set of
different parties with the assurance that it remains secure if at least one of these
parties is honest.
...
that adapt proofs generated by different parties. Indeed, the malleability of ECDSA signatures led to transaction malleability attack against Bitcoin payments and such issues can also affect private currencies such as Zcash. 
Nevertheless, most zkSNARKs
are only shown to satisfy a (standard) knowledge soundness definition. 
%We argue that simulation-extractability (SE) is the property that should be
...
which relates to \emph{forking lemma},  we
call our flavour of simulation extractability \emph{forking}. This is also to
...
of simulation extractability than forking, we sometimes omit that adjective and
...
forking soundness, generalized unique response, and a generalized
...
State restoration attack security~\cite{TCC:BenChiSpo16,EPRINT:Holmgren19} defines a stronger soundness notion for interactive protocols, that allows for a tighter reduction from interactive to non-interactive protocols. \cite{cryptoeprint:2020:1351} combine this approach with the algebraic group model to obtain tight security guarantees for the non-interactive versions of BulletProof~\cite{SP:BBBPWM18} and Sonic. A similar approach is also taken by~\cite{cryptoeprint:2019:1177}. These works do, however, not consider simulation soundness and simulation extractability.
