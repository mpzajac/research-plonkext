...
We prove that $\plonk$ is simulation-extractable. Although we do not change the original protocol at all, we had to solve a number of problems in order to show that. The idea of the proof goes as follows. 
First, we show that $\plonk$ has witness-extended emulation. To that end we show how from a number of accepting transcript a witness can be extracted. Unfortunately,  \plonk{} seem not to fit well in the standard definitions of witness-extended emulations like by Lindell \cite{JC:Lindell03} or by Groth and Ishai \cite{EC:GroIsh08}. The former does not consider arguments that utilises a CRS, the latter does, but does not consider giving the CRS trapdoor to the extractor. Thus, we had to modify the definition for the witness-extended emulation. We believe that our formulation would fit well in showing simulation-extractability of NIZKs. 

Then, we took the classical result by Faust et al.~\cite{INDOCRYPT:FKMV12} which shows that a special-sound $\Sigma$-protocol that has the unique response property (URP) is simulation-extractable if made non-interactive by the Fiat--Shamir transform. This result also could not be used directly, as \plonk{} is not a $3$-round $\Sigma$-protocol. It was also not obvious that it poses URP. 
To make Faust et al.~result working in our setting we had to prove it fits multi-round protocols and show that \plonk{} in fact has URP.
...
Besides that, we additionally require that a protocol $\proofsystem$ has \emph{unique responses after $i$}. We call such a protocol an $\iur$-protocol.
We call a $(2\mu + 1)$-move interactive protocol $\proofsystem = \ip{\prover}{\verifier}$ to be an $\iur$ protocol if after the $i$-th round of the protocol (i.e.~after the $2i - 1$ message) all the responses of the prover are fixed. More precisely:

\begin{definition}[Unique responses after $i$]
	\label{def:iur}
	Let $\proofsystem = \ip{\prover}{\verifier}$ be an interactive proof system. Denote by $a_1, b_1, \ldots, a_{\mu - 1}, b_{\mu - 1}, a_{\mu}$ the consecutive messages exchanged in the protocol, where messages $a_i$ come from the prover and $b_i$ from the verifier. We say that $\proofsystem$ has \emph{unique responses after $i$} if for any PPT adversary $\adv$ probability
	\[
		\condprob{
		\begin{aligned}
			& \verifier(\REL, \crs, (a_1, \ldots, a_{i + 1}, \ldots, a_{\mu})) = 1 \, \land \\ 
			& \verifier(\REL, \crs, (a_1, \ldots, a'_{i + 1} \ldots, a'_{\mu})) = 1
		\end{aligned}
		}
		{
		\begin{aligned}
			& \crs \gets \kcrs(\REL), \\
			& (a_1, b_1, \ldots, a'_{i + 1}, b'_{i + 1}, \ldots, a'_{\mu}) \gets \adv(\REL, \crs),\\ 
...
\begin{lemma}[Generalised general forking lemma~\cite{EC:BCCGP16}]
	...
	Let $\proofsystem = \ip{\prover}{\verifier}$ be a $(2\mu + 1)$-move, public coin interactive  protocol. Let $\ext$ be a PPT witness extraction algorithm that always succeeds in extracting a witness from an $(n_1,\ldots, n_\mu)$-tree of accepting transcripts. Assume that $\prod_{i = 1}^{\mu} n_i \leq \poly$. 
...
\begin{theorem}[Witness-extended emulation of the Fiat--Shamir transform]
	\label{thm:wit_ext_em_FS}
	Let $\proofsystem = \ip{\prover}{\verifier}$ be an interactive protocol with unique responses for a language $\LANG \in \npol$. 
	In the random oracle model, the NIZK proof system $\proofsystem_{\FS} = \ip{\prover^\ro}{\verifier^{\ro}}$ 	resulting by applying the Fiat--Shamir transform to $\proofsystem$ has witness-extended emulation. 
	\comment{Here, $q$ is the number of random oracle queries and $h$ is the number of elements in the range of $\ro$. Furthermore, the extractor $\ext_\adv$ needs to run $\adv^{\simulator_1}, \adv^{\simulator_2}$ twice.}
\end{theorem}
