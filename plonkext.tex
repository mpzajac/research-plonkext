...
\ncase{$\sonic$ CRS generation $\kgen(\REL)$} The CRS generating algorithm picks
randomly $\alpha, \chi \sample \FF_p$ and outputs
	\[
      \crs = \left( \gone{\smallset{\chi^i}_{i = -\dconst}^{\dconst},
        ...
        \gtar{\alpha} \right)
	\]
\ncase{$\sonic$ prover $\prover(\REL, \crs, \inp, \wit=\va, \vb, \vc)$}
\begin{description}
...
\ncase{$\sonic$ verifier $\verifier(\REL, \crs, \inp, \zkproof)$} The verifier
...
$\PCOMs$, cf.~\cref{fig:pcoms}, has the unique opening property in the AGM. 
\end{lemma}
\begin{proof}
Let 
