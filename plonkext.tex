...
\begin{proof}
	...
	the corresponding witness $\wit$.  
	Note that $\REL(\inp, \wit)$ may not hold, but, since the polynomial protocol is sound, this may happen only when a batching error occurred. Probability of such event is upper-bounded by some negligible $\epsbatch$.
  ...
  \markulf{17.09}{You rewrote this. Why is batching special, wouldn't
...
\begin{theorem}[Simulation extractability]
	Assume that $(\numberofconstrains + 2)$-dlog is $\eps_\dlog(\secpar)$-hard and $\plonkprot$ is $\ur{3}$ with security $\eps_{\ur{3}}(\secpar)$. 
...
For the sake of completeness we also provide reduction $\rdv_\se$ that from a given tree of acceptable transcripts extracts a witness (effectively making $\advse$ lose). 
...
In the following we assume that a reduction $\rdv$, for $\rdv \in \smallset{\rdvdlog, \rdvur, \rdv_\se}$, provides $\advse$ with access to a random oracle and simulator. That is, the reductions provide the adversary with simulated proofs. 
...
Since we rely on the special soundness property, we denote by $\event{F}$ event when the reduction manages to build a $(1, 1, 1, \numberofconstrains + 3, 1)$-tree of acceptable transcripts $\tree$ (by rewinding $\advse$ and providing it with a fresh random oracle).
...
all transcripts in $\tree$ are accepted by the idealised verification
equation---for that proof $\vereq(X) = 0$. Alternatively, event $\nevent{E}_1$
covers a case when for some $\zkproof_\advse$ it holds that $\vereq(\chi) = 0$,
but $\vereq(X) \neq 0$, where $\chi$ is $\plonkprotfs$'s trapdoor.
%
As all these events are mutually exclusive and exhaustive, we have
\begin{multline*}
...
Assume that $\inp_{\advse}$ was submitted to the simulator $\simulator$. 
We show how $\rdvur$ utilizes $\advse$, which outputs $\inp_\advse, \zkproof_{\advse}\range{1}{3}$, to break the $\ur{3}$ property of $\plonkprot$. 
This way we bound the probability $\prob{\adv \text{ wins}, \event{E}}$ by the probability of $\rdvur$ being able to win in the $\ur{3}$ game.
