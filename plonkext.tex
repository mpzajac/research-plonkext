...
The unique response property has been first proposed by Fischlin in \cite{C:Fischlin05}.
Fischlin's formulation, although perfectly fine for applications presented in \cite{C:Fischlin05}, is not enough in our case. First of all, the property assumes that the protocol has three rounds, with the middle being the challenge from the verifier. That is not the case we consider here. Furthermore, it is not entirely clear how to generalize the property. Should one require that after the first challenge from the verifier the responses are fixed? That could not work since if there are more challenges then they are random.
...
We walk around this obstacle by providing a generalized notion of the unique response property. More precisely, we say that a $(2\mu + 1)$-round protocol has \emph{unique responses after $i$} and is called an $\ur{i}$-protocol if
\begin{definition}[$\ur{i}$-protocol]
	\label{def:iur}
	Let $\proofsystem = \ip{\prover}{\verifier}$ be an interactive proof system. Denote by $a_0, b_0, \ldots, a_{\mu - 1}, b_{\mu - 1}, a_{\mu}$ the consecutive messages exchanged in the protocol, where messages $a_i$ come from the prover and $b_i$ from the verifier. We say that $\proofsystem$ has \emph{unique responses after $i$} if for any PPT adversary $\adv$ probability
	\[
		\condprob{
		\begin{aligned}
...
For example $\Sigma$-protocols are often $\ur{1}$-protocols (e.g~Schnorr's protocol) i.e.~when a conversation between the prover and the verifier compounds of an instance $\inp$ and three messages $(a, z, \gamma)$, then given $(a, z)$ it is infeasible for the prover to produce $\gamma, \gamma'$, such that $\gamma \neq \gamma'$ and $\verifier(\REL, \inp, a, z, \gamma) = \verifier(\REL, \inp, a, z, \gamma) = 1$.
...
\begin{theorem}[Simulation extractability of the Fiat--Shamir transform II]
	...
	Let $\proofsystem = (\prover, \verifier, \simulator_\crs, \simulator_\zkproof)$ be an interactive $\ur{i}$-protocol for a relation $\REL$.
	...
	Furthermore the extractor $\ext_\fs$ for $\proofsystem_\fs$ needs to run $(\simulator_\crs, \simulator_\zkproof)$ $N$ times.
\end{theorem}
\task{6.06}{One problem with this proof: in the original proof by Faust et al.~they use a unique response property for a CRS-free interactive proof, i.e. the proof the simulator makes could be prepared by anyone if allowed to pick the proof messages in any order wanted. That's may not be the case for Plonk, as the simulator (may) require the knowledge of trapdoor $x$ (i.e. the adversary cannot produce the simulated proof himself).}
\begin{proof}
	Let $\adv^{\simulator_{\crs}, \simulator_\zkproof}$ produces for an instance $\inp$ an acceptable proof $\zkproof_\fs = (a_0, \ldots, a_{i + 1}, \ldots, a_\mu)$ that is not in the list $Q$.
