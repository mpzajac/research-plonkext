...
\begin{proof}
	...
	by the simulator $\simulator$ that proceeds as follows:
...
Here $\pR = \smallset{b_1, \ldots, b_9, b_1 b_3, }$ \hl{continue}.
The reduction $\rdv$ learns $\gone{b_1, \ldots, }$ and challenge $\gone{w}$
where $w$ is either $b_A b_B b_C$ \michals{7.11}{Decide which} or a random value. Then it picks $\chi$, $\chz$,
...
\]
for $i \in \range{1}{9}$ and $\xi_i, \zeta_i \sample \FF_p$. And sets $\tw = $
\michals{7.11}{compute it as $w = b_A b_B b_C$, i.e. $\tw = \tb_A \tb_B \tb_C$}
...
\begin{description}
	\item[Round 1] $\rdv$ computes $\gone{\pa(\chi)}$ using as randomisers
		...
		\gone{\pc(\chi)}$.
		$\rdv$ publishes obtained values and picks a Round 1 challenge $\beta,
		\gamma$ at random.
	\item[Round 2] $\rdv$ computes $\gone{\pz(\chi)}$ using $\tb_7, \tb_8,
		\tb_9$ and publishes it.
