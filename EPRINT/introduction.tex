...
Zero-knowledge proof systems, which allow a prover to convince a verifier of an NP statement $\REL(\inp,\wit)$ %\hamid{10.5}{Why we call $\REL(\inp,\wit)$ the statement? Should it not be $\inp$?}
without revealing anything else about the witness $\wit$ have broad application in cryptography and theory of computation~\cite{FOCS:GolMicWig86,STOC:Fortnow87,C:BGGHKMR88}.
...
$k$ of its messages. For our proof, it is sufficient that Plonk is $\ur{3}$, and Sonic and Marlin are $\ur{2}$\hamid{10.5}{Should it not be $\ur{1}$?}.
