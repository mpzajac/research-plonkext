...
$\marlinprot$ have more than three messages, and the number of transcripts required for extraction is more than two. Concretely, $(3 \noofc + 6)$ for Plonk, where $\noofc$ is the number of constraints in the proven circuit, $(\noofc + 1)$ for Sonic
%, where $\multconstr$ and $\linconstr$ are the numbers of multiplicative and linear constraints
, and $(\multconstr + 3)$ for Marlin, where $\multconstr$ is the number of multiplicative constraints. Hence, we do not have a pair of transcripts but a \emph{tree of transcripts}.
