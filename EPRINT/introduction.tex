...
knowledge~\cite{C:CamSta97,C:ChaLys06}, sometimes abbreviated SoK and also known as SNARKY signatures~\cite{C:GroMal17,EPRINT:BKSV20}. 
...
This correspondence is made precise by SoK, which use an instance of an
...
witness. Intuitively, an SoK is thus a proof of knowledge (PoK) of the witness that is tied to a message. To model strong existential unforgeability of SoK signatures, even
...
notion of simulation extractability which guarantees extraction of the witness
...
the chain for all blockchain participants to see. We thus argue that SE is a more suitable notion for robust protocol design. We also claim that SE has primarily an intellectual cost, as it is harder to prove SE than KE---another analogy here is CCA vs CPA security for encryption. However, we will show that the proof systems we consider are SE out-of-the-box.
...
Most modern zkSNARK constructions follow a modular blueprint that involves the design of an information-theoretic interactive protocol, e.g. an Interactive Oracle Proof (IOP)\markulf{1.5}{Anyone knows best paper to cite?}, that is then compiled via cryptographic tools to obtain an interactive argument system. This is then turned into a zkSNARK using the Fiat-Shamir transform. By additionally hashing the message, the Fiat-Shamir transform is also a popular technique for constructing signatures. While well-understood for 3-message sigma protocols and justifiable in the ROM~\cite{CCS:BelRog93}, Fiat--Shamir should be used with
care both because there are counterexamples in theory~\cite{FOCS:GolKal03} and real-world attacks in practice when implemented incorrectly~\cite{Blog:FrozenHeart20}.
...
Although inefficient, as the SRS length is quadratic in the size of the circuit,~\cite{C:GKMMM18} set a new
