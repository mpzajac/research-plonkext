...
\michals{4.10}{Unify ordering of oracles for the adversary -- ro first, $\initU$, then game specific ones}
\michals{4.10}{Add $\ro$ to all algorithms that have access to the random oracle? Or remove it from all algs but adversary? Need consistency!}
...
% new definition to be explicit that simulator answers false instances 
\textcolor{blue}{
Our definition of zero-knowledge for non-interactive arguments is in the explicitly programmable random oracle model where the simulator $\simulator_\fs$ can program the random oracle.
We model $\simulator_\fs$ as a stateful algorithm that runs in two modes. The first mode, $(c,st') \leftarrow \simulator_\fs(1,st,q)$ answers random oracle calls to $\ro$ on input $q$. The second mode $(\pi,st') \leftarrow \simulator_\fs(2,st,x)$ simulates the actual argument for instance $x$.
In order to allow $\advse$ to ask for simulated proofs of false statements in our definition of simulator extractability, we define the following``wrappers'' that are oracles that share state.
\begin{itemize}
\item $\simulator_1(q)$ to denote the oracle that returns the first output of $\simulator_\fs(1,st,q)$;
\item $\simulator_2(x,w)$ that returns the first output of $\simulator_\fs(2,st,x)$ if $(x,w) \in \REL$ and $\bot$ otherwise;
\item $\simulator_2'(x)$ that returns the first output of $\simulator_\fs(2,st,x)$.
\end{itemize}
Since the zero-knowledge property is only guaranteed for statements in the language, the definition of ZK is with respect to oracle $\simulator_2$. That is, $\simulator_\fs = (\simulator_1,\simulator_2)$.
We define \emph{simulation-extractability} with respect to oracle $\simulator_2'$; that is, simulation-extractability is with respect to $\simO = (\simulator_1,\simulator_2')$.
\begin{definition}[Simulation-extractable NIZK]
	\label{def:updsimext}
  \label{def:simext}
	Let $\ps = (\kgen, \prover, \verifier, \simulator_\fs)$ be a NIZK proof system. We say that $\ps$
  is \emph{updatable \COMMENT{forking }simulation-extractable} with respect to $\simO$ with \emph{extraction error} $\nu$ if for
  any $\ppt$ adversary $\adv$ that is given oracle access to an updatable SRS setup $\initU$, a
  simulation oracle $\simO$, cf.~\cref{fig:upd}, and a random oracle $\ro$, and that produces an
  accepting proof for $\ps$ with probability $\accProb$, where
	\[
...
\chaya{13.10}{later, we should show $\simulator_\fs$ for plonk/sonic/marlin. I think it will follow from TLS. But we should emphasize there how to define the above oracles so it is clear that $\simO$ can produce accepting proofs for false statements.}

\chaya{13.10}{there seems to be inconsistency wrt where we use the FS subscript.}
...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
  \michals{4.10}{Is that property of $\Psi$ of $\Psi_\fs$?}
	\label{def:wiuru}
  ...
  \michals{4.10}{use $\zkproof$ instead of $\vec{a}$}
  \michals{4.10}{maybe that def could be written w.r.t.~$\Psi$? For example, we could fix verifier's randomness to assure same challenges.}
	\[
	\Pr\left[
	\begin{aligned}
...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
	\label{def:wiuru-int}
	\changedm{ Let $\proofsystem$ be a $(2\mu + 1)$-message public coin proof system
    $\ps = (\kgen, \prover, \verifier, \simulator)$. Denote by $ \vec{\zkproof})$ the vector
    $\zkproof_1, \ldots, \zkproof_{\mu}, \zkproof_{\mu + 1}$ of  messages output in each round 
    by the prover, We say that $\proofsystem$ has \emph{unique responses from $i$ on}
    if for any $\ppt$ adversary $\adv = (\adv_0, \adv_1)$:
	\[
	\Pr\left[
	\begin{aligned}
	& \zkproof \neq \zkproof', \zkproof_1, \ldots, \zkproof_{i} = \zkproof'_1,
	\ldots, \zkproof'_{i}, \\
	& \verifier (\srs, \inp, \vec{\zkproof}) =
	\verifier(\srs, \inp, \vec{\zkproof'}) = 1  \\
	\end{aligned}
	\,\left|\,
    \begin{aligned}
      & (\srs, \inp) \gets \adv_0^{\initU}, \\
      & (\zkproof, \zkproof') \gets \adv_1^{\verifier (\srs, \inp; \rho)} %\\ \ip{\adv_1 (\srs, \inp)}{\verifier (\srs, \inp; \rho)},\\
      %& \zkproof' \gets \ip{\adv_1 (\srs, \inp)}{\verifier (\srs, \inp; \rho)}
% 	& \inp, \vec{a},  \vec{a'}  \gets \adv^{\ro, \initU}(1^\secpar) \\
% & \vec{a} = (a_1, \ldots, a_{\mu + 1}), \vec{a'} = (a'_1, \ldots,
% 	a'_{\mu + 1})
	\end{aligned}
	\right.\right]
	\]
	is upper-bounded by some negligible function $\negl$. Here adversary's access to $\verifier$ is \emph{single instance only}. That is, the verifier gets as in put instance $\inp$ and involves in a single proving interaction with $\adv$ (esp.~$\adv$ cannot use $\verifier$ to simultanously make two independent proofs for $\inp$). However, $\adv$ can rewind $\verifier$ to a chosen partial transcript.
  \michals{7.10}{Any ideas how to write it more formally?} \michals{7.10}{I think it is fine to have two kinds of verifiers here -- one that interacts with a prover, another that checks given transcript}
}
...
\changedm{
  Another take:
	...
	\Pr\left[
    \begin{aligned}
      & (\zkproof, 1) \gets \ip{\adv (\srs, \inp)}{\verifier (\srs, \inp; \rho)},\\
      & (\zkproof', 1) \gets \ip{\adv (\srs, \inp)}{\verifier(\srs, \inp; \rho)}, \\
	& \zkproof \neq \zkproof', \zkproof_1, \zkproof_1.\ch, \ldots, \zkproof_{i} = \zkproof'_1, \zkproof'_1.\ch, 
	\ldots, \zkproof'_{i}, 
	\end{aligned}
	\,\left|\,
    \vphantom{
  ...
  \michals{7.10}{In that take we need to state that  $\ip{}{}$ returns proof and verifier's bit}}
...
\begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
  ...
  \michals{11.10}{tls definition for interactive protocols}
  \changedm{
    \begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
      Let $\ps$ be a $(2\mu + 1)$-message ZK proof system.  We call $\ps$
      \emph{trapdoor-less simulatable} if for any adversary $\adv$
  \begin{align*}
    \left\{
    \begin{aligned}
      \ip{\prover (\inp, \wit)}{\verifier (\srs, \inp)}
    \end{aligned}
      \left|\,
    \begin{aligned}
      (\srs, \inp, \wit) \gets \adv^{\initU}(\secparam)
    \end{aligned}
    \right.\right\}
    \approx
    \left\{
  ...
  Additionally, we say that $\ps$ is $k$-programmable, if $\simulator$ produces a
  simulated proof such that it picks $\zkproof_{i}.\ch$
  \emph{after} $\zkproof_i$ has been computed, for $i \in \range{1}{k - 1}$.
\end{definition}
}

\michals{11.10}{TLS definition for nizk}
\changedm{
  \begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
    Let $\ps$ be a NIZK which proof compounds of $(2\mu + 1)$ parts
    $\zkproof_1, \zkproof_1.\ch, \ldots, \zkproof_{\mu}, \zkproof_{\mu}.\ch,
    \zkproof_{\mu + 1}$.  We call $\ps$ \emph{trapdoor-less simulatable} if for any
    adversary $\adv$, $\eps_0 \approx \eps_1$, where
    \begin{align*}
      \eps_b = \Pr \left[
      \adv^{\ro, \initU, \oracleo_b} (\secparam) = 1
      \right]
    \end{align*}
    where $\oracleo_b$ on input $(\inp, \wit) \in \REL$ and $\srs$ being the SRS
    finalized by $\adv$ responds with $\zkproof \gets \prover (\srs, \inp, \wit)$
    when $b = 1$ or $\zkproof \gets \simulator (\srs, \inp)$ when $b = 0$.
 
    Additionally, we say that $\ps$ is $k$-programmable, if $\simulator$ produces a
    simulated proof such that for instance $\inp$ it picks a random witness $\wit$ and runs $\prover (\srs, \inp, \wit)$ to get partial proof $\zkproof_1, \ldots, \zkproof_{k - 1}.\ch$. Then it continues to produce the whole proof $\zkproof = \zkproof_1, \ldots, \zkproof_{\mu}$ and outputs it.
\end{definition}
  }
