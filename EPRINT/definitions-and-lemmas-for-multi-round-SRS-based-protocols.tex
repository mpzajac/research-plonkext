...
\textcolor{blue}{
Our definition of zero-knowledge for non-interactive arguments is in the explicitly programmable random oracle model where the simulator $\simulator$ can program the random oracle.
We model $\simulator$ as a stateful algorithm that runs in two modes. The first mode, $(c,st') \leftarrow \simulator(1,st,q)$ answers random oracle calls to $\ro$ on input $q$. The second mode $(\pi,st') \leftarrow \simulator(2,st,x)$ simulates the actual argument for instance $x$.
...
\begin{itemize}
\item $\simulator_1(q)$ to denote the oracle that returns the first output of $\simulator(1,st,q)$;
\item $\simulator_2(x,w)$ that returns the first output of $\simulator(2,st,x)$ if $(x,w) \in \REL$ and $\bot$ otherwise;
\item $\simulator_2'(x)$ that returns the first output of $\simulator(2,st,x)$.
\end{itemize}
Since the zero-knowledge property is only guaranteed for statements in the language, the definition of ZK is with respect to oracle $\simulator_2$. That is, $\simulator = (\simulator_1,\simulator_2)$.
