...
\begin{itemize}
	...
	\item We use the Attema et al. extractor in Fig. 3 to extract a $(k,n):=(1, \ldots, 1, n, 1, \ldots, 1)$-tree of accepting transcripts. To this end, we first consider the algorithm $\adv$ (that is an extension of dishonest prover $\prover^*$ that additionally checks the correctness of the output proof $\pi$) as the recursion base (which outputs a $(1, \ldots, 1)$-tree) and then run the subextractor $\zdv_k (\inp)$ which outputs a $(k,n)$-tree of accepting transcript. The run time and success probability of $\zdv_k (\inp)$ is given in proposition 2 of Attema paper: $\zdv_k (\inp)$ makes an expected number of at most $n+Q(n-1)$ queries to $\adv$ and outputs the tree with probability at least $\frac{\accProb - (Q+1) \epsk(\secpar)}{1-\epsk(\secpar)}$.
\end{itemize}
}
