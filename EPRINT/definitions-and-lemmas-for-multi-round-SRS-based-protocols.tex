...
$\simulator = (\simulator_1, \simulator_2')$.\hamid{8.2}{Would this not be confusing with the original simulator $\simulator = (\simulator_1, \simulator_2)$ in the next definition? Maybe better to call it $\simulator'$?}
%
\begin{definition}[Simulation-extractable NIZK]
  ...
  proof for $\ps$ with probability $\accProb$, where
	...
	\]
	there exists an extractor $\extse$ such that\hamid{8.2}{I think we can remove some inputs of $\ext_\se$ such as $\inp_{\advse}, \pi_{\advse}$!}
	\[
	\extProb = \condprob{
	\begin{matrix}
  \verifier(\srs, \inp_{\advse}, \zkproof_{\advse}) = 1 \\
 \wedge  ~(\inp_{\advse}, \zkproof_{\advse}) \not\in Q   \\
	...
	queries to $\simulator_1$ and the (simulated) random oracle's answers. \hamid{8.2}{And list $Q_\srs$ contains all $(\srs, \rho)$ of update SRSs and their proofs.}
...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
  ...
  where $\inp$ is the proven statement. We say that $\proofsystem$ \hamid{8.2}{Should it not be $\psfs$?} has \emph{unique
...
& \zkproof = (\zkproof_1, \zkproof_1.\ch, \ldots, \zkproof_{\mu}.\ch, \zkproof_{\mu + 1}), \zkproof' = (\zkproof'_1, \ldots,
	...
	\right.\right]
	\]
	is upper-bounded by some negligible function $\negl$.\hamid{8.2}{Better to use a more explicit notation e.g., ``We say that $\proofsystem$ has \emph{unique
			responses from $i$ on} with security $\epsur(\secpar)$ if .....is upper-bounded by some function $\epsur(\secpar)$. If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ has unique
		responses from $i$.''}
\end{definition}
%
...
string.
 ...
 allow for such a simulation \emph{trapdoor-less simulatable} (TLS).\hamid{8.2}{I think it's better to combine this subsection with "zero-knowledge" part of subsection 2.5. They both define ZK in programmable ROM, although inconsistently. For example, we consider update oracle in one case and don't consider it in the other case. Also, what is $\simulator_{\fs}$ in the next definition? Is it $\simulator_{\fs} = (\simulator_1, \simulator_2)$? Also, $\adv$ has oracle access to $P_{\fs}$ and not $P$, right?}\hamid{8.2}{wherever we define ZK, I think better to define it with some parameter e.g., $\epszk$-zk; otherwise, using this notation later is ambiguous!}
...
\begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
  ...
  $\zkproof_\simulator$.  We call $\ps$ \hamid{8.2}{Should it not be $\psfs$?} \emph{trapdoor-less simulatable} if for any
...
\begin{lemma}[Generalized forking lemma]\hamid{8.2}{should we refer to the above fig.3 here?}
  \michals{19.1}{Here I am focused on a case where the tree branches at single point, hope
    we could generalize it} %
  Consider algorithm $\genforking$ that internally runs $\zdv_1 (x = (\srs, \inp))$ and
  ...
  queries. Let proof system $\proofsystem$ be $(1, \ldots, n, \ldots, 1)$-forking special sound and
  $\epsk (\secpar)$ knowledge sound. Then for every probability that $\genforking$ outputs
...
\oursubsub{Forking special soundness}\hamid{8.2}{I am a bit confused here! Forking special soundness should be defined for interactive arguments, no? Why we define it in the ROM and talk about FS in the definition below? To me, this definition should be something like $k$-special soundness property in Attema paper, based on which one can argue extractability of the witness from the tree of accepting transcripts (generated by the forking lemma).}
\hamid{8.2}{In fact, I am thinking about adaptation of Attema result to our theorem in this way:
\begin{itemize}
	\item We are given a protocol that is $(k,n)$-special sound; that is, given a $(1, \ldots, 1, n, 1, \ldots, 1)$-tree of accepting transcripts outputs a witness for the proven statement.
	\item $(k,n)$-special soundness implies knowledge soundness with knowledge error $\epsk(\secpar) = \frac{n-1}{N}$, where $N$ is the cardinality of the challenge set.
	\item We use the Attema et al. extractor in Fig. 3 to extract a $(k,n):=(1, \ldots, 1, n, 1, \ldots, 1)$-tree of accepting transcripts. To this end, let $\adv$ be an adversarial prover that makes up to $Q$ random oracle queries and outputs an acceptable proof with probability at least
	$\accProb$. We consider $\adv$ as the recursion base which outputs a $(1, \ldots, 1)$-tree and then run the subextractor $\zdv_k (\inp)$ which outputs a $(k,n)$-tree of accepting transcript. The run time and success probability of $\zdv_k (\inp)$ is given in proposition 2 of Attema paper: $\zdv_k (\inp)$ makes an expected number of at most $n+Q(n-1)$ queries to $\adv$ and outputs a (k,n)-tree with probability at least $\frac{\accProb - (Q+1) \epsk(\secpar)}{1-\epsk(\secpar)}$.
\end{itemize}
}
...
% \chaya{a notion of computational special soundness has been used to mean exactly
% the above in prior works, like BBF19. we should clarify if forking soundness
% different from computational special soundness? seems to me like the difference is
...
\changedm{
  ...
  $(k, n)$-tree of accepted transcripts in expected
