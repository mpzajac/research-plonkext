...
\mxout{Our definition of zero-knowledge for non-interactive arguments is in the
  explicitly programmable random oracle model where the simulator
  ...
  runs in two modes. The first mode, $(c, st') \gets \simulator (1, st, q)$ answers
  random oracle calls to $\ro$ on input $q$. The second mode
...
$\simulator_2$ answers with simulated proofs for true statements.  Since the
