...
\begin{definition}[$(k,n)$-forking special soundness in the updatable setting]
  ...
  update oracle queries.
	...
	accepting transcripts $\tree$.\hamid{1.3}{Not clear if this always happen with probability 1, or is such that over which the below probability is defined? Maybe we should say $\genforking_{\zdv}^{n}$ produce a tree or $\bot$?}
...
\hamid{1.3}{If we want to keep the definition of forking special soundness, I suggest to remove Lemma 2 above and have something like the following:
\begin{enumerate}
	\item make the definition of forking special soundness clear!
	\item add a lemma like this:
	\begin{lemma}[Forking special soundness]\label{lem:forking_ss}
		Let $\proofsystem$ be a proof system that is knowledge sound
		with knowledge error $\epsk$. Assume adversary $\advse$ that makes up to $q$ random
	...
	\end{lemma}  
\end{enumerate}}
\hamid{1.3}{Discuss this at Friday: I'm not yet convinced that the definition of forking special soundness in needed! Can we not just define $(k,n)$-special soundness and show that }
