...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
    ...
    $\ps = (\kgen, \prover, \verifier, \simulator)$. Denote by
    $\zkproof_1, \ldots, \zkproof_{\mu}, \zkproof_{\mu + 1}$ protocol messages output
...
\changedm{
  \begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
    Let $\ps$ be a NIZK which proof compounds of $(2\mu + 1)$ elements
    $\zkproof_1, \zkproof_1.\ch, \ldots, \zkproof_{\mu}, \zkproof_{\mu}.\ch, \zkproof_{\mu + 1}$.  We
    call $\ps$ \emph{trapdoor-less simulatable} if for any adversary $\adv$, $\eps_0
    \approx \eps_1$, where
    \begin{align*}
      \eps_b = \Pr \left[
      ...
      \right]
    \end{align*}
    where $\oracleo_b$ on input $(\inp, \wit) \in \REL$ and $\srs$ being the SRS finalized by $\adv$ responds with $\zkproof \gets \prover (\srs, \inp, \wit)$ when $b = 1$ or $\zkproof \gets \simulator (\srs, \inp)$ when $b = 0$.
 
    Additionally, we say that $\ps$ is $k$-programmable, if $\simulator$ produces a
    simulated proof such that it picks $\zkproof_{i}.\ch$ \emph{after} $\zkproof_i$
    has been computed, for $i \in \range{1}{k - 1}$.
\end{definition}
  }
...
\changedm{
  \begin{definition}[$(k,n)$-forking special soundness in the updatable setting]
	\michals{7.10}{I am not sure that this definition would make sense in the interactive setting. Maybe use just witness extended emulation?}
Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$  be an
	$(2 \mu + 1)$-message proof system for a relation $\REL$. 
	
	For any $\ppt$ adversary $\advse^{\initU, \ro} (1^\secpar; r)$ with access to
  ...
  update oracle queries.
	%
	$\zdv$ returns the index $i$ of the random oracle query made for challenge $k$ and
  the proof $\adv$ returns.
	
	Consider the algorithm $\genforking_{\zdv}^{n}$ that rewinds $\zdv$ to produce a
  $(1,\dots, n,\dots, 1)$-tree of accepting transcripts.
	
	We say that $\psfs$ is $(k, n)$-forking special sound with security loss
  $\eps(\secpar)$, if for any PPT adversary the probability that
	\begin{align*}
	\Pr\left[
	\REL(\inp, \wit) = 0
	\,\Biggl|\,
    ...
    & r \sample \RND{\advse},
    (\inp_{\advse}, \zkproof_{\advse}) \gets \advse^{\initU, \ro} (1^\secpar; r), \\
    & (1, \tree) \gets \genforking_{\zdv}^{m}((\srs,\adv,r),Q_{H}), \wit \gets
    \extt(\tree)
	\end{aligned}
	\right] \leq \eps(\secpar).
	\end{align*}
	Here, $\srs$ is the SRS that $\advse$ finalised using the update oracle $\initU$.
  List $Q_\ro$ contains all $\advse$'s queries to $\ro$ and the random oracle's
  answers.
\end{definition}
}
