...
al.~\cite{EPRINT:AttFehKlo21} generalization of the forking lemma\footnote{The
  ...
  extraction success probability bound.}  which show security of the Fiat--Shamir
...
\begin{figure}[t]
  ...
  %\fbox{
    \begin{enumerate}
    \item Run $\zdv_{m + 1}(x)$ as follows to obtain $(\vec{I}, y_1, v)$: relay the
    ...
    \item Else, repeat
      ...
      \item run $\zdv_{m + 1} (x)$ as follows to obtain $(I',y',v')$, aborting right after
        the initial run of $\adv (x)$ if $I'_m \neq I_m$: answer the query to $i$ with
        $c'_i$, while answering all other queries consistently if the query was performed
        by $\zdv_{m + 1} (x)$ already on a previous run and with a fresh random value in $C$
        otherwise;
      \end{itemize}
    ...
    \item In the former case, output $I$, the $k_m$ accepting
      $(1, \ldots, 1, k_{m + 1}, \ldots, k_\mu)$-trees $y_1, \ldots ,y_{k_m}$, and
      $v \gets 1$; in the latter case, output $v \gets 0$.
    \end{enumerate}
   % }
    \caption{Extractor $\zdv_m (x)$ from Attema et al. Here $\vec{I}$ is an index vector
      ...
      verification bit, that is $v = \verifier (\srs, \inp, \zkproof) = \verifier(x, \vec{y})$. }
\end{figure}
