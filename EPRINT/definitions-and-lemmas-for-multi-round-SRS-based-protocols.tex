...
\begin{figure}[t]
  ...
  %\fbox{
    \begin{enumerate}
    \item Run $\zdv_{m + 1}(x)$ as follows to obtain $(I, y_1, v)$: relay the
    ...
    \end{enumerate}
   % }
\caption{Extractor $\zdv_m (x)$ from Attema et al.}
\end{figure}


\begin{lemma}{Generalized forking lemma}
  Consider algorithm $GF$ that internally runs $\zdv_1 (x = (\srs))$ . Probability that
  $GF$ outputs $N = k_1 \cdot \ldots \cdot k_\mu$ tree of acceptable transcripts is at
  least 
\end{lemma}
