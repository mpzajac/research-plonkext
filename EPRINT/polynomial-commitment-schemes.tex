...
\begin{pcvstack}[center,boxed]
		\begin{pchstack}
			...
			{
			\chi \sample \FF_p \\ [\myskip]
			\pcreturn \gone{1, \ldots, \chi^{\maxdeg}}, \gtwo{\chi}\\ [\myskip]
      ...
      \vec{o} = \gone{\vec{\p{o}}(\chi)}\\ [\myskip]
			...
			\procedure{$\verifyb(\srs, \gone{\vec{c}}, \vec{z}, \vec{s}, \gone{\p{o}(\chi)})$}
			{
				\vec{r} \gets \FF_p^{\abs{\vec{z}}}\\ [\myskip]
				\pcfor j \in \range{1}{\abs{\vec{z}}} \pcdo \\ [\myskip]
				\pcind \pcif 
          \sum_{j = 1}^{\abs{\vec{z}}} r_j \cdot \gone{\sum_{i \in K_j}
          \gamma_j^{i - 1} c_{j, i} - \sum_{i \in K_j} \gamma_j^{i - 1} s_{j, i}} + \bullet \gtwo{1} + \\ [\myskip] 
          \pcind \sum_{j = 1}^{\abs{\vec{z}}} r_j z_j o_j
          \bullet \gtwo{1} \neq \gone{\sum_{j = 1}^{\abs{\vec{z}}} r_j o_j }
      ...
      \procedure{$\verify(\srs, \gone{\vec{c}}, \vec{z}, \vec{s}, \gone{\p{o}(\chi)})$}
			{
				...
				\pcind \pcif 
          \gone{\sum_{i \in K_j}
          \gamma_j^{i - 1} c_{j, i} - \sum_{i \in K_j} \gamma_j^{i - i} s_{j, i}} \bullet
...
%
    ...
    % \end{pchstack}
	\end{pcvstack}
	\caption{$\PCOM$ polynomial commitment scheme. Here $\abs{\vec{z}} = l$ is the number of evaluation points. We denote by }
	\label{fig:pcomp}
  \end{figure}
...
\begin{lemma}
\label{lem:pcomp_op}
Let $\PCOM$ be a batched version of a KZG polynomial commitment,
cf.~\cref{fig:pcomp}, that commits to $l$ polynomials of degree up to $\maxdeg$. Let $\vec{z} = (z_0, \ldots, z_{l - 1}) \in \FF_p^l$ be the points the polynomials are  evaluated at, $k_i \in \NN$ be the number of the committed polynomials to be evaluated at $z_i$, and $K_i$ be the set of indices of these polynomials. Let $\vec{s_{K_i}} \in \FF_p^{k_i}$ the evaluations of polynomials at $z_i$,
and $\vec{o} = (o_0, \ldots, o_{l - 1}) \in \FF_p^l$ be the commitment openings. We show that the probability an algebraic adversary $\adv$ opens the same commitment in two different ways is at most $\epsop(\secpar)$, for
$\epsop(\secpar) \leq l \epsudlog(\secpar) + \infrac{1}{\abs{\FF_p}}$, where
$\epsudlog(\secpar)$ is security of the $(\maxdeg, 1)$-$\udlog$ assumption and
$\FF_p$ is the field used in $\PCOM$.
\end{lemma}
\begin{proof}
  First, we consider a case when for all evaluation points, all openings are verified independently. To that end, we show that the real verifier accepts am incorrect polynomial commitment opening with negligible probability only. Second, we show that the batching increases only negligibly the probability that the adversary opens a commitment to a valid evaluation in two different ways.
%
  The proof goes by game hops.
  ...
  Any discrepancy between the idealised verifier rejection and real verifier acceptance allows one to break the (updatable) discrete logarithm problem.  The reduction $\rdvdulog$ proceeds as follows. It answers $\adv$'s queries for SRS updates according to the answers it receives from its udlog update oracle. When $\adv$ finalizes an SRS, $\rdvdulog$ finalizes the corresponding udlog challenge $(\gone{1, \ldots, {\chi'}^{\maxdeg}}, \gtwo{1})$. Consider a proof $\zkproof$ such that $\vereq_{\inp, \zkproof}(X) \neq 0$, but $\vereq_{\inp, \zkproof}(\chi') = 0$. Since $\adv$ is algebraic, all proof elements are extended by their representation as a combination of the input $\GRP_1$-elements. Therefore, all coefficients of the verification equation polynomial $\vereq_{\inp, \zkproof}(X)$ are known. Now, $\rdvdulog$ computes the roots of $\vereq_{\inp, \zkproof}(X)$, finds $\chi'$ among them, and returns $\chi'$.
  \michals{28.04}{describe $\vereq$}
  ...
  We note that the idealised verifier $\vereq(X)$ does not accept two different openings of a correct evaluation. Hence hte probability that the adversary wins Game 2 is $0$.

  \ncase{Conclusion} %
  
  
  Furthermore, any discrepancy between the idealised verifier rejection and real verifier acceptance allows one to break the (updatable) discrete logarithm problem. The reduction $\rdvdulog$ proceeds as follows. It answers $\adv$'s queries for SRS updates according to the answers it receives from its udlog update oracle. When $\adv$ finalizes an SRS, $\rdvdulog$ finalizes the corresponding udlog challenge $(\gone{1, \ldots, {\chi'}^{\maxdeg}}, \gtwo{1})$. Consider a proof $\zkproof$ such that $\vereq_{\inp, \zkproof}(X) \neq 0$, but $\vereq_{\inp, \zkproof}(\chi') = 0$. Since $\adv$ is algebraic, all proof elements are extended by their representation as a combination of the input $\GRP_1$-elements. Therefore, all coefficients of the verification equation polynomial $\vereq_{\inp, \zkproof}(X)$ are known. Now, $\rdvdulog$ computes the roots of $\vereq_{\inp, \zkproof}(X)$, finds $\chi'$ among them, and returns $\chi'$. Hence the 

  The idealised verifier checks whether the following equality, for $\gamma, r$
  picked at random, holds:
  \begin{multline}
    \label{eq:ver_eq_poly}
    \sum_{j = 0}^{l - 1} r^{j}\left(\sum_{i \in K_j} \gamma^{i - 1} \cdot \p{f}_i(X) - \sum_{i \in K_j} \gamma^{i - 1} \cdot {s_i}_j \right) 
    \equiv \sum_{j = 0}^{l - 1} r^{j} \p{o_{j}}(X)(X - z_j).
  \end{multline}
  Since $r$ has been picked at random from $\FF_p$, probability that
  ...
  \]
  is $\infrac{(l - 1)}{\abs{\FF_p}}$~cf.~\cite{EPRINT:GabWilCio19}. 
  When \(
    r^j \left(\sum_{i \in K_j} \gamma^{i - 1} \cdot \p{f}_i(X) - \sum_{i \in K_j}
    \gamma^{i - 1} \cdot {s_i}_j \right) \not\equiv r^j \p{o_j}(X)(X - z_j)
  \)
  ...
  Any discrepancy between the idealised verifier rejection and real verifier acceptance allows one to break the (updatable) discrete logarithm problem.
%
...
%
  The reduction $\rdvdulog$ proceeds as follows. It answers $\adv$'s queries for SRS updates according to the answers it receives from its udlog update oracle. When $\adv$ finalizes an SRS, $\rdvdulog$ finalizes the corresponding udlog challenge $(\gone{1, \ldots, {\chi'}^{\maxdeg}}, \gtwo{1})$. Consider a proof $\zkproof$ such that $\vereq_{\inp, \zkproof}(X) \neq 0$, but $\vereq_{\inp, \zkproof}(\chi') = 0$. Since $\adv$ is algebraic, all proof elements are extended by their representation as a combination of the input $\GRP_1$-elements. Therefore, all coefficients of the verification equation polynomial $\vereq_{\inp, \zkproof}(X)$ are known. Now, $\rdvdulog$ computes the roots of $\vereq_{\inp, \zkproof}(X)$, finds $\chi'$ among them, and returns $\chi'$.
  
 Hence, the probability that the adversary breaks the unique opening property of $\PCOM$ is upper-bounded by
  $l \cdot \epsdlog + \infrac{1}{\FF_p}$.
