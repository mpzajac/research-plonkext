...
\begin{pcvstack}[center,boxed]
      ...
      \vec{o} = \gone{\vec{\p{o}}(\chi)}\\ [\myskip]
			...
			\procedure{$\verifyb(\srs, \gone{\vec{c}}, \vec{z}, \vec{s}, \gone{\p{o}(\chi)})$}
			{
				...
				\pcind \pcif 
          \sum_{i = 1}^{\abs{\vec{z}}} r_i \cdot \gone{\sum_{j = 1}^{t_j}
          \gamma_i^{j - 1} c_{i, j} - \sum_{j = 1}^{t_j} s_{i, j}} \bullet
...
%
    ...
    % \end{pchstack}
	\end{pcvstack}
	\caption{$\PCOM$ polynomial commitment scheme.}
	\label{fig:pcomp}
  \end{figure}
...
\begin{proof}
  ...
  \ncase{Game 1} This game is identical to Game 0 except it is additionally aborted if for some $j$ \michals{27.04}{Idealised batched verifier does not accept?}.

  \ncase{Game 2} This game is identical to Game 1 except it is additionally aborted if one of the \michals{27.04}{udlog reduction}

  We note that the idealised verifier $\vereq(X)$ does not accept two different openings of a correct evaluation. Furthermore, any discrepancy between the idealised verifier rejection and real verifier acceptance allows one to break the (updatable) discrete logarithm problem. The reduction $\rdvdulog$ proceeds as follows. It answers $\adv$'s queries for SRS updates according to the answers it receives from its udlog update oracle. When $\adv$ finalizes an SRS, $\rdvdulog$ finalizes the corresponding udlog challenge $(\gone{1, \ldots, {\chi'}^{\maxdeg}}, \gtwo{1})$. Consider a proof $\zkproof$ such that $\vereq_{\inp, \zkproof}(X) \neq 0$, but $\vereq_{\inp, \zkproof}(\chi') = 0$. Since $\adv$ is algebraic, all proof elements are extended by their representation as a combination of the input $\GRP_1$-elements. Therefore, all coefficients of the verification equation polynomial $\vereq_{\inp, \zkproof}(X)$ are known. Now, $\rdvdulog$ computes the roots of $\vereq_{\inp, \zkproof}(X)$, finds $\chi'$ among them, and returns $\chi'$. Hence the 
