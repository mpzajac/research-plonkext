...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
  ...
  where $\inp$ is the proven statement. We say that $\proofsystem$ has \emph{unique
    responses from $i$} with security $\epsur(\secpar)$ if for any $\ppt$ adversary $\adv$:\hamid{21.4}{There seems to be a typo in $\exists \zkproof_{j'} \neq \zkproof_{j'}: j' \leq j$ below!}\michals{21.4}{Check now.}
  \[
	\Pr\left[
	\begin{aligned}
	...
	\]
	\michals{22.04}{Remove desc of $\zkproof$ and $\zkproof'$ from the right side of prob.}
	\michals{22.04}{Remove the random oracle, let the adversary pick its own challenges conditioned on a fact the adversary changes the message first, then the challenge}
	Here, $\tilde{\verifier}$ behaves as $\verifier$ until round $i$ where it does not validate challenges $\zkproof_i.\ch$ sent in the proof against the random oracle $\ro$, but accepts them unconditionally. In other words, the verifier accepts proofs where challenges after $i$ are picked by the adversary. If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ is $\ur{i}$.
\end{definition}
%
...
\end{definition}
\michals{22.04}{Tree should be defined w.r.t. particular $x$}\hamid{22.4}{I checked other related papers e.g., Attema paper and they define it similarly without respect to a particular statement; so I think the definition is okay (unless we're in a different 'adaptive' setting?)}
...
\begin{definition}[$(n_1,	\ldots, n_\mu)$-computational special soundness]
	...
	\michals{17.04}{I think it is bad idea to have on the right side just $(x, w) \gets \adv^{\ro, \initU}$ as we don't have $x, w$ on the left side, so it is confusing when we introduce them on the right side. To clarify how $\tdv$ simulates the oracles, we should add a text below the formula imho.}
	\markulf{22.4}{I think its a good idea: we need that $w$ is for the $x$ that resulted from the right execution of $A$. But we need to set it up right.}
	\begin{align*}
	\Pr\left[
...
where $\tdv$ simulates oracles $\ro$ and $\initU$ to the adversary, $\srs$ is the finalized SRS of $\initU$, and $\tree$ is a $(n_1, \ldots, n_\mu)$-tree of accepting transcripts for $x$, and
	...
	\end{align*}
	 ...
	 \michals{22.04}{Make $\tdv$ output bot as well (if necessary)}
	 \michals{22.04}{Use Attema et tree builder directly?}
\end{definition}
...
\begin{figure}[t]
	...
	\michals{21.04}{Updated to a new extractor with adaptive soundness}
\end{figure}
