...
\begin{definition}[$\ur{i}$-protocol in the updatable setting]
  ...
  \[
	...
	\]
	Here, $\tilde{\verifier}$ behaves as $\verifier$ until round $k$\hamid{21.4}{what is $k$?} where it does not validate challenges $\zkproof_i.\ch$ sent in the proof against the random oracle $\ro$, but accepts them unconditionally. In other words, the verifier accepts proofs where challenges after $i$ are picked by the adversary. If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ is $\ur{i}$.
\end{definition}
%
