...
\begin{definition}[$(n_1,	\ldots, n_\mu)$-computational special soundness]
	...
	We say that $\psfs$ is $(n_1, \ldots, n_\mu)$-computational special sound with security loss $\epss(\secpar)$, if there exists an expected polynomial time algorithm $\tdv$, and an expected polynomial time extractor $\extt$, such that for any PPT adversary $\advse$, the following two conditions hold:
	\begin{align*}
	\Pr\left[
	...
	\begin{aligned}
  	& r \sample \RND{\advse},
  	(\inp_{\advse}, \zkproof_{\advse}) \gets \advse^{\ro, \initU} (1^\secpar; r), \\
	&\verifier_\fs(\srs, \inp_{\advse}, \pi_\advse) = 1
	\end{aligned}
	\right] \geq 1-\varepsilon_t(\secpar).
	\end{align*}
where $\tree$ is a tree of accepting transcripts.  
	\begin{align*}
	\Pr\left[
	\REL(\inp, \wit) = 0
