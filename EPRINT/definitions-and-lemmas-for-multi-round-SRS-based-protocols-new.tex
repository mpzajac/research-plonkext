...
\begin{definition}[Simulation-extractable NIZK]
  ...
  $\simulator$ with \emph{extraction error} $\nu$ if for
...
%	& (\zkproof_{1}, \ldots, \zkproof_{j}) = (\zkproof'_{1}, \ldots, \zkproof'_{j}) \implies \zkproof_{j}.\ch = \zkproof'_{j}.\ch),\\
	...
	\]
	where $\verifier'(\srs,\inp,\zkproof=(a_1,c_1, \dots, a_\nu,c_\nu,a_{\nu+1}))$ checks that $c_i = \ro(\pi[0..i])$, for $i\in [0,\nu]\setminus k$. That is it allows the $k$th challenge to be programmed by $\adv$. 
