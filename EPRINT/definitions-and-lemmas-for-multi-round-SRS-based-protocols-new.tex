...
\begin{definition}[$(n_1,	\ldots, n_\mu)$-computational special soundness]
	...
	% For any $\ppt$ adversary $\advse^{\ro, \initU} (1^\secpar; r)$ with access to oracle $\initU$ defined in~\cref{fig:upd}, and random oracle $\ro$, \changedmx{we consider the algorithm $\tdv$}{there exists an expected $\ppt$ algorithm $\tdv$} that rewinds $\adv$ to produce a $(n_1, \ldots, n_\mu)$-tree of accepting transcripts $\tree$.
	
	We say that $\psfs$ is $(n_1, \ldots, n_\mu)$-computational special sound with security loss $\epss(\secpar)$, if there exists \changedmx{an expected}{a} polynomial time extractor $\extt$ and an expected polynomial time algorithm $\tdv$ such that for any PPT adversary $\advse$,
	\begin{align*}
	\Pr\left[
	\REL(\inp, \wit) = 0
	...
	& r \sample \RND{\advse},
	(\inp_{\advse}, \zkproof_{\advse}) \gets \advse^{\ro, \initU} (1^\secpar; r), \\
	&    \tree \gets \tdv((\srs,\adv,r), Q_{H}, \changedm{Q_{upd}}),
	\wit \gets \extt(\tree)
	\end{aligned}
	\right] \leq \epss(\secpar).
	\end{align*}
	...
	Here, $\srs$ is the SRS that $\advse$ finalised using the update oracle $\initU$.
	List $Q_\ro$ contains all $\advse$'s queries to $\ro$ and the random oracle's answers; \changedm{list $Q_{srs}$ contains $\advse$'s queries to $\initU$ and the oracle's responses}. We call the protocol $(k, n)$-computational special sound when $\tree$ is a $(k, n)$-tree of accepting transcripts.
\end{definition}
