...
\ourpar{Simulation-extractability for multi-message protocols.} \markulf{22.04}{Way too long and repeats stuff from the intro. We only need a very short paragraph.} \chaya{25.04}{condensed. please see if it reads well.}
Most recent SNARK schemes follow the same blueprint of construting an interactive information-theoretic proof system 
...
We first give a definition of simulation-extractability
...
we generalize the unique response property, define a zero-knowledge
property that we call trapdoor-less simulation, and define a notion of computational special
...
\markulf{22.04}{Could have ZK here. For updateable setting, it is also a new notion}
...
\begin{definition}[Simulation-extractable NIZK]
	\label{def:updsimext}
  \label{def:simext}
	Let $\ps = (\SRScer, \prover, \verifier, \simulator)$ be a NIZK proof system. We say that
...
\chaya{25.4}{define UR for FS compiled protocols?}
...
string.
 ...
 \michals{22.04}{Rename to Trapdoor-Less ZK?}
\markulf{22.04}{Lets give the zero-knowledge definition only once.}
\chaya{25.04}{cleaned this up a bit, please check.}
