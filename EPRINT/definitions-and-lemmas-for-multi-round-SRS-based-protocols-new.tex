...
\end{definition}
	...
	\begin{lemma}[Forking special soundness]\label{lem:forking_ss}
		...
		makes at most $n + q \cdot (n - 1)$ queries to $\advse$. Then $\proofsystem_\fs$ is $(k, n)$-forking special sound with security loss $\frac{\accProb - (q + 1) \epsk (\secpar)}{1 - \epsk (\secpar)} + \epsdlog$.
	\end{lemma}
