...
\begin{figure}
	\centering
		...
		%		Q_\ro \gets \{(x,H[x])\}\\
			...
			\pchspace
			%
			\procedure{$\boxed{\simulator\oracleo.\prover(\inp, \wit)} \ \simulator\oracleo.\prover'(\inp)$}
			{ 
...
In this paper we distinguish between simulators that require a trapdoor to simulate and those that do not. We call a protocol \emph{trapdoor-less
 ...
 Moreover, it may be not allowed to program every random oracle challenge but only challenges that come after the $k$-th prover's message. We call protocols which allow for such a simulation $k$-\emph{programmable trapdoor-less zero-knowledge}, and define this in~\cref{def:tls}.
...
language.
%; for simulator $\simulator = (\simulator_1, \simulator_2)$, $\simulator_2$
...
\begin{definition}[Simulation-extractable NIZK]
  ...
  $\simulator$ \hamid{29.4}{with security loss $\epsse(\lambda,\accProb, q)$? --- we should add $\epsse$ parameter somewhere!} if for
  ...
  \verifier(\srs, \inp, \zkproof) = 1, \\
   (\inp, \zkproof) \not\in Q,  \\
	...
	queries to $\simulator_1$ and the (simulated) random oracle's answers, $\abs{Q_\ro}\leq q$, and list $Q_\srs$ contains all $(\srs, \rho)$ of update SRSs and their proofs.
...
%	& (\zkproof_{1}, \ldots, \zkproof_{j}) = (\zkproof'_{1}, \ldots, \zkproof'_{j}) \implies \zkproof_{j}.\ch = \zkproof'_{j}.\ch),\\
	...
	\]
	where $\srs$ is the finalized SRS and $\verifier'(\srs,\inp,\zkproof=(a_1,c_1, \dots, a_\mu,c_\mu,a_{\mu+1}))$ checks that $c_i = \ro(\pi[0..i])$, for $i\in [0,\mu]\setminus k$\hamid{29.4}{$\verifier'(\srs,\inp,\zkproof=(a_1,c_1, \dots, a_\mu,c_\mu,a_{\mu+1}))$ should also run $\verifier(\srs,\inp,\zkproof=(a_1,c_1, \dots, a_\mu,c_\mu,a_{\mu+1}))$, no?}. That is, it allows the $k$th challenge to be programmed by $\adv$. 
	If $\epsur(\secpar)$ is negligible, we simply say $\proofsystem$ is $\ur{k}$.
\end{definition}

% \begin{definition}[Updatable $\ur{i}$-protocol]
% 	\label{def:wiuru}
% 	Let $\proofsystem$ be a $(2\mu + 1)$-message FS transform NIZK proof system
%   $\ps = (\SRScer, \prover, \verifier, \simulator)$. Denote
...
\begin{definition}[Tree of accepting transcripts, cf.~{\cite{EC:BCCGP16}}]
	...
	Consider a $(2\mu + 1)$-message proof system. \markulf{24.04}{Is this meaningful for interactive or also non-interactive prove systems obtained via FS?} A $(n_1,
...
\end{definition}
\michals{22.04}{Tree should be defined w.r.t. particular $x$}\hamid{22.4}{I checked other related papers e.g., Attema paper and they define it similarly without respect to a particular statement; so I think the definition is okay (unless we're in a different 'adaptive' setting?)} \markulf{24.04}{Accepting transcript is with respect to a statement? So $x$ should be defined.}
...
\markulf{22.04}{Don't repeat discussion from introduction. Improve it in one place.}
\chaya{29.04}{attempted to address}
