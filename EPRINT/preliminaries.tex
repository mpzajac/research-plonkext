...
\chb{\begin{remark}[Dlog assumptions in the updatable setting]\label{dlog-upd} Since all our protocols and security notions are in the updatable setting, we may require to rely on Dlog assumptions that are defined in the updatable setting. That is, the adversary $\adv$ in the $(q_1, q_2)\mhyph\dlog$ assumption (\cref{def:dlog}) is not given as input a Dlog challenge $\Ch = (\gone{1, \chi, \ldots, \chi^{q_1}}, \gtwo{1, \chi, \ldots, \chi^{q_2}})$, but access to an update oracle as defined in~\cref{fig:upd}, wherein an honestly generated SRS is defined as $\srs:=\Ch$ and update algorithm $\upd$ works by sampling $\chi' \sample \FF_p$ and defining the updated SRS as $(\gone{1, \chi'', \ldots, \chi''^{q_1}}, \gtwo{1, \chi'', \ldots, \chi''^{q_2}})$, where $\chi'' = \chi \chi'$. It is easy to see that $(q_1, q_2)\mhyph\dlog$ assumption reduces to the assumption in the updatable setting. The reduction $\reduction$ proceeds as follows: given the input dlog instance, $\reduction$ answers adversary's queries for the dlog updates and sets the honest update to be the input dlog instance. Once the dlog challenge in the updatable setting is finalized, it runs the adversary and obtains the answer $\chi'$. Let $\chi_1, \ldots, \chi_\ell$ be the partial discrete logarithms of dlog instances corresponding to the adversary's dlog updates. These values can be computed by $\reduction$ by extracting from the update proofs given by the adversary. $\reduction$ now returns  $\chi = \chi' (\chi_1 \chi_2 \ldots \chi_\ell)^{-1}$. The same argument holds for the $(q_1, q_2)\mhyph\ldlog$ assumption, ~\cref{def:ldlog}.
	\end{remark}}
