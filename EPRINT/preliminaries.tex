...
\begin{definition}[$(q_1, q_2)\mhyph\udlog$ assumption]\label{def:udlog}
	...
	$\chi \sample \FF_p$, and defines
	$\Ch' := 
	\left( \gone{\smallset{\chi^i A_i}_{i = 0}^{q_1}},
	\gtwo{\smallset{\chi^i B_i}_{i = 0}^{q_2}} \right)$.
	\end{itemize}
	Then
	\[
...
\begin{definition}[$(q_1, q_2)\mhyph\uldlog$ assumption]\label{def:uldlog}
	...
	\begin{itemize}
		...
		$\chi \sample \FF_p$, and defines
		$\Ch' := 
		\left( \gone{\smallset{\chi^i A_i}_{i = -q_1}^{q_1}},
		\gtwo{\smallset{\chi^i B_i}_{i = -q_2}^{q_2}} \right)$.
	\end{itemize}
	Then
	\[
	\prob{\bar{\chi} \gets \adv^{\initU}(\secpar)} \leq \negl,
