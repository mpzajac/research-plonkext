		...
		\begin{minipage}[t]{0.58\linewidth}
			\procedure{$\initU(\intent, \srs_n,\{\rho_j \}_{j=1}^n)$}{
		 	...
		 	\Scale[0.75]{ // Q_\srs = (Q^{(1)}_\srs, Q^{(2)}_\srs) \text{ s.t. }   Q^{(2)}_\srs \text{ contains the update proofs in } Q_\srs }\\
				...
				\t 
				\textcolor{red}{\td \gets \ext_\srs(\srs_n,, Q_\srs, \{\rho_j \}_{j=1}^{n})} \\
...
%   \end{minipage}
	...
	\caption{The oracle defines the notion of updatable SRS setup.}\hamid{24.3}{I think the red line above can be removed as we don't need trapdoor for simulation!}% The right oracle is the simulation oracle.} 
	\label{fig:upd}
\end{figure}
