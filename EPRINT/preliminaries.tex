...
\begin{definition}[$(\pR, \pS, \pT, \p{F}, 1)$-uber assumption]
  ...
  elements of $\GRP_1$. 
  \anca{do not get thie previous sentence. What are not target group elements but $\GRP_1$, you mean $\GRP_2$? }
...
\chaya{12.10}{this theorem is perhaps misplaced? There is no definition of simulation extractability yet}
...
$(\srs, \inp, \wit)$. We say that proof is acceptable if $\verifier (\srs, \inp, \zkproof)$ accepts it. In this paper we focus on proof systems where $\prover$ and $\verifier$ are given oracle access to a random oracle $\ro$. The simulator $\simulator$ is not only given access to $\ro$, but it can also \emph{program} it. That is, it can require that for $(x, y)$ of its choice, $\ro (x) = y$.
