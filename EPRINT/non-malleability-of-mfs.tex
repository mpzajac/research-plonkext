...
\begin{lemma}
	...
	\[
	\epscss(\secpar,\accProb, q) \leq \left(1 - \frac{\accProb - (q + 1) \left(1 - \frac{\multconstr + 3}{\abs{\FF_p}}\right)}{\frac{\multconstr + 3}{\abs{\FF_p}}}\right) + (\multconstr + 3)\cdot\epsudlog (\secpar) + (\multconstr + 3) \cdot \epsid (\secpar)\,,
	\]
...
\begin{corollary}[Simulation extractability of $\marlinprotfs$]
	...
	\]
	where $\epserr (\secpar) = \left(1 - \frac{\multconstr + 3}{\abs{\FF_p}} \right)$, $\abs{\FF_p}$ is the size of the field, and $\noofc$ is the number of constrains in the circuit. 
\end{corollary}
