...
\title{What Makes Fiat--Shamir SNARKS \\ (Updatable) Simulation Extractable?}
...
\begin{abstract}
  In the random oracle model (ROM), the three-message Fiat--Shamir (FS) transformation turns sigma protocols into
  signature schemes, non-interactive proof systems, and signatures of knowledge
  (SoK).  The security of the transformation relies on rewinding to
  ...
  We define three properties (trapdoor-less zero-knowledge, rewinding-based knowledge soundness, and a unique response property) that are sufficient for argument systems based on multi-round FS to be also simulation extractable in this strong sense. We show that three popular universal zero-knowledge SNARKs (Plonk, Sonic, and Marlin) satisfy these properties and are thus simulation extractable NIZKs and SoKs with updatable SRS out-of-the-box avoiding unnecessary compilation overheads.
\end{abstract}
