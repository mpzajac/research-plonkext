...
\begin{theorem}[Simulation-extractable multi-message protocols]
	...
	\emph{updatable simulation-extractable} against $\ppt$ adversaries that make up to $q$ random oracle
	queries\hamid{19.4}{Do we still need $\sharp$queries parameter $q$?}
	\michals{20.04}{Yes} and return an acceptable proof with probability at least $\accProb$.  The
...
%	\frac{\waccProb - (q + 1) \epsk(\secpar)}{1 - \epsk(\secpar)} -
	...
	proof as defined in \cref{def:updsimext}. Thus, \hamid{19.4}{The next equation should be changed. Todo: how should we compute $\epst(\secpar, \accProb)$ based on $\epst(\secpar, \waccProb)$?}\michals{21.04}{I think we have $\waccProb \leq \accProb + \epsur (\secpar)$}
	\begin{equation}
	\label{eq:frk}
	\extProb \geq \frac{\accProb - \epsur(\secpar) - (q + 1) \epsk(\secpar)}{1 - \epsk(\secpar)} 
	- \epsur(\secpar) - \epss(\secpar)\,.
	\end{equation}}
	\michals{21.04}{The final equation utilises Atteama et al lemma. I think we can be more general here. Just express the probability using $\epsur, \epst$ etc. and then make a corollary that expresses the same probability but with Attema et al result plugged instead of $\epst$.}
	\qed
\end{proof}
