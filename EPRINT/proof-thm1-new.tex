...
\end{theorem}
\hamid{28.1}{We do not need to have both KS error $\epsk$ and forking special sound error $\epss$ in the statement, because one can be computed by another and they're not independent!}

\hamid{28.1}{why do we need ZK/HVZK property for the interactive protocol? why TLS for NI system is not enough?}

\hamid{28.2}{Why do we need $\epszk$ parameter? Without it, it means the simulation is computationally indistinguishable from real prover; why this is not enough?}
...
\begin{proof}		
	...
	%
	\chb{\ngame{2} In this game the environment additionally aborts if extractor $\ext$ fails to build a
	...
	the error event in Game $\game{1}$. As previously, we have $\abs{\prob{\game{1}} - \prob{\game{2}}} \leq \prob{\event{\errfrk}}$, where $\event{\errfrk}$ denotes the event of $\ext$ failing to build a
	$(1, \ldots, 1, n, 1, \ldots, 1)$-tree of accepting transcripts.}
	\fbox{\begin{minipage}{45em}
	 \hamid{28.2}{Is this needed?:} We build an adversary $\bdv$ against forking special soundness such that,
...
\end{minipage}}
	...
	\chb{Denote by $\waccProb$ the probability that $\advse$ outputs a proof that is accepted
	and does not break $\ur{k}$-ness of $\ps$. By using the recursively constructed extractors of Attema et al.~\cite{EPRINT:AttFehKlo21} (see~\cref{fig:Attema-ext}),
	we get that a $(k, n)$-tree $\tree$ of accepting transcripts can be built in expected
	...
	TODO: show that transition from Game 2 to Game 3 adds $\epsdlog(\secpar)$ to the probability, where $\epsdlog(\secpar)$ is the hardness of $(\noofc + 2, 1)$-$\dlog$ problem.
