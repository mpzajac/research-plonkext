...
\begin{proof}	
	...
	We denote by $\waccProb$ the probability that $\advcss$ outputs an acceptable proof. We note that $\waccProb$ is at most $\epsur (\secpar)$ from the probability that $\advse$ outputs an acceptable proof. By the union bound $\accProb + \epsur (\secpar) \leq \waccProb \leq \accProb - \epsur (\secpar)$. 
	\michals{23.04}{I am not sure about that bound. Why we could have a case when $\waccProb \geq \accProb$? intuitively, $\advcss$ never outputs an acceptable proof if $\advse$ doesn't}
