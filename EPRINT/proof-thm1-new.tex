...
\begin{proof}	
	Let $(\srs, \inp, \zkproof) \gets \advse^{\initU, \simOH, \simOP'}(r_{\advse})$ be the USE adversary. We construct extractor $\extse (\srs, \advse, r_{\advse}, \Qro, \Qsrs, \Qsim)$ that outputs a witness $\wit$, such that $\REL(\inp, \wit)$ holds with high probability. To that end we define an algorithm $\advcss^{\initU,\ro}(r)$ against rewinding-based knowledge soundness of $\psfs$ that runs internally $\advse^{\initU, \simOH, \simOP'}(r_{\advse})$. Here $r = (\rsim, r_{\advse})$ with $\rsim$ the randomness that will be used to simulate $\simOP'$. 

	The code of $\advcss^{\initU,\ro}(r)$ hardcodes $r$, $\Qro$ and $\Qsim$ such that on randomness $r=(\cdot,r_{\advse})$ and random oracle replies $\Qro$, it does not use any randomness for simulation but deterministically returns proofs from $\Qsim$ but nevertheless queries $\simOHprog$. While it is hard to construct such an adversary without knowing $r$, $\Qro$ and $\Qsim$, it clearly exists and $\extse$ has all the necessary ingredients to construct $\advcss$.
	%
...
%
	...
	When $\advcss$ outputs a proof $\zkproof$ for $\inp$ such that $\event{E}$ holds, $\advur$ looks through lists $Q$ and $Q_\ro$ until it finds $\tzkproofs[0..k]$ such that $\tzkproof[0..k] = \tzkproofs[0..k]$ and a programmed random oracle query $\tzkproofs[k].\ch$ on $\tzkproofs[0..k]$.	$\advur$ returns two proofs for $\inp$:
		\begin{align*}
