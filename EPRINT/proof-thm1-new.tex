...
\begin{proof}	
	...
	% with probability greater than $\epscss(\secpar, \accProb)$ if $\advse$ makes $\extse$ fail with probability greater than $\extProb$. %Adversary $\advcss$ proceeds as follows. 
%
	Adversary $\advcss^{\ro, \initU}$ runs internally $\advse^{\initU, \simulator_1, \simulator'_2}$, hence it needs to take care of $\advse$'s oracle queries. %Since both algorithms use similar set of oracles, we write $\adv.\oracleo$ to state that a query is make to $\oracleo$ given to algorithm $\adv$ to avoid confusion.
