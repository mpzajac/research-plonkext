...
\begin{proof}	
	...
	Let $(\srs, \inp, \wit) \gets \advse^{\initU, \simulator\oracleo.\ro, \simulator\oracleo.\prover'}(r_{\advse})$ be the USE adversary. We construct an extractor $\extse (\srs, \advse, r_{\advse}, \Qro, \Qsrs, \Qsim)$ that outputs a witness $\wit$, such that $\REL(\inp, \wit)$ holds with high probability. To that end we define an algorithm $\advcss^{\initU,\ro}(r)$ against rewinding-based knowledge soundness of $\ps$ that runs internally $\advse^{\initU, \simulator\oracleo.\ro, \simulator\oracleo.\prover'}(r_{\advse})$. Here $r = (\rsim, r_{\advse})$. Eventually, we use the computational special soundness extractor $\extcss$ on $\advcss$ to extract the witness.
