...
\begin{lemma}[\cref{lem:plonkprot_ss} restated]
	...
	\]
	Here $\accProb$ is a probability that the adversary outputs an accepting proof, $\epsid(\secpar)$ is a soundness error of the ideal verifier for $\plonkprot$, and $\epsudlog(\secpar)$ is security of $(\numberofconstrains + 5, 1)$-$\udlog$ 
	assumption.
\end{lemma}
