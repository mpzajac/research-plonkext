...
\begin{proof}		
  ...
  transcripts of $\advse$'s operations \michals{4.10}{Explicitly introduce lists
    $Q, Q_\ro, Q_\srs$ here} is tasked to extract a witness $\wit_{\advse}$ such that
  ...
  output, i.e.~$\inp_{\adv}$. We show a reduction $\rdvur$ \michals{4.10}{The reduction also takes simulator's randomness} that utilises $\adv$
...
%
    ...
    \michals{4.10}{Define the actual game (not only $\bdv$'s description. Show how $\bdv$ gets $r_\sim$ and how it uses it.}
