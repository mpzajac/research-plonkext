...
tree of accepting transcripts $\tree$, which allows to extract a witness.
  ...
  \begin{lemma}[Generalized forking lemma~\cite{EPRINT:AttFehKlo21}]\label{lem:attema}
	Let $N = n_1 \cdot \cdots \cdot n_\mu$.  Let $\psfs$ be a NIZK proof system compiled using $()$ Fiat-Shamir ... has trial knowledge error where $\epsk(\secpar)= Er(n_1, \ldots, n_\mu; \Omega(2^\lambda))$ where
	$$
