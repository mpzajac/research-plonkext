...
\ncase{Conclusion} Probability that the adversary's outputs differ in Game 0 and Game 2 is upper-bounded by $\leq \epszk(\secpar) + \epsuber(\secpar)$. \qed
\end{proof}

\subsection{Simulation extractability of~$\plonkprotfs$}
Since \cref{lem:plonkprot_ur,lem:plonkprot_ss} hold, $\plonkprot$ is $\ur{2}$
and forking special sound. We now make use of \cref{thm:se} and show that
$\plonkprot_\fs$ is\COMMENT{ forking} simulation-extractable as defined in
\cref{def:updsimext}.

\begin{corollary}[\COMMENT{Forking }Simulation extractability of $\plonkprot_\fs$]
...
$\ro\colon \bin^* \to \bin^\secpar$ be a random oracle. Let $\advse$ be an
 adversary that can make up to $q$ random oracle queries, up to $S$
simulation oracle queries, and outputs an acceptable proof for $\plonkprotfs$
with probability at least $\accProb$. Then $\plonkprotfs$ is \COMMENT{forking
