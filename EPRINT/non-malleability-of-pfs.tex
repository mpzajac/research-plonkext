...
\begin{lemma}
  ...
  $\plonkprot$ is $(3, 3 \noofc + 1)$-updatable computational special sound with security loss $(\epst(\accProb, \secpar), \epsid (\secpar) + \epsdlog (\secpar))$ against algebraic adversary $\adv^{\ro, \initU} (\secparam)$ where
  \[
...
\begin{proof}
  ...
  \ncase{Game 0} %
  In this game the adversary wins if it outputs a valid instance--proof pair $(\inp, \zkproof)$, and the extractor $\ext$ does not manage to output a witness $\wit$ such that $\REL (\inp, \wit)$ holds.
