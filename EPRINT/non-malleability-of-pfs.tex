...
Given forking special soundness and $\ur{2}$ of $\plonkprot$, we invoke \cref{thm:se} and conclude that $\plonkprot_\fs$ is \COMMENT{forking }simulation-extractable.\hamid{19.1}{To conclude USE of Plonk, I think we should rely on TLS property too!}
...
\begin{lemma}
  ...
  property with loss $\epsop(\secpar)$, \changedm{let $\plonkprotfs$ be $\epss$-sound and
    $(\noofc + 2, 1)$-dlog problem by $\epsdlog (\secpar)$ hard.} Then probability that a
...
\begin{proof}
  ...
  \item $\verifier$ would check these each of these openings separately,
  \end{itemize}
...
\begin{lemma}
	...
	Let $\plonkprot$'s idealized verifier fails with \hamid{11.3}{negligible?} probability $\epsid (\secpar)$, and
	$(\noofc + 2, 1)$-$\dlog$ problem be $\epsdlog (\secpar)$ hard. Then $\plonkprot$ is
	$(3, 3 \noofc + 1)$-forking special sound with security loss $\epsid (\secpar) + \epsdlog (\secpar)$ against algebraic
	adversary $\adv$.
\end{lemma}
...
\begin{lemma}
  ...
  is 2-programmable trapdoor-less simulatable zero knowledge with simulator $\simulator$
...
forking special sound.\hamid{19.1}{Again, TLS is not mentioned?} We now make use of \cref{thm:se} and show that
