...
\changedm{
  \begin{proof}
    ...
    commitment scheme. The proof goes by game hops.

    \ncase{Game 0} In this game the adversary wins if it breaks either evaluation
    binding or unique opening property for one of the committed polynomials.
    ...
    \ncase{Game 0 to Game 1} Note that since did not win in Game 0, it did not break
    polynomial commitment binding nor unique opening property. Hence, since KZG
    polynomial commitment scheme is deterministic, $\adv$ had to commit to different
    polynomials in both proofs. Since these polynomials are uniquely determined by
    the first messages of the protocol, one of the proofs is incorrect.
    Probability that the verifier accepts an incorrect proof is at most $\epss$.
