...
\begin{lemma}
	...
	property with loss $\epsop(\secpar)$, let $\plonkprotfs$ be $\epss$-sound and
		$(\noofc + 2, 1)$-$\dlog$ problem \textcolor{blue}{in the updatable setting (See~\cref{dlog-upd})} be $\epsdlog (\secpar)$ hard. Then $\plonkprotfs$ is $\ur{2}$ \chb{against algebraic adversaries} with security loss $\epsdlog(\secpar) + \epss (\secpar) + 8 \cdot \epsbind (\secpar) + 9 \cdot \epsop(\secpar)$.
\end{lemma}
...
\ngame{0} In this game, the adversary wins if it provides two transcripts that match on the
first two messages of the proof.

\ngame{1} In this game, the adversary additionally wins if it provides two transcripts that
match on the first three messages of the proof.
...
\michals{25.03}{But the adversary can provide invalid commitment and then try to break e.g. evaluation binding. More precisely, the adversary picks $c^0 = \gone{t^0 (\chi)}$ that is correct and $c^1 = \gone{t^1 (\chi)}$ that is incorrect. Then, in the later part of the protocol with $c^1$, it tries to break evaluation binding to open $t^1$ as if $t^1 = t^0$. Probability for that is negligible but we cannot rule it just by using polynomial commitment deterministicy.}
...
assumption in the updatable setting if $\p{t^0}(X) \neq \p{t^1} (X)$ and $\gone{\p{t^0_{lo}}(\chi),
...
other is thus upper bounded by $\epsdlog (\secpar) + \epss (\secpar)$.
