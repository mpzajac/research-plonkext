...
\michals{25.10}{To do: rewrite as an FS protocol}
...
\begin{description}
\item[Move 1] Sample $b_1, \ldots, b_9 \sample \FF_p$; compute
...
\item[Move 2] Compute challenges $\beta, \gamma \in \FF_p$ by querying random oracle
...
\item[Move 3] Compute the challenge $\alpha = \ro(\zkproof[0..2])$, compute the quotient
  polynomial
	\begin{align*}
    & \p{t}(X)  = \\
...
\item[Move 4] Get the challenge $\chz \in \FF_p$, $\chz = \ro(\zkproof[0..3])$.
...
\item[Move 5] Compute the opening challenge $v \in \FF_p$,
...
\begin{enumerate}
  ...
  \item In prover's Move 3, it computes polynomial $\pt(X)$ honestly, however uses
...
\begin{proof}
  ...
  message where the challenge $\chz$ is presented, thus tree $\tree$ is
	...
	$\p{a} (X), \p{b} (X), \p{c} (X), \p{z} (X)$ described in Move 3 of the
...
\begin{proof}
  ...
  different values of $\chz$.
	...
	described in Move 3 of the prover holds. Hence, $\p{a}, \p{b}, \p{c}$
...
\begin{compactdesc} 
\item[Move 1] $\rdv$ computes $\gone{\pa(\chi)}$ using as
...
\item[Move 3] The reduction computes
...
\item[Move 4] The reduction outputs
...
\item[Move 5] $\rdv$ computes the openings of the polynomial commitments assuring
