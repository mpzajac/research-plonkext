...
with two different valid commitments for the same polynomial. Without loss of generality, let $\gone{\p{t^0_{lo}}(\chi), \p{t^0_{mid}}(\chi), \p{t^0_{hi}}(\chi)}$ be invalid commitments to incorrect polynomials (i.e., different from split polynomials of $\pt(X)$) provided by the adversary.  
If the adversary can later open any of these commitments $\gone{\p{t^0_{\star}}(\chi)}$ as if $\p{t^0_{\star}} = \p{t_{\star}}$ for any $\star \in \{\p{lo, mid, hi}\}$, it can break the evaluation binding property, which by assumption is upper-bounded by $\epsbind(\secpar)$.
