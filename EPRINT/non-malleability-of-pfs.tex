...
\begin{lemma}
	...
	property with loss $\epsop(\secpar)$, let $\plonkprotfs$ be $\epss$-sound and
		$(\noofc + 2, 1)$-$\udlog$ problem be $\epsdlog (\secpar)$ hard. Then $\plonkprotfs$ is $\ur{2}$ \chb{against algebraic adversaries} with security loss $\epsdlog(\secpar) + \epss (\secpar) + 8 \cdot \epsbind (\secpar) + 9 \cdot \epsop(\secpar)$.
\end{lemma}
