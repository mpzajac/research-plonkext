...
\begin{proof}[Intuitions]
  ...
  \]
  Then we analyse the case if one of the proof in the tree $\tree$ outputted by $\tdv$ is not acceptable by the ideal verifier. This discrepancy can be used to break an instance of an updatable dlog assumption what happens with probability at most $\epsudlog (\secpar)$. Additionally, it may be impossible to extract the witness from a tree where each of the transcripts is acceptable by the ideal verifier if the adversary broke soundness of the ideal verifier in one of the transcripts. That happens with probability at most $(3 \noofc + 5) \epsid (\secpar)$.
 \end{proof}
