...
\end{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\linewidth}
      \begin{tikzpicture}
      \draw (0,0) node (gate) {\Huge{$g$}};
      \draw (-1, -1) node (leftinp) {\Huge{$a_1$}};
      \draw (1, -1) node (rightinp) {\Huge{$b_7$}};
      \draw (0, 1.5) node (out) {\Huge{$c_2$}};      
      \draw[thick,->] (leftinp.north) to (gate.west);
      \draw[thick,->] (rightinp.north) to (gate.east);
      \draw[thick,->] (gate.north) to (out.south);
      \end{tikzpicture}
    \end{column}
  \end{columns}
  \begin{columns}
