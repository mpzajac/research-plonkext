...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Intuitions about extracting witness from proofs (and some problems
      we had to solve)
    \item Glimpse into our main proof for simulaiton extractability
  \end{enumerate}
\end{frame}
...
\begin{column}{0.3\linewidth}
  \only<2>{\begin{tikzpicture}[scale=0.7]
...
\begin{frame}
  ...
  \begin{block}{}
    \begin{compactenum}
    \item<1-> Let the adversary $\adv$ commit to polynomials $\p{a} (X), \p{b} (X),
      \p{c} (X)$
      (only these polynomials are required to learn the whole witness)
    \item<2-> Run a proof till evaluation challenge $\chz$ is given
    \item<3-> \label{it:challenge} Pick a random $\chz$ and send it to $\adv$
...
\begin{frame}
  ...
  \begin{block}{}
    \begin{enumerate}
    \item<1-> What if $\adv$ utilizes some simulated proof up to Step
      \ref{it:challenge}?\\
      ...
      \fbox{Need to make sure that is not possible -- \emph{unique response
            property}}
    \item<2-> What is the probability that $\adv$ doesn't change its mind after
      seeing new $\chz$? \\
