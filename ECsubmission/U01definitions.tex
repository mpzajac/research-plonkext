...
Previous related definitions and results were formulated for $3$-round protocols that require two transcripts for standard model extraction,e.g. the work of Faust et al.\cite{INDOCRYPT:FKMV12}. 
In this section we prepare our analysis for multi-round protocols with an universal or updatable SRS. 
In order to prove simulation-extractability for those,  we will require more than
just two transcripts for extraction.  Moreover, we will consider protocols that rely on an SRS,  in the updatable setting, where the adversary gets to contribute to the SRS in the updatable setting which makes the analysis more complicated.
