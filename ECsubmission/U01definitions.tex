...
We first give a definition of simulation-extractability which we base on~\cite{INDOCRYPT:FKMV12} adapted to the updatable SRS setting. Then, we adapt special soundness to forking special soundness, and generalize the forking lemma and the unique response property for
...
\begin{definition}[Simulation-extractable NIZK]
  ...
  simulation oracle $\simO$, cf.~\cref{fig:upd}, and a random oracle $\ro$, and produces an
  accepting transcript of $\ps$ with probability $\accProb$, where
	\[
	\accProb = \condprob{
	\begin{matrix}
...
\begin{definition}[($k$-programmable) Trapdoor-Less Simulatable Proof System]
  ...
  Fiat--Shamir variant and $\ro$ the random oracle. $\simulator_{\fs}$ takes as input an SRS $\srs$
...
\begin{definition}[$(\eps(\secpar), k,n)$-forking special soundness in the updatable setting]
	Let $\proofsystem = (\kgen, \prover, \verifier, \simulator)$ be an
	$(2 \mu + 1)$-message proof system for a relation $\REL$.
...
how it is used in our \COMMENT{forking} simulation-extractability proof.  Let $\proofsystem$ be a
...
randomness; $Q$ the list of simulator queries made by $\advse$ \markulf{20/09/2021}{Q is now hidden within the forking special soundness adversary.} along with
