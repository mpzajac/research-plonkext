...
since the protocols we consider have an \hamid{updatable} SRS, more than three messages, require more than
...
multi-round SRS-based protocols \hamid{in the updatable setting}.

\hamid{I think we should somewhere write about the fact that the following definitions are in the updatable setting, meaning there is an SRS update oracle defined in \cref{fig:upd}, and due to this, the simulation oracle $\simO$ in our SE definition is also w.r.t. the finalised SRS and is defined in \cref{fig:upd}.}
...
\begin{definition}[$(\eps(\secpar), k,n)$-forking soundness in the updatable setting]
	...
	For any $\ppt$ adversary $\advse^{\initU,
		\ro} (1^\secpar; r)$ \hamid{with access to oracles $\initU$ defined in~\cref{fig:upd}, and random oracle $\ro$,} we consider the procedure $\zdv$ that provided the transcript $(\srs, \adv, r, Q_H)$ and $h_1, \ldots, h_q$ runs $\adv$ by providing it with random oracle queries
	and simulated proofs \hamid{1-why simulated proofs? 2-...and update oracle queries?}. While $Q_{H}$ is consistent with $h_1, \ldots, h_q$, it replays the proofs of $Q$.
	%
	$\zdv$ returns the index $i$ of the
	random oracle query made for challenge $k$ and the proof $\adv$ returns.
	...
	transcripts such that none of the $n$ challenges in round $k$ were used in
	simulated proofs.\hamid{what are simulated proofs? we don't have it anymore, no?}
