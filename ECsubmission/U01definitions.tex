...
\begin{definition}[$(\eps(\secpar), k,n)$-forking soundness in the updatable setting]
	...
	For any $\ppt$ adversary $\advse^{\initU, \simulator_\fs,
		\ro} (1^\secpar; r)$ \hamid{with access to oracles $\initU$ and $\simO$ defined in~\cref{fig:upd}, and random oracle $\ro$,} we consider the procedure $\zdv$ that provided the transcript $(\srs, \adv, r, Q_H)$ and $h_1, \ldots, h_q$ runs $\adv$ by providing it with random oracle queries
	and simulated proofs. While $Q_{H}$ is consistent with $h_1, \ldots, h_q$, it replays the proofs of $Q$.
	%
	$\zdv$ returns the index $i$ of the
	random oracle query made for challenge $k$ and the proof $\adv$ returns.
...
\begin{definition}[Forking simulation-extractable NIZK, \cite{INDOCRYPT:FKMV12}]
	...
	adversary $\adv$ that is given oracle access to an updatable SRS setup $\initU$, a simulation oracle $\simulator_\fs$ \hamid{(See~\cref{fig:upd})}, and a random oracle $\ro$, and produces an accepting transcript of $\ps$ with
