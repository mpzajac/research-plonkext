...
\hamid{Please uncomment if you agree: In the next section, we define security notions in the updatable setting. To this end, we define an SRS update oracle $\initU$ in~\cref{fig:upd} by which the adversary finalises the SRS. We also define the simulation oracle $\simO$ in~\cref{fig:upd} that is the simulator w.r.t. the SRS finalised by $\initU$. This simulation oracle will be used in the SE definition.}
