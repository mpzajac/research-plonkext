...
\emph{statistical distance} $\SD(X_\secpar, Y_\secpar)=\epsilon(\secpar)$ if $\sum_{a \in
...
group $\GRP$, if the relation is a relation of group elements (as is usually the case for zkSNARKs).
...
\]
where the adversary has access to simulated signatures via the oracle $\simsign_\td(\inp_i, \mesage_i) \coloneq \simsign(\td, \inp_i, \mesage_i)$ and the extractor $\ext_\adv$ takes as input the transcript ${\sf trans}_\adv$ of all queries made by $\adv$ including its randomness. \markulf{30/9}{Added the part about randomness. I don't dislike the Queries SimSign td notation. Lets use it consistently throughout paper? Or also use Q here?
Where is td generated? changed signsetup to simsetup}
...
the $\srs$ defines the relation, and for universal proof systems, such as Plonk
