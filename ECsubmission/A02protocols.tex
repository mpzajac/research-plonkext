...
\begin{proof}
  ...
  The reduction $\rdvdlog$ proceeds as follows: $\rdvdlog$ initializes the SRS $\srs$ using the input $\dlog$ instance, and then answers $\adv$'s queries for SRS updates.\hamid{We can also say: $\rdvdlog$ builds a SRS $\srs$ using the input $\dlog$ instance. I.e., it answers $\adv$'s queries for SRS updates and sets the honest update of the SRS to be $\srs$.} Let $\srs'$ be the finalised SRS. 
