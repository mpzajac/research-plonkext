...
$\maxdeg$ can be read from the output SRS.\hamid{While the figures only describe trusted SRS setup, it is not hard to lift the SRS generation into the updatable setting by defining the extra algorithms $\upd$, $\verifyCRS$ as defined in~\cref{def:upd-scheme}.}


We emphasize the following properties of a secure polynomial commitment $\PCOM$: \hamid{Note that since we are in the updatable setting, $\srs$ in the following definitions is the SRS that $\advse$ finalises using the update oracle $\initU$ (See~\cref{fig:upd}).}
...
%				& \srs \gets \kgen(\secparam, \maxdeg),\\
				& (\vec{c}, \vec{z}, \vec{s}, \vec{s}', \vec{o}, \vec{o}') \gets \adv(\secparam, \maxdeg)
			\end{aligned}
			\right.\right] \leq \negl\,.
	\]
  ...
  be the commitment openings. Then for every $\ppt$ adversary $\adv$
	\[
		\Pr
			...
			\begin{aligned}
%				& \srs \gets \kgen(\secparam, \maxdeg),\\
				& (\vec{c}, \vec{z}, \vec{s}, \vec{o}, \vec{o'}) \gets \adv(\secparam, \maxdeg)
			\end{aligned}
			\right.\right] \leq \negl\,.
	\]
\end{description}
  ...
  \begin{align*}
    ...
    \begin{aligned}
      ...
      \end{aligned}
        \,\left|\,
        \begin{aligned}
    & (f_0, f_1, c, k, b') \gets \adv^{\oraclec}(\secparam, \maxdeg), f_0, f_1 \in \FF^{\maxdeg}
    [X]
    \end{aligned}
