...
\label{sec:pcom}
A polynomial commitment scheme $\PCOM = (\kgen, \com, \open, \verify)$ consists in four algorithms and allows to commit to a
...
crucial in showing \COMMENT{forking }simulation-extractability of $\plonk$ and $\sonic$. We show
...
\item[Hiding:] Let $\HHH$ be a set of size $\maxdeg + 1$ and $\ZERO_\HHH$ its
  ...
  \begin{align*}
    \condprob
   { b' = b}{
    (f_0, f_1, c, k, b') \gets \adv^{\initU, \oraclec}(\maxdeg), f_0, f_1 \in \FF^{\maxdeg}
    [X]}
...
know the underlying polynomial $\p{f}$ which commits to that value.
