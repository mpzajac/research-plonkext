...
\begin{definition}[Updatable Forking simulation-extractable NIZK, \cite{INDOCRYPT:FKMV12}]
	\label{def:updsimext}
	...
	adversary $\adv$ that is given oracle access to an updatable SRS setup $\initU$\hamid{TODO: clarify that we mean the LUSE version of $\initU$ here!}, a random oracle $\ro$ and
	...
	\begin{aligned}
	& \verifier_\fs(\srs, \inp_{\advse}, \zkproof_{\advse}) = 1 \ \wedge \\
	& (\textcolor{blue}{\srs}, \inp_{\advse}, \zkproof_{\advse}) \not\in Q \wedge
	\REL(\textcolor{blue}{\srs}, \inp_{\advse}, \wit_{\advse}) = 1
	\end{aligned}
	\, \left| \,
	\begin{aligned}
	& r \sample \RND{\advse},
	...
	$\accProb \geq \nu$. List $Q$ contains all \hamid{$(\srs, \inp, \zkproof)$ tuples} where \hamid{$\srs$ is the finalised SRS.}\hamid{In the case of USE, this should be: $\srs$ is the finalised SRS at the time of making the query to $\simulator$.}
...
\begin{figure}[!ht]
	\begin{center}
		...
		\begin{minipage}[t]{0.6\linewidth}
			...
			\wit_\advse \gets \ext_\uks(\srs, \advse, r, \inp_{\advse}, \zkproof_\advse, Q, Q_\ro, Q_\srs) \\
			...
			\\
				...
				\wit_\advse \gets \ext_{\luse}(\srs, \advse, r, \inp_{\advse}, \zkproof_\advse, Q, Q_\ro, Q_\srs) \\
				\pcreturn \verifier(\srs, \inp_\advse, \zkproof_\advse) \wedge (\srs, \inp_\advse, \zkproof_\advse) \not \in Q \wedge (\srs, \inp_\advse, \wit_\advse) \notin \REL\\				
				%
				\\
				%
				...
				\srs \gets \bot, Q_\srs \gets \emptyset \\
				(\inp_\advse, \zkproof_\advse) \gets \advse^{\initU, \simO, \ro}(1^\secpar; r) \\
				\wit_\advse \gets \ext_{\use}(\srs, \advse, r, \inp_{\advse}, \zkproof_\advse, Q, Q_\ro, Q_\srs) \\
				\pcreturn \verifier(\srs, \inp_\advse, \zkproof_\advse) \wedge (\srs, \inp_\advse, \zkproof_\advse) \not \in Q \wedge (\srs, \inp_\advse, \wit_\advse) \notin \REL \\
		}
		\end{minipage}
