...
\hamid{Maybe last paragraph can be shortened as follows: Given forking-soundness of $\plonkprot$, and the fact that it is also
	$\ur{2}$, we make use of \cref{thm:se} and show that
	$\plonkprot_\fs$ is forking simulation-extractable.}
...
\begin{proof}
  Let
  $\adv(\REL,\srs = (\gone{1, \chi, \ldots, \chi^{\noofc + 2}}, \gtwo{\chi}))$ \hamid{what are $\adv$'s inputs in the updatable case?}
  ...
  \ngame{0} In this game, the adversary is \hamid{given the SRS} and wins if provides two
