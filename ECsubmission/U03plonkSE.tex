...
Given forking-soundness of $\plonkprot$, and the fact that it is also $\ur{2}$, we make use of \cref{thm:se} and show that $\plonkprot_\fs$ is forking simulation-extractable.

\hamid{Should we uncomment this: Due to page limit, we omit description of \plonk{} here and refer to
...
cf.~\cref{fig:pcomp}, then $\PCOMp$ has the unique opening property \hamid{(see~\cref{sec:pcom})} in the AGM
with security
...
\begin{proof}
  ...
  \hamid{The reduction $\rdvdlog$ in the case of updatable setting proceeds as follows: $\rdvdlog$ builds a SRS $\srs$ using the input $\dlog$ instance and answers $\adv$'s queries for SRS updates. It sets the honest update of the SRS to be $\srs$. Let $\srs'$ be the finalised SRS. Now, any discrepancy between the idealised verifier rejection and real verifier acceptance indicates that $\adv$ can break $\dlog$ assumption for $\srs'$ by computing the exponent $\chi'$.
  Let $\chi_1, \ldots, \chi_\ell$ be the partial trapdoors of $\adv$'s SRS updates. These trapdoors can be extracted by $\rdvdlog$ from the $\adv$'s update proofs. $\rdvdlog$ returns $\chi = \chi' (\chi_1 \chi_2 \ldots \chi_\ell)^{-1}$.}
\end{proof}
