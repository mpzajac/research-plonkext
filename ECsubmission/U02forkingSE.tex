...
\begin{proof}		
  ...
  as a black-box:
  \begin{compactitem}
  	\item \hamid{The reduction answers $\adv$ update queries by asking the same query from the update oracle in the unique response experiment. The reduction finalises the same SRS $\srs$ as the one $\adv$ does.}
...
%
		\ngame{2} Define an adversary $\bdv$ against forking soundness such that, given access to oracles $\initU$ and $\ro$, and randomness $r_\bdv$, it internally runs $\advse^{\initU, \simO, \ro} (1^\secpar; r_\adv)$, where
		\begin{compactenum}
