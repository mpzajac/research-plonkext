...
simulation \emph{trapdoor-less simulatable} (TLZK). More precisely,
...
\begin{lemma}
  ...
  $\sonicprot$ is $(\epsss(\secpar), 2, \noofc + 1)$-forking sound against
...
%
  ...
  $\gone{\p{r} (\chi, 1), \p{r} (\chi, y), \p{s} (\chi, y), \p{t} (\chi, y)}$ use
    ...
    holds. Note that since the $\tdv$ produces $(\noofc + 1)$
